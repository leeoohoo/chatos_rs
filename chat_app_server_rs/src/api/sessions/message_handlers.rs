...
use crate::core::messages::{
    build_message, create_message_and_maybe_rename, message_turn_id, MessageOut, NewMessageFields,
};
...
fn merge_missing_project_requirement_execution_messages(
...
fn merge_missing_project_requirement_execution_turn_items(
    mut items: Vec<Value>,
    all_messages: &[Message],
    before_turn_id: Option<&str>,
) -> Vec<Value> {
...
mod tests {
    ...
    use super::{
        ...
        merge_missing_project_requirement_execution_turn_items, parse_compact_history_offset,
    };
    use crate::models::message::Message;
    ...
    fn compact_history_merge_adds_missing_project_requirement_execution_message() {
        ...
        let mut execution = build_message("exec-1", "user", "执行需求：登录流程");
