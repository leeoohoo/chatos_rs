...
#[derive(Debug, Clone)]
pub struct SandboxManagerInternalTlsConfig {
...
impl SandboxManagerInternalTlsConfig {
    pub fn from_env(host: IpAddr, public_port: u16) -> Result<Self, String> {
        let port = required_env("SANDBOX_MANAGER_INTERNAL_MTLS_PORT")?
            .parse::<u16>()
            .map_err(|err| {
                format!("SANDBOX_MANAGER_INTERNAL_MTLS_PORT must be a valid port: {err}")
            })?;
        if port == public_port {
            return Err(
                "SANDBOX_MANAGER_INTERNAL_MTLS_PORT must differ from SANDBOX_MANAGER_PORT"
                    .to_string(),
            );
        }
        Ok(Self {
            bind_addr: SocketAddr::new(host, port),
            server_cert_path: PathBuf::from(required_env("SANDBOX_MANAGER_MTLS_SERVER_CERT_PATH")?),
            server_key_path: PathBuf::from(required_env("SANDBOX_MANAGER_MTLS_SERVER_KEY_PATH")?),
            client_ca_cert_path: PathBuf::from(required_env(
                "SANDBOX_MANAGER_MTLS_CLIENT_CA_CERT_PATH",
            )?),
        })
    }
}
...
pub fn load_internal_mtls_config(
    config: &SandboxManagerInternalTlsConfig,
...
fn load_internal_mtls_config_from_paths(
    ...
    for certificate in read_certificates(client_ca_cert_path)? {
        client_roots
            .add(certificate)
            .map_err(|err| format!("invalid Sandbox Manager mTLS client CA: {err}"))?;
    }
    if client_roots.is_empty() {
        return Err("Sandbox Manager mTLS client CA contains no certificates".to_string());
    }
    let verifier = WebPkiClientVerifier::builder(Arc::new(client_roots))
        .build()
        .map_err(|err| format!("build Sandbox Manager mTLS client verifier failed: {err}"))?;
    let server_config = ServerConfig::builder()
        .with_client_cert_verifier(verifier)
        .with_single_cert(server_certificates, server_key)
        .map_err(|err| format!("build Sandbox Manager mTLS server config failed: {err}"))?;
...
mod tests {
    ...
    fn unique_test_dir(label: &str) -> PathBuf {
        ...
        std::env::temp_dir().join(format!(
            "chatos-sandbox-manager-mtls-{label}-{}-{nonce}",
            std::process::id()
        ))
    }
    ...
    fn generate_material(output_dir: &PathBuf) {
        let script = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
            .join("../../scripts/generate-sandbox-manager-mtls.sh");
        let status = Command::new(script)
            .arg(output_dir)
            .status()
            .expect("run Sandbox Manager mTLS generator");
