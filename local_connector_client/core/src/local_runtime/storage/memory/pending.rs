...
impl LocalDatabase {
    ...
    pub(crate) async fn pending_memory_messages_including_turn(
        &self,
        owner_user_id: &str,
        session_id: &str,
        include_turn_id: &str,
        ...
        let after_sequence = self
            .summary_end_sequence(session_id, summary.as_ref())
            .await?;
        self.list_summarizable_messages_after_including_turn(
            owner_user_id,
            session_id,
            after_sequence,
            include_turn_id,
    ...
    async fn list_summarizable_messages_after_including_turn(
        &self,
        owner_user_id: &str,
        session_id: &str,
        after_sequence: i64,
        ...
        .bind(owner_user_id)
        .bind(after_sequence.max(0))
        .bind(include_turn_id)
        .bind(limit.clamp(1, 2_000))
        .fetch_all(self.pool())
        .await
        .context("list summarizable local memory messages including active turn")
    }
}
