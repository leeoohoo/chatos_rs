...
该命令会自动识别 x64/ARM64、安装当前用户缺少的 .NET 8 SDK 与 Inno Setup、编译并生成自包含 EXE 安装包、安装到当前电脑并启动 ChatOS。普通用户安装不会运行仓库的开发测试套件；生成的安装包同时保留在 `clients\windows\BundleArtifacts\installer-<架构>`，可以继续分发给其他相同架构的 Windows 电脑。整个流程不需要管理员权限；正式对外发布时仍应对安装包进行代码签名。
