    ...
    with_db(
        |db| {
            ...
            Box::pin(async move {
                ...
                let mut cursor = db.collection::<Document>("ai_model_configs").find(filter, None).await.map_err(|e| e.to_string())?;
                let mut docs = Vec::new();
                while let Some(doc) = cursor.try_next().await.map_err(|e| e.to_string())? { docs.push(doc); }
                let mut items: Vec<AiModelConfig> = docs.into_iter().filter_map(|d| normalize_doc(&d)).collect();
...
pub async fn get_ai_model_config_by_id(id: &str) -> Result<Option<AiModelConfig>, String> {
    with_db(
        |db| {
            let id = id.to_string();
            Box::pin(async move {
                let doc = db.collection::<Document>("ai_model_configs").find_one(doc! { "id": id }, None).await.map_err(|e| e.to_string())?;
        ...
        |pool| {
            let id = id.to_string();
            Box::pin(async move {
                let row = sqlx::query_as::<_, AiModelConfigRow>("SELECT * FROM ai_model_configs WHERE id = ?")
                    .bind(&id)
                    .fetch_optional(pool)
                    .await
...
pub async fn delete_ai_model_config(id: &str) -> Result<(), String> {
    with_db(
        |db| {
            let id = id.to_string();
            Box::pin(async move {
                db.collection::<Document>("ai_model_configs").delete_one(doc! { "id": id }, None).await.map_err(|e| e.to_string())?;
                Ok(())
            })
        },
        |pool| {
            let id = id.to_string();
            Box::pin(async move {
                sqlx::query("DELETE FROM ai_model_configs WHERE id = ?").bind(&id).execute(pool).await.map_err(|e| e.to_string())?;
                Ok(())
            })
        }
    ).await
