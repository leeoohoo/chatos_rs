...
const COMPACT_RECENT_WINDOW_MIN: i64 = 400;
const COMPACT_RECENT_WINDOW_MAX: i64 = 5000;
const COMPACT_RECENT_WINDOW_FACTOR: i64 = 8;

fn compact_required_item_count(limit: Option<i64>, offset: i64) -> Option<usize> {
    let Some(limit) = limit else {
        return None;
    };
    if limit <= 0 {
    ...
    Some(
        (limit as usize)
            .saturating_add(offset.max(0) as usize)
            .saturating_add(2),
    )
}

fn choose_initial_compact_window(limit: Option<i64>) -> i64 {
    let limit_factor = limit
        .unwrap_or(COMPACT_RECENT_WINDOW_MIN)
        .max(1)
        .saturating_mul(COMPACT_RECENT_WINDOW_FACTOR);
    limit_factor
        .max(COMPACT_RECENT_WINDOW_MIN)
        .min(COMPACT_RECENT_WINDOW_MAX)
}

async fn fetch_compact_messages_from_recent_window(
    session_id: &str,
    limit: Option<i64>,
    offset: i64,
    ...
    if limit.is_none() {
        let full_messages = MessageService::get_by_session(session_id, None, 0).await?;
        let compact_messages = build_compact_history_messages(full_messages);
        debug!(
            target: "perf",
            "session_compact_fetch mode=full_unbounded session_id={} limit=none offset={} compact_count={} elapsed_ms={}",
            session_id,
            offset,
            compact_messages.len(),
            started_at.elapsed().as_millis()
        );
        return Ok(compact_messages);
    }
    ...
    let required_count = compact_required_item_count(limit, offset);
    let mut window_limit = choose_initial_compact_window(limit);
    ...
    loop {
        ...
        if enough_items {
            debug!(
                target: "perf",
                "session_compact_fetch mode=recent_window session_id={} limit={:?} offset={} attempts={} window_limit={} compact_count={} elapsed_ms={}",
                session_id,
                limit,
                offset,
                attempts,
        ...
        if window_limit >= COMPACT_RECENT_WINDOW_MAX {
            break;
        }
    ...
    debug!(
        target: "perf",
        "session_compact_fetch mode=fallback_full session_id={} limit={:?} offset={} attempts={} final_window_limit={} compact_count={} elapsed_ms={}",
        session_id,
        limit,
        offset,
        attempts,
...
pub(super) async fn fetch_session_messages_for_display(
    session_id: &str,
    limit: Option<i64>,
    offset: i64,
    compact: bool,
    compact_recent_strategy: bool,
) -> Result<Vec<Message>, String> {
    if compact {
        let compact_messages = if compact_recent_strategy {
            fetch_compact_messages_from_recent_window(session_id, limit, offset).await?
        } else {
            let messages = MessageService::get_by_session(session_id, None, 0).await?;
            build_compact_history_messages(messages)
        };
