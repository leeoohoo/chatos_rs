...
pub(super) fn definitions(now: &str) -> Vec<ConfigDefinitionRecord> {
    vec![
        ...
        definition(
            MEMORY_ENGINE_CLOUD_AGENT_OUTBOX_RECONCILE_MS_CONFIG_KEY,
            "Cloud Agent Outbox 补偿间隔",
            "低频补偿发布异常遗留的 Memory Engine Cloud Agent 事件，不用于任务发现",
            "Memory Engine / Queue",
            "service",
            Some("memory-engine"),
            "duration_ms",
            ...
            &[],
            "restart_required",
            &["MEMORY_ENGINE_CLOUD_AGENT_OUTBOX_RECONCILE_MS"],
            36471,
            now,
        ),
        definition(
            MEMORY_ENGINE_CLOUD_AGENT_OUTBOX_BATCH_SIZE_CONFIG_KEY,
            "Cloud Agent Outbox 补偿批次",
            "单轮最多补偿发布的 Memory Engine Cloud Agent 事件数量",
            "Memory Engine / Queue",
            "service",
            Some("memory-engine"),
            "integer",
            ...
            &[],
            "restart_required",
            &["MEMORY_ENGINE_CLOUD_AGENT_OUTBOX_BATCH_SIZE"],
            36472,
            now,
        ),
        definition(
