...
pub const REQUIREMENT_USER_STATUS_VALUES: &[&str] = &["draft", "reviewing", "approved"];
pub const PROJECT_TASK_USER_STATUS_VALUES: &[&str] = &["todo", "ready"];
...
pub fn requirement_user_status_values() -> Vec<&'static str> {
    REQUIREMENT_USER_STATUS_VALUES.to_vec()
}

pub fn project_task_user_status_values() -> Vec<&'static str> {
    PROJECT_TASK_USER_STATUS_VALUES.to_vec()
}
...
fn tool_definitions(include_delete_tools: bool) -> Vec<Value> {
    let mut definitions = vec![
        ...
        tool_definition(
            ...
            "Initialize or update the current project's base description and one-to-one profile fields such as background and introduction. Write human-readable content in the current user's language; preserve technical identifiers and proper nouns.",
            object_schema(
                vec![
        ...
        tool_definition(
            ...
            "Create a draft requirement in the current project. The initial status is always draft; execution statuses are system-managed and must not be inferred from planning activity. If the work is similar to a done requirement, create a new requirement instead of modifying or extending the done one. Do not attach new child requirements under a done parent requirement. Write all human-readable requirement fields in the current user's language; preserve technical identifiers and proper nouns.",
            object_schema(
                vec![
        ...
        tool_definition(
            ...
            "Update a requirement and optionally replace its prerequisite requirement ids. User-managed status changes are limited to draft, reviewing, and approved. Use approved after planning is ready for explicit execution; never set in_progress from planning activity because execution statuses are system-managed. Done requirements are immutable through MCP; create a new requirement for similar new work. Newly written human-readable fields must follow the current user's language; do not translate untouched fields unless requested.",
            object_schema(
                vec![
    ...
    definitions.extend([
        ...
        tool_definition(
            ...
            "Create a new typed technical document for a requirement, or update an existing one when document_id is provided. Done requirements are immutable through MCP, so create a new requirement for similar new work instead of editing completed docs. Keep each document focused and split long content by doc_type or title. Write the document title and prose in the current user's language while preserving code, commands, APIs, paths, and product names.",
            object_schema(
                vec![
                    string_field("requirement_id", "Requirement id."),
        ...
        tool_definition(
            ...
            "Create a todo project-management task/work item under a requirement. The initial status is always todo; execution statuses are system-managed and must not be inferred from planning activity. The requirement must be open and must already have at least one non-empty technical document. If similar work was already done under another requirement, create a new task for this requirement instead of modifying the completed one. Write the task title and description in the current user's language while preserving technical identifiers and proper nouns.",
            object_schema(
                vec![
        ...
        tool_definition(
            ...
            "Update a project-management task/work item and optionally replace its prerequisite project task ids. User-managed status changes are limited to todo and ready; in_progress and terminal statuses are system-managed from real Task Runner execution. Done project tasks are immutable through MCP; create a new task for similar new work. Newly written title or description text must follow the current user's language; do not translate untouched fields unless requested.",
            object_schema(
                vec![
    ...
    (
        ...
        object_schema(
            vec![
                ...
                enum_field(
                    "status",
                    "Optional planning status. Use approved when the requirement is ready for explicit execution; execution states are system-managed.",
                    REQUIREMENT_USER_STATUS_VALUES,
                ),
                optional_string_field("assignee_user_id", "Optional assignee user id."),
            ],
            vec![],
    ...
    (
        ...
        object_schema(
            vec![
                ...
                enum_field(
                    "status",
                    "Optional pre-execution status. Execution states are system-managed.",
                    PROJECT_TASK_USER_STATUS_VALUES,
                ),
                integer_field("priority", "Optional priority; higher means more important."),
                optional_string_field("assignee_user_id", "Optional assignee user id."),
                integer_field("estimate_points", "Optional estimate points."),
