...
mod tests {
    ...
    fn status_schema_values_do_not_advertise_archived() {
        ...
        assert_eq!(
            schemas::requirement_user_status_values(),
            vec!["draft", "reviewing", "approved"]
        );
        assert_eq!(
            schemas::project_task_user_status_values(),
            vec!["todo", "ready"]
        );
    }
    ...
    fn mutation_status_schemas_only_expose_user_managed_states() {
        ...
        let create_requirement = definitions
            .iter()
            .find(|tool| {
                tool.get("name").and_then(Value::as_str) == Some(tools::CREATE_REQUIREMENT)
            })
            .expect("create_requirement");
        assert!(create_requirement
            .pointer("/inputSchema/properties/status")
            .is_none());

        let update_requirement = definitions
            .iter()
            .find(|tool| {
                tool.get("name").and_then(Value::as_str) == Some(tools::UPDATE_REQUIREMENT)
            })
            .expect("update_requirement");
        let requirement_statuses = update_requirement
            .pointer("/inputSchema/properties/patch/properties/status/enum")
            .and_then(Value::as_array)
            .expect("requirement status enum");
        assert_eq!(
        ...
        let update_task = definitions
            ...
            .expect("update_project_task");
        let task_statuses = update_task
            .pointer("/inputSchema/properties/patch/properties/status/enum")
            .and_then(Value::as_array)
            .expect("project task status enum");
        assert_eq!(
            task_statuses,
            &vec![
    ...
    #[test]
    fn write_tool_descriptions_require_user_language_consistency() {
        let definitions = schemas::project_management_server_tool_definitions();
        for name in [
            ...
            assert!(description.contains("current user's language"), "{name}");
        }
    }
