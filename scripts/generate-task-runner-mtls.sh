...
if ! command -v openssl >/dev/null 2>&1; then
  echo "[ERROR] openssl is required to generate Task Runner mTLS material" >&2
  exit 1
fi
...
if material_is_current; then
  echo "[INFO] Task Runner mTLS material is current: $OUTPUT_DIR"
  exit 0
fi
...
openssl req -x509 -new -sha256 -days 3650 \
  ...
  -subj "/CN=ChatOS Task Runner Internal CA" \
...
openssl req -new -sha256 \
  ...
  -subj "/CN=task-runner-backend" \
  -out "$temporary_dir/server.csr"
cat >"$temporary_dir/server.ext" <<'EOF'
basicConstraints=critical,CA:FALSE
keyUsage=critical,digitalSignature,keyEncipherment
extendedKeyUsage=serverAuth
subjectAltName=DNS:task-runner-backend,DNS:task-runner,DNS:localhost,IP:127.0.0.1
EOF
...
echo "[INFO] generated Task Runner mTLS material: $OUTPUT_DIR"
