...
export function SettingsPage() {
  ...
  const healthQuery = useQuery({
    queryKey: ['task-runner', 'health'],
    queryFn: api.health,
  });
  const configQuery = useQuery({
    queryKey: ['task-runner', 'system-config'],
    queryFn: api.getSystemConfig,
  });
  const mcpServerQuery = useQuery({
    queryKey: ['task-runner', 'mcp-server-info'],
  ...
  const internalPromptsQuery = useQuery({
    queryKey: ['task-runner', 'task-runner-internal-prompts', internalPromptLocale],
  ...
  const builtinPromptPreviewQuery = useQuery({
    queryKey: ['task-runner', 'settings-mcp-prompt-preview', internalPromptLocale],
    queryFn: () =>
  ...
  const tabItems = [
    {
      ...
      children: (
        <SettingsOverviewTab
          ...
          onOpenMcpCatalog={() => navigate('/task-runner/mcp')}
        />
      ),
    },
    {
