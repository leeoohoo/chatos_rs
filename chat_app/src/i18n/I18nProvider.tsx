...
const resolveUiLocaleFromSettings = (response: UserSettingsResponse | null | undefined): UiLocale => {
  const effective = response?.effective || response?.settings;
  if (!effective || typeof effective !== 'object') {
    return 'zh-CN';
  }
  return normalizeLocale((effective as Record<string, unknown>).UI_LOCALE);
};
  ...
  React.useEffect(() => {
    ...
    void apiClient.getUserSettings(userId)
      .then((response) => {
        ...
        const nextLocale = resolveUiLocaleFromSettings(response);
