...
pub fn resolve_for_user(user_id: Option<&str>) -> Result<Option<ProcessIsolationSpec>, String> {
...
pub fn prepare_workspace_for_user(
    path: &Path,
    spec: Option<&ProcessIsolationSpec>,
) -> Result<(), String> {
...
pub fn terminal_helper_command(
    shell: &str,
    spec: Option<&ProcessIsolationSpec>,
    cwd: Option<&Path>,
    home: Option<&Path>,
    ...
    let target_command = helper_target_command(shell, spec)?;
...
fn helper_target_command(command: &str, spec: &ProcessIsolationSpec) -> Result<OsString, String> {
    if !filesystem_view_enabled(Some(spec))? || Path::new(command).is_absolute() {
        return Ok(OsString::from(command));
    }
    find_executable_in_path(command)
        .map(OsString::from)
        .ok_or_else(|| format!("resolve isolated command '{command}' from PATH failed"))
}
...
pub fn helper_env_vars() -> Vec<(&'static str, OsString)> {
    HELPER_ENV_KEYS
        .iter()
