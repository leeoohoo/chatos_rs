...
async fn task_runner_context_from_state(
    ...
    let task = state.task;
    let durable = DurableTaskState::from_record(&task)
        .map_err(|error| format!("durable Task state is invalid: {error}"))?;
    if task.metadata.id != run.owner_entity_id
        || task.metadata.scope.owner_user_id != run.owner_user_id
        || durable.current_run_id != run.run_id
    {
        return Err("durable Task identity does not match the frozen Task Runner run".to_string());
    }
    durable
        .validate_run_identity(&task.metadata.id, run)
        .map_err(|error| format!("durable Task identity is invalid: {error}"))?;
    let prompt_snapshot = durable.prompt_snapshot;
    let project_snapshot = durable.project_snapshot;
    let capability_snapshot = durable.capability_snapshot;
    ...
    let objective = durable.objective;
    let acceptance_criteria = durable.acceptance_criteria;
    let retry_instruction = durable.current_retry_instruction;
