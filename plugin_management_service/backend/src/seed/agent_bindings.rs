...
pub(super) async fn seed_agent_bindings(
    ...
    // The conversation agent executes locally. Remove stale server-owned MCP bindings on every
    // seed so deleted execution-plane capabilities cannot survive an upgrade in MongoDB.
    store
