...
尚未具备云端等价安全设施的配置继续 fail closed：仅声明 package-relative cwd 但仍使用 PATH command 的混合模式不会退化为明文、本地 OAuth Broker 或 MCP Management 本地执行。`PluginCloud` 当前仍声明 `cancel_supported=false`。
