...
impl TaskPluginRuntimeContext {
    pub(crate) fn server(owner_user_id: &str, project_id: &str) -> Self {
        Self {
...
pub(crate) async fn resolve_task_plugin_runtime_context(
    ...
    if project_id == PUBLIC_PROJECT_ID {
        return Ok(TaskPluginRuntimeContext::server(owner_user_id, &project_id));
    }
...
fn task_plugin_runtime_context_from_project(
    ...
    if context.project_id.trim() != expected_project_id.trim() {
    ...
    match context.workspace_provider {
        WorkspaceProviderKind::None => Ok(TaskPluginRuntimeContext {
            ...
            project_id: expected_project_id.trim().to_string(),
            workspace_id: None,
            device_id: None,
            runtime_provider: "server".to_string(),
