...
> 状态：实施中
> 创建日期：2026-07-21
> 最新增量（Presentations canonical XY scatter）：2026-07-27 新增 Presentations `1.30.0` 的 canonical XY `scatter`。创建、追加、检查、完整快照和安全替换统一使用 exact `<c:scatterStyle val="lineMarker"/>`，共享 numeric `x_values` 写入每个 series 的 literal `xVal`，series `values` 写入 literal `yVal`；series 支持 canonical line color、marker、marker size 和 smooth。主轴使用 visible bottom X value axis + left primary Y value axis；双轴使用 hidden top X value axis + visible right secondary Y value axis。现有 `category_axis_title` 对 scatter 表示 X-axis title，既有 bounds/log/ticks/units/number-format 合同只控制主/次 Y axis，X axis 暂保持 canonical auto scaling。检查新增逐 chart group raw `scatter_styles` 与 X/Y preview；缺失、未知或带额外属性样式降级只读，重复、错误 namespace 或超过 128 bytes 失败关闭。不可变 Release ID 为 `bundled-release-presentations-1-30-0`，发布时间 `2026-07-28T05:00:00Z`，artifact revision `presentations-1.30.0`，Catalog revision `2026-07-27.19`；Skill JSON/Instructions SHA-256 为 `2b1f563d6a59147150e6ddf137cc0ecdc1bfb958cdb05eb490de6cde4ff7e921`/`e54be1f958d34f70540dc7d234dd7663c4147774a659a03a9e60f0af125969b2`，Bundle hash 为 `f128f608c93af53970710fec3848bf44beb1e679c834f636da13a7f571730a08`，Manifest/Artifact SHA-256 为 `9a21487e60776d2616fe9f2338f04874259fe24e1d4ddfec8cd0e17eebdc2673`/`3a3310ff470674f0cadd6eb0bffb79a72953f0edeb905fc069ac83e801837b57`，macOS arm64/Windows x64 staged content SHA-256 均为 `ead8b679c38a646432a2d68cbde06b0f870be42aa9700f6990f15d68e7ac001d`，ready/all fingerprints 分别为 `4804728fce7404e7993ac329fdbb7b1a2b0b15d63f83b98419b03d96cd4c1e03` 和 `794c2c4ce1ec66c3cc5ad07343288bf935c037dca185fe3d579facdc82605d1b`。
...
| Presentations | 部分可用，本机多布局富媒体与简单矩形表格创建、保守编辑及页面渲染 | `1.30.0` 已覆盖六种原有 16:9 布局与正式 `table` layout、严格矩形 DrawingML 表格创建/追加及单元格/行列/格式保守编辑、editable text/bullets、PNG/JPEG、alt text、标准 speaker notes、真实 presentation-order 检查、distinct-output 追加、可见 slide/notes 的单 run 与同格式相邻 runs 精确替换、完整排列重排、可见位置删除，以及签名 LibreOffice/Poppler 连续 1–8 页 PNG、可选 PDF 和逐页视觉 QA 合同；标准 DrawingML chart 支持真实可见顺序只读检查，并以 literal caches 和内部唯一 chart part 创建/追加 2D clustered column、clustered horizontal bar、line、pie、area、doughnut、standard radar 与 lineMarker XY scatter，支持 raw `barDir`/`radarStyle`/`scatterStyle`、右/左/上/下图例、none/value/percentage 数据标签、category/X/value/Y 轴标题、可选严格 `#RRGGBB` 系列 line/fill color、line/scatter series 的 none/circle/square/diamond/triangle marker、`2–72` size 与逐系列 smooth 开关，以及 column/bar/line/area/radar/scatter series 的 primary/secondary value-axis 分配、独立 secondary value-axis title、主/次 Y 值轴不裁剪数据的显式 minimum/maximum、`2–1000` 对数刻度、正数 major/minor unit、none/inside/outside/cross major/minor tick mark 和九种受限 canonical number format。column/line/area/radar 双轴使用 primary bottom/left 与 hidden top/visible right；bar 使用 visible left/bottom 与 hidden right/visible top；scatter 使用 bottom/left X/Y valAx 与 hidden-top/right 次 Y valAx。只有无 chart relationships、公式、嵌入工作簿或外部数据，且由包含 chart type/direction/style、categories/x_values、全部格式、系列颜色、系列 marker、系列 smooth、轴归属、轴范围、对数刻度、轴单位、轴 tick marks 和轴数字格式字段的完整检查快照重建后与原 XML 字节完全一致的 ChatOS canonical chart 才可绑定 snapshot 与 SHA-256 安全替换类型、标题、categories/x_values、series、values、legend、data labels 和 axis titles。仍缺 merged/nested/复杂单元格、跨表复制与任意区域/边框级表格格式编辑、跨 shape/paragraph/格式边界富文本与跨 run notes 编辑、任意 Office/第三方或工作簿关联图表编辑、bubble/stock/surface/3D、tertiary axis、任意 scatter X-axis bounds/format、任意 display units/crossing behavior、任意 custom number format、任意复杂 chart styling/custom labels、chartEx/SmartArt、任意主题/母版导入和动画/转场 |
