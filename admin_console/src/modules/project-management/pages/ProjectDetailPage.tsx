...
export function ProjectDetailPage() {
  ...
  const profileQuery = useQuery({
    queryKey: ['project-management', 'project-profile', projectId],
    queryFn: () => api.getProjectProfile(projectId!),
    enabled: Boolean(projectId),
  });
  const requirementsQuery = useQuery({
    queryKey: ['project-management', 'requirements', projectId, showArchived],
    queryFn: () => api.listRequirements(projectId!, { include_archived: showArchived }),
    enabled: Boolean(projectId),
  });
  const workItemsQuery = useQuery({
    queryKey: ['project-management', 'work-items', projectId, showArchived],
  ...
  const workItemDepsQuery = useQuery({
    queryKey: ['project-management', 'work-item-deps', workItemDepTarget?.id],
    queryFn: () => api.listWorkItemDependencies(workItemDepTarget!.id),
    enabled: Boolean(workItemDepTarget),
  });
  const docQuery = useQuery({
    queryKey: ['project-management', 'requirement-documents', docTarget?.id],
  ...
  const invalidateProjectData = () => {
    queryClient.invalidateQueries({ queryKey: ['project-management', 'requirements', projectId] });
    queryClient.invalidateQueries({ queryKey: ['project-management', 'work-items', projectId] });
    queryClient.invalidateQueries({ queryKey: ['project-management', 'project-graph', projectId] });
  };
  ...
  const profileMutation = useMutation({
    ...
    onSuccess: (profile: ProjectProfileRecord) => {
      ...
      setEditingProfileField(null);
      queryClient.setQueryData(['project-management', 'project-profile', projectId], profile);
    },
