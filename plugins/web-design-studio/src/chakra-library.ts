...
export const CHAKRA_COMPONENT_VARIANTS: Record<string, ChakraComponentVariant[]> = {
  ...
  OverlayManager: [
    variant('confirm', '发布确认浮层', { kind: 'confirm', title: '发布当前设计？', description: '确认后会把当前页面发布到共享预览环境。' }, { content: '发布当前设计', width: 190, height: 48 }),
    variant('details', '组件详情浮层', { kind: 'details', title: '组件详情', description: '查看组件尺寸、响应式状态和最后修改信息。' }, { content: '查看组件详情', width: 190, height: 48 }),
    variant('form', '项目表单浮层', { kind: 'form', title: '编辑项目资料', description: '修改项目名称、类型和说明，提交后由管理器关闭浮层。' }, { content: '编辑项目资料', width: 190, height: 48 })
  ],
