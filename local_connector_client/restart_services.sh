...
start_frontend() {
  ...
  echo "[INFO] starting local connector client frontend on 0.0.0.0:$FRONTEND_PORT"
  if command -v tmux >/dev/null 2>&1; then
    tmux new-session -d -s "$FRONTEND_SESSION" \
      "cd '$FRONTEND_DIR' && LOCAL_CONNECTOR_CLIENT_FRONTEND_PORT='$FRONTEND_PORT' LOCAL_CONNECTOR_CORE_API_PROXY_TARGET='http://127.0.0.1:$CORE_PORT' npm run dev -- --host 0.0.0.0 --port '$FRONTEND_PORT' --strictPort 2>&1 | tee '$FRONTEND_LOG_FILE'"
  else
    nohup bash -lc "cd '$FRONTEND_DIR' && LOCAL_CONNECTOR_CLIENT_FRONTEND_PORT='$FRONTEND_PORT' LOCAL_CONNECTOR_CORE_API_PROXY_TARGET='http://127.0.0.1:$CORE_PORT' exec npm run dev -- --host 0.0.0.0 --port '$FRONTEND_PORT' --strictPort" \
