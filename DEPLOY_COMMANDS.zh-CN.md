...
指定服务部署只构建所选镜像，并且只重启使用这些镜像的运行服务。例如选择 `task-runner-backend` 时会一起重启同镜像的 API、Worker 和 Scheduler；没有选择的容器不会重启。`cloud`、`cloud-backends`、`cloud-frontends` 和 `all` 是显式的批量范围。
