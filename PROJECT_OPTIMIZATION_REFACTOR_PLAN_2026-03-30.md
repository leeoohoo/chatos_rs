...
- `agent_orchestrator/target`：`53G`
- `memory_server/backend/target`：`6.9G`
- `agent_workspace/node_modules`：`594M`
- `memory_server/frontend/node_modules`：`204M`
- `agent_orchestrator/docs/codex`：`400M`
- `agent_orchestrator/data/agent_orchestrator.db`：`80M`
- `agent_orchestrator/logs`：`21M`
...
- 多个 `__pycache__`
- 多个 `.pyc`
- `agent_orchestrator/docs/codex` 下存在嵌套复制内容
...
| Rust 后端 | `agent_orchestrator/src/services/v3/ai_request_handler/parser.rs` | 1119 | 协议解析器过大 |
| 前端 store | `agent_workspace/src/lib/store/actions/sendMessage.ts` | 885 | optimistic UI、SSE、工具面板、错误处理、runtime 合并都在一起 |
| 前端 UI | `agent_workspace/src/components/SessionList.tsx` | 881 | 状态和动作编排过重 |
| Memory 前端 | `memory_server/frontend/src/pages/JobConfigsPage.tsx` | 852 | 管理端配置职责集中 |
| 前端数据层 | `agent_workspace/src/lib/api/client.ts` | 831 | API 面过大，`Promise<any>` 很多 |
| Rust 后端 | `agent_orchestrator/src/core/chat_runtime.rs` | 826 | metadata 兼容逻辑和运行时解析热点 |
| Rust 后端 | `agent_orchestrator/src/api/chat_v3.rs` | 818 | API、鉴权、SSE、runtime guidance、编排混在一起 |
...
| 前端 UI | `agent_workspace/src/components/ProjectExplorer.tsx` | 729 | 即使已经抽了 hook，主容器仍然过重 |
...
- `agent_orchestrator/target` 已达 `53G`
- `memory_server/backend/target` 已达 `6.9G`
- `agent_orchestrator/docs/codex` 达到 `400M`
...
6. `agent_orchestrator/docs/codex` 更适合作为外部缓存或子模块，而不是常规仓库内容
...
- `agent_orchestrator/src/core/chat_runtime.rs:42`
- `agent_orchestrator/src/core/chat_runtime.rs:90`
- `agent_orchestrator/src/core/chat_runtime.rs:101`
- `agent_orchestrator/src/core/chat_runtime.rs:110`
- `agent_orchestrator/src/core/chat_runtime.rs:120`
- `agent_orchestrator/src/core/chat_runtime.rs:130`
...
- `agent_workspace/src/lib/store/actions/sendMessage.ts:50`
- `agent_workspace/src/lib/store/actions/sendMessage.ts:56`
- `agent_workspace/src/lib/store/actions/sendMessage.ts:69`
- `agent_workspace/src/lib/store/actions/sendMessage.ts:144`
- 文件内 `any` 使用较多
...
- `agent_orchestrator/src/api/chat_v2.rs`：`596` 行
- `agent_orchestrator/src/api/chat_v3.rs`：`818` 行
- 两者 diff 结果显示重叠度很高
- `agent_orchestrator/src/services/v2/ai_request_handler/parser.rs`：`439` 行
...
- `agent_workspace/src/components/SessionList.tsx:55`
- `agent_workspace/src/components/ProjectExplorer.tsx:41`
...
- `agent_workspace/src/lib/api/client.ts:95`
- `agent_workspace/src/lib/api/client.ts:167`
- 大量接口返回 `Promise<any>`
...
- 将前端 `SessionList.tsx` 从 `621` 行继续收缩到 `281` 行，新增 `agent_workspace/src/components/sessionList/useSessionListController.ts`，把 store 选择、刷新状态、远程连接表单、本地 picker、确认弹窗和各 section controller 编排集中外提
- 将前端 `ProjectExplorer.tsx` 从 `729` 行收缩到 `570` 行
- 将前端 `agent_workspace/src/components/projectExplorer/PreviewPane.tsx` 从 `525` 行收缩到 `278` 行，新增 `agent_workspace/src/components/projectExplorer/useProjectPreviewRunController.ts`，把运行命令状态、终端轮询、退出诊断与重启/停止编排从预览组件外提
...
- 将前端 `NotepadPanel.tsx` 从 `581` 行收缩到 `78` 行，新增 `agent_workspace/src/components/notepad/useNotepadPanelController.ts`，把记事本的数据加载、目录/笔记 CRUD、右键菜单、剪贴板与编辑器状态全部外提
- 将前端 `agent_workspace/src/components/notepad/useNotepadPanelController.ts` 从 `642` 行继续收缩到 `563` 行，新增 `agent_workspace/src/components/notepad/controllerHelpers.ts`、`useNotepadPanelEffects.ts`，把 clipboard / markdown 导出 / context menu style 和面板副作用从 controller 本体外提
...
- 新增 `agent_workspace/src/lib/store/actions/sendMessage/types.ts`，为附件、SSE 事件、工具调用、流式消息 metadata 建立局部契约层
...
- 新增 `agent_workspace/src/lib/store/actions/sendMessage/streamEventHandler.ts`，把 `chunk/content/thinking/tools/runtime_guidance/cancelled/complete` 等流式事件分发从主流程外提
- 新增 `agent_workspace/src/lib/store/actions/sendMessage/streamReader.ts`，把 reader/buffer/SSE 事件消费循环从主流程外提
- 新增 `agent_workspace/src/lib/store/actions/sendMessage/streamExecution.ts`，把流式执行、finalize 和失败回滚从主流程外提
...
- 当前 `agent_workspace/src/lib/store/actions/sendMessage.ts` 为 `264` 行，`agent_workspace/src/lib/store/actions/messages.ts` 为 `606` 行；`sendMessage/` 子目录总计 `3591` 行，主文件已进一步收缩为请求组装、草稿创建与顶层异常边界
- 当前 `agent_workspace/src/lib/store/actions/messages.ts`、`agent_workspace/src/lib/store/actions/sendMessage.ts` 以及 `agent_workspace/src/lib/store/actions/sendMessage/` 目录内显式 `any` 已清空
...
- 将 `agent_workspace/src/lib/store/actions/messages.ts` 从 `606` 行收缩到 `56` 行，新增 `messagesLoading.ts`、`messagesTurnProcess.ts`、`messagesState.ts`，把消息加载、流式 draft 合并、turn process 状态机拆开
- 为 `agent_workspace/src/lib/api/client/account.ts`、`tasks.ts`、`summary.ts` 以及 `agent_workspace/src/lib/api/client/types.ts` 补充认证、用户设置、TaskManager、UI Prompt、SessionSummary DTO，清理一批公开接口上的 `Promise<any>`
- 为 `agent_workspace/src/lib/api/client/workspace.ts`、`messages.ts`、`conversation.ts` 以及 `agent_workspace/src/lib/api/client/types.ts` 补充 `SessionResponse`、`ContactResponse`、`SessionMessageResponse`、conversation envelope 等 DTO，清理一批 session/contact/message 相关 `Promise<any>`
- 为 `agent_workspace/src/lib/api/client/workspace.ts` 与 `agent_workspace/src/lib/api/client/types.ts` 继续补充联系人项目记忆、联系人项目列表、agent recall、project run、terminal、remote、SFTP、FS 等 DTO，进一步收口 workspace 领域 API 返回值
- 为 `agent_workspace/src/lib/api/client/configs.ts`、`memory.ts`、`notepad.ts`、`stream.ts` 与 `agent_workspace/src/lib/api/client/types.ts` 补充 `ApplicationResponse`、`MemoryAgentResponse`、`MemoryAgentRuntimeContextResponse`、notepad 系列响应以及 `StopChatResponse`
- 当前 `agent_workspace/src/lib/api/client.ts`、`workspace.ts`、`configs.ts`、`memory.ts`、`notepad.ts`、`stream.ts` 中公开接口上的 `Promise<any>` / `Promise<any[]>` 已清空
- 将 `agent_workspace/src/lib/api/client.ts` 从 `952` 行收缩到 `154` 行，新增 `agent_workspace/src/lib/api/client/facades/workspaceFacade.ts`、`configFacade.ts`、`runtimeFacade.ts`，把主入口改为 request/context 壳层 + facade 注入，保留现有实例方法调用方式不变
- 继续收敛 `agent_workspace/src/lib/api/client/configs.ts`、`conversation.ts`、`runtimeFacade.ts` 与 `agent_workspace/src/lib/store/actions/mcp.ts` 的类型边界，补齐 `McpConfigUpdatePayload`、`ConversationDetailsResponse` fallback、`SessionMessageResponse` fallback 与 `StreamChatModelConfigPayload` 约束，并将 `mcp` store 从“强转后端返回”改为显式 normalize
- 继续收敛 `agent_workspace/src/lib/store/actions/aiModels.ts`，新增 `AiModelConfigResponse -> AiModelConfig` normalize helper，补齐 `AiModelConfigCreatePayload` / `ChatStoreDraft` 边界，并清空该文件内显式 `any`
- 将 `agent_workspace/src/components/McpManager.tsx` 从 `514` 行收缩到 `252` 行，新增 `agent_workspace/src/components/mcpManager/helpers.ts`、`types.ts`、`McpManagerForm.tsx`、`DynamicConfigFields.tsx`、`McpServerList.tsx`，把表单区、动态参数区、列表区和兼容 helper 从主组件外提
- 将 `agent_workspace/src/components/AiModelManager.tsx` 从 `522` 行收缩到 `168` 行，新增 `agent_workspace/src/components/aiModelManager/helpers.ts`、`types.ts`、`icons.tsx`、`AiModelManagerForm.tsx`、`AiModelList.tsx`，把 provider/thinking 规则、表单区和列表区从主组件外提，并去掉组件内随机 `Math.random()` ID 生成
- 将 `agent_workspace/src/components/ApplicationsPanel.tsx` 从 `482` 行收缩到 `231` 行，新增 `agent_workspace/src/components/applicationsPanel/helpers.ts`、`types.ts`、`icons.tsx`、`ApplicationsManageView.tsx`、`ApplicationsBrowseView.tsx`，把 modal/embedded 共享内容抽成管理视图和浏览视图，减少重复 JSX
- 继续收敛 `agent_workspace/src/lib/store/actions/applications.ts`，将 `client/set/get` 改为显式 `ApiClient` / `ChatStoreDraft` / DTO 边界，补齐 `ApplicationResponse.icon_url` / `iconUrl` 兼容，并将系统提示词应用关联持久化改为读取当前 `name/content` 后再提交，移除 `undefined as any`
- 将 `agent_workspace/src/components/TaskWorkbar.tsx` 从 `557` 行收缩到 `273` 行，新增 `agent_workspace/src/components/taskWorkbar/types.ts`、`helpers.ts`、`TaskCard.tsx`、`RuntimeGuidanceSection.tsx`、`TaskHistoryDrawer.tsx`，把 workbar 卡片、引导区、历史抽屉和显示文案 helper 从主组件外提
- 将 `agent_workspace/src/components/inputArea/PickerWidgets.tsx` 从 `587` 行收缩到 `5` 行，新增 `agent_workspace/src/components/inputArea/pickerWidgets/` 子目录，拆出 `InputAreaProjectFilePicker.tsx`、`InputAreaProjectSelector.tsx`、`InputAreaWorkspacePicker.tsx`、`InputAreaRemoteConnectionPicker.tsx`、`InputAreaMcpPicker.tsx`，让主文件只保留导出壳层
- 将 `agent_workspace/src/components/RemoteSftpPanel.tsx` 从 `543` 行收缩到 `334` 行，新增 `agent_workspace/src/components/remoteSftp/helpers.ts`、`useRemoteSftpBrowsers.ts`、`useRemoteSftpTransfer.ts`，把目录浏览状态、传输队列轮询、路径 helper 与响应 normalize 从主组件外提
- 将 `agent_workspace/src/components/projectExplorer/TeamMembersPane.tsx` 从 `585` 行收缩到 `536` 行，新增 `agent_workspace/src/components/projectExplorer/teamMembers/useTeamMemberRuntimeContext.ts`，把“联系人会话最近一轮 runtime 上下文”的加载、跨项目校验、抽屉状态和刷新逻辑从主组件外提
- 继续收敛 `agent_workspace/src/components/projectExplorer/TeamMembersPane.tsx`、`teamMembers/useTeamMemberConversation.ts`、`useProjectMembersManager.ts`、`TeamMemberWorkspace.tsx`、`TeamMemberSummaryView.tsx` 的类型边界，移除 `messages as any[]`，将会话工作区 / 成员管理 / 总结视图的 props 与 hook 参数改为显式 `Message`、`AiModelConfig`、`SendMessageRuntimeOptions`、`ProjectContactLinkResponse`、`ContactRecord`
...
- `agent_workspace` 执行 `npm run type-check` 通过
...
- `agent_orchestrator` 执行 `cargo test join_stream_text --package agent_orchestrator` 通过
- `agent_orchestrator` 执行 `cargo test ensure_complete_event_content --package agent_orchestrator` 已在上一版完整性合并策略下通过；当前实现已进一步简化为“不再合并 complete 内容”
- `agent_workspace` 再次执行 `npm run type-check` 通过，确认 API client DTO 收口、`ProjectExplorer` 终端日志类型对齐和 `NotepadPanel` 响应类型修正已稳定
- `agent_workspace` 再次执行 `npm run type-check` 通过，确认 `NotepadPanel` controller 拆分后前端类型与行为边界保持稳定
- `agent_workspace` 再次执行 `npm run type-check` 通过，确认 `PreviewPane` 运行状态机外提到 `useProjectPreviewRunController.ts` 后无类型回归
- `agent_workspace` 再次执行 `npm run type-check` 通过，确认 `SessionList` orchestration 外提到 `useSessionListController.ts` 后无类型回归
- `agent_workspace` 再次执行 `npm run type-check` 通过，确认 `ApiClient` facade 化拆分和 `useNotepadPanelController` 二次下沉后无类型回归
- `agent_workspace` 再次执行 `npm run type-check` 通过，确认 `conversation/config/runtimeFacade/mcp` 这一轮 API 类型收口和 `McpManager` 组件拆分后无类型回归
- `agent_workspace` 再次执行 `npm run type-check` 通过，确认 `aiModels.ts` 的 DTO normalize / store draft 类型收口后无回归
- `agent_workspace` 再次执行 `npm run type-check` 通过，确认 `AiModelManager` / `ApplicationsPanel` 拆分为 `aiModelManager/`、`applicationsPanel/` 子目录后无类型回归
- `agent_workspace` 再次执行 `npm run type-check` 通过，确认 `applications.ts` 的 store 类型收口和 `TaskWorkbar` 拆分为 `taskWorkbar/` 子目录后无类型回归
- `agent_workspace` 再次执行 `npm run type-check` 通过，确认 `PickerWidgets.tsx` 下沉为 `inputArea/pickerWidgets/` 子目录后无类型回归
- `agent_workspace` 再次执行 `npm run type-check` 通过，确认 `RemoteSftpPanel` 的浏览器/传输状态 hook 下沉和 `TeamMembersPane` 的 runtime context hook 外提后无类型回归
- `agent_workspace` 再次执行 `npm run type-check` 通过，确认 `TeamMembersPane` 团队成员会话工作区链路去掉 `messages as any[]` 并补齐会话 / 成员管理 / 总结视图 DTO 后无类型回归

### 下一批建议

- 考虑将 `sendMessage` 的流式状态收尾与 finalize/failure rollback 再下沉到 controller，进一步压缩主文件并补前端单测
- 继续处理 `agent_workspace/src/components/projectExplorer/TeamMembersPane.tsx` 周边的 `useSessionWorkbarPanels.ts` / `useWorkbarState.ts` / `useSessionSummaryPanel.ts`，把 `unknown[]`、`RuntimeGuidanceWorkbarItem[]` 强转和总结列表 normalize 进一步收敛到显式 DTO
