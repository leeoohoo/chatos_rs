...
use crate::repositories::{agents as agents_repo, ai_model_configs as ai_repo, system_contexts as ctx_repo};
...
pub fn router() -> Router {
    Router::new()
        .route("/api/agent_v3/agents/chat/stream", post(chat_stream))
}
...
async fn stream_agent_v3(sender: SseSender, req: AgentChatRequest) {
    ...
    let effective_reasoning = (supports_reasoning || model_cfg.thinking_level.is_some()) && reasoning_enabled;

    let mcp_ids: Vec<String> = agent.mcp_config_ids.iter().filter(|s| !s.is_empty()).cloned().collect();
    ...
    if use_tools && (!http_servers.is_empty() || !stdio_servers.is_empty() || !builtin_servers.is_empty()) {
        let _ = mcp_exec.init().await;
    }

    let mut ai_server = AiServer::new(
        model_cfg.api_key.clone().unwrap_or_else(|| cfg.openai_api_key.clone()),
        model_cfg.base_url.clone().unwrap_or_else(|| cfg.openai_base_url.clone()),
    ...
    let callbacks = AiClientCallbacks {
        ...
        on_tools_start: if use_tools { Some(std::sync::Arc::new(on_tools_start)) } else { None },
        on_tools_stream: if use_tools { Some(std::sync::Arc::new(on_tools_stream)) } else { None },
        on_tools_end: if use_tools { Some(std::sync::Arc::new(on_tools_end)) } else { None },
    };
