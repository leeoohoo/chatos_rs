...
| CP-20260922-001 | Agent 团队与需求调研 | Core、SQLite、Connector、Presentation、WinUI | 待实现 | 项目级统一渐进调研、附件文本读取、跨会话 Inbox、run-scoped opaque refs、文档发送 receipt、成员提案原子审批，以及 Todo 不可变执行合同/来源关系/builtin+Plugin 能力快照/原子串行调度/隔离 executor 上下文与工具权限已补；继续实现持久化资产快照 |
...
- 已确认剩余代码差距：Todo 的持久化资产快照与更严格 lane 状态边界，以及触发图片/PDF 多模态输入和项目级调研中心 UI。
