...
fn build_connect_options(
    ...
    // Explicitly set SSL mode to avoid accidental TLS negotiation against non-TLS MySQL servers.
    options = if let Some(tls) = &datasource.tls {
        if tls.enabled {
            let mut configured = options.ssl_mode(map_ssl_mode(tls.ssl_mode));
            if let Some(ca_cert) = tls.ca_cert.clone().filter(|value| !value.trim().is_empty()) {
                let ca_path = PathBuf::from(ca_cert);
                if ca_path.exists() {
                    configured = configured.ssl_ca(ca_path);
                }
            }
            configured
        } else {
...
fn map_ssl_mode(mode: Option<SslMode>) -> MySqlSslMode {
...
pub fn map_db_error(stage: &str, err: sqlx::Error) -> AppError {
    ...
    let code = if lower.contains("access denied") || lower.contains("authentication") {
        "CONN_AUTH_FAILED"
