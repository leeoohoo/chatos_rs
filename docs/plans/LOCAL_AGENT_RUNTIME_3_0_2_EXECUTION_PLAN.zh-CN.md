...
- 当前在制：继续阶段 5 的 macOS 全业务 Storage Provider 迁移，并完成 Main Chat 创建与 Task 创建的真实签名 App 冒烟。阶段 4 的 Windows Host 打包和原生 runner 验收保持未完成，不得标记完成。
- 下一切片：按阶段 7 的完成门槛重新审计并物理删除仓库中仍残留的 Cloud Agent、Task Runner Service、Agent RabbitMQ/Mongo 执行面；仅把 Memory Engine 仍需要的无工具模型请求、摘要和记忆提炼能力收归 `memory_engine` 自身，不迁移旧 Agent Loop、任务状态或队列兼容层。随后迁移 macOS Story Project Store 到共享 `StoryRepository`。
