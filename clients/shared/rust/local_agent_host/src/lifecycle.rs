...
use crate::{
    ...
    LocalProjectIpcExecutor, LocalStoryIpcExecutor, RegisteredLocalCapabilityRuntime,
...
pub fn build_local_agent_ipc_server(
    ...
    let project_executor: Arc<dyn LocalAgentIpcMutationExecutor> =
        Arc::new(LocalProjectIpcExecutor::new(
            storage.clone(),
            scope.clone(),
            device_id.clone(),
            storage_executor,
        ));
    let clipboard_executor: Arc<dyn LocalAgentIpcMutationExecutor> =
        Arc::new(LocalClipboardIpcExecutor::new(
            storage.clone(),
            scope.clone(),
            device_id.clone(),
            project_executor,
        ));
    let media_executor: Arc<dyn LocalAgentIpcMutationExecutor> =
        Arc::new(LocalMediaIpcExecutor::new(
            storage.clone(),
            scope.clone(),
            device_id.clone(),
            clipboard_executor,
        ));
    let story_executor: Arc<dyn LocalAgentIpcMutationExecutor> =
        Arc::new(LocalStoryIpcExecutor::new(
            storage.clone(),
            scope.clone(),
            device_id.clone(),
            media_executor,
