# 3.0.4 进度：拆分 Todo Source 读取 Repository

- 时间：2026-09-20 04:18:06（Asia/Shanghai）
- 本轮目标：保持 Todo 来源归属校验、排序和错误语义不变，将 `listAgentTodoSources(...)` 的纯读取与行映射迁入 `AgentTodoRepository`。
- 起始提交：`a7aa24d06`
- 代码提交：`926951450`

## 实际改动

- 为 `AgentTodoRepository` 增加按 owner 与 Todo 读取来源链接的入口。
- 将 Todo source relation 的枚举校验与行映射迁入 repository。
- facade 继续完成 owner、Agent、Todo ID 校验及 Todo 归属存在性校验，再委托 repository 返回来源列表。
- Todo 来源链接写入、去重与事务边界均未移动。
...
- SELECT 的五个列及列顺序保持不变。
- 排序仍为创建时间、message ID、relation 升序。
- 无效 relation 仍抛出 `invalid Agent Todo source relation` 存储错误。
- Todo 不存在或不属于指定 Agent 时仍在查询前抛出 not found。
- repository 查询仍在 prepare 前精确增加一次 DEBUG statement 计数。
- source 链接写入、幂等去重和调用该列表读取的现有事务边界均未改变。
...
- facade 中仍有 Todo dependency、progress 与调度相关读取；部分读取嵌在写事务中，需要逐项保留事务边界。
...
- 以单个有界工作单元拆分 Todo dependency 列表纯读取与行映射，保留 Todo 归属校验和写事务边界。
