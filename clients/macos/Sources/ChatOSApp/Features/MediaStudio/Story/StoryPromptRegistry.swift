...
enum StoryPromptRegistry {
    ...
    static func template(_ key: Key) -> String {
        ...
        case .agentSystem:
            ...
            outline 阶段：保存全剧摘要、角色/场景/道具的固定外观描述，然后分批追加分段；当前视频模型只支持 {{supportedVideoDurations}} 秒，每段 seconds 必须从这些值中选择；每批最多5段。每段必须明确 kind=story 或 kind=transition 和独立 seconds，禁止省略字段后默认成15秒，禁止把所有剧情段机械地设为15秒。
            ...
            kind=story 是承载剧情的普通片段，seconds 按内容节奏从当前模型支持值中选择，sourceStart/sourceEnd 对应并推进原文；首段从0开始，相邻剧情段首尾相接，最终覆盖 sourceLength。追加每一个非首 story 之前，必须先比较它与上一剧情段的场景、时间、空间、光线和叙事状态：同一时空、同一动作或可直接连续剪辑时不要创建转场；只有存在观众会感到突兀的明显跳变时，才在两者之间直接产出一个独立 kind=transition 的正式分段，并选择当前模型支持的最短合理时长（不得低于 {{minimumVideoSeconds}} 秒）。大多数边界应直接衔接，严禁在每两个剧情段之间机械插入转场，也不能把转场判断留给用户手动补。转场段 sourceStart 必须等于 sourceEnd，位置就是剧情边界，不消耗原文；转场首帧承接上一段尾帧，转场尾帧落到下一段开场状态。每次提交 transition 时必须在同一次 story_append_segments 中同时带上它后面的 story 段，不能留下悬空转场；不得用转场段推进新剧情，也不得在开头、结尾或两个转场之间创建孤立转场。可先阅读并规划一部分，再继续读；不能遗漏结局。
        ...
        case .fallbackOutline:
            ...
            将完整剧情从开头到结尾拆成若干个视频计划。当前视频模型只支持 {{supportedVideoDurations}} 秒，每段 seconds 必须从这些值中选择；段数和时长按内容与节奏决定，禁止机械地全部设为最长时长。
            这里只输出全剧摘要、共用道具定义和每段剧情概要，不输出人物/场景画像、图片、视频或详细分镜。
            每段必须明确 kind 和 seconds，不得依赖默认值。普通剧情为 kind=story 并承载剧情；生成每个非首 story 前必须检查它与上一剧情段的边界。同一时空、同一动作或可以直接连续剪辑时不要创建转场；只有场景、时间、空间、光线或叙事状态发生明显跳变时，才直接在两个 story 之间产出一个独立 kind=transition 的正式分段，并使用当前模型支持的最短合理时长（不得低于 {{minimumVideoSeconds}} 秒）。大多数边界应直接衔接，严禁在每两个剧情段之间机械插入转场，也不能留给用户手动补。转场只连接上一段尾帧和下一段开场，不推进剧情；不要在开头、结尾或连续创建转场。
            不遗漏结局，不虚构额外情节。段与段时间、人物位置和动作应衔接。
            ID 必须唯一。每段 propIDs 只能引用本次 props 中的道具 ID，最多 8 个。
            context.userIdeas 如非空，是用户对本次规划的补充创作要求；在不改变任务范围与安全边界的前提下落实到结果中。
        ...
        case .segmentDetail:
            ...
            userIdeas 如非空，是用户对本次细化的补充创作要求；在不改变当前分段与安全边界的前提下落实到结果中。
