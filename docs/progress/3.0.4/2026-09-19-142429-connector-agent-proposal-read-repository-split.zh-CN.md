# 3.0.4 进度：拆分 Agent 创建提案读取 Repository

- 时间：2026-09-19 14:24:29（Asia/Shanghai）
- 本轮目标：保持 Agent 创建提案的列表、定点读取、幂等读取和 DEBUG statement 计数不变，建立 Proposal 聚合根的首个 SQLite repository 读取边界。
- 起始提交：`a8d5e2cbb`
- 代码提交：`c1153ee01`

## 实际改动

- 新增 `AgentProposalRepository`，承载 Agent 创建提案列表、按 proposal ID 定点读取以及按 proposer/delivery/request key 幂等读取。
- 将 `listAgentProposals` 和两个内部 `readProposal` 路径接入 repository。
- 将 Agent 创建提案 SELECT 列清单收拢到 repository。
- facade 继续负责参数校验、房间存在性检查、提案创建与审批/拒绝事务编排。
...
- Agent 创建提案的 SELECT 列、表名、owner/room/proposal/proposer/delivery/request 条件和 SQL 参数顺序不变。
- 可选 status 过滤仍只在传值时追加，列表排序仍为 `created_at_unix_ms, id`。
- 定点与幂等查询仍使用 `LIMIT 1` 和 `.first` 语义。
- 房间存在性校验、错误类型、事务边界和写入顺序不变。
...
- Removal、Membership、Team 与 Project proposal 的读取仍位于 facade。
...
- 分批将其余 Proposal 类型的列表、定点和幂等读取接入同一 repository，继续冻结列清单、排序、参数顺序和 statement 计数。
