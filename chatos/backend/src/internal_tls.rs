...
#[derive(Debug, Clone)]
pub struct ChatosInternalTlsConfig {
...
impl ChatosInternalTlsConfig {
    pub fn from_config(host: IpAddr, config: &Config) -> Result<Self, String> {
        Ok(Self {
            bind_addr: SocketAddr::new(host, config.internal_mtls_port),
            server_cert_path: PathBuf::from(required_env("CHATOS_MTLS_SERVER_CERT_PATH")?),
            server_key_path: PathBuf::from(required_env("CHATOS_MTLS_SERVER_KEY_PATH")?),
            client_ca_cert_path: PathBuf::from(required_env("CHATOS_MTLS_CLIENT_CA_CERT_PATH")?),
        })
    }
}

pub fn load_internal_mtls_config(config: &ChatosInternalTlsConfig) -> Result<RustlsConfig, String> {
...
fn load_internal_mtls_config_from_paths(
    ...
    for certificate in read_certificates(client_ca_cert_path)? {
        client_roots
            .add(certificate)
            .map_err(|err| format!("invalid ChatOS mTLS client CA: {err}"))?;
    }
    if client_roots.is_empty() {
        return Err("ChatOS mTLS client CA contains no certificates".to_string());
    }
    let verifier = WebPkiClientVerifier::builder(Arc::new(client_roots))
        .build()
        .map_err(|err| format!("build ChatOS mTLS client verifier failed: {err}"))?;
    let server_config = ServerConfig::builder()
        .with_client_cert_verifier(verifier)
        .with_single_cert(server_certificates, server_key)
        .map_err(|err| format!("build ChatOS mTLS server config failed: {err}"))?;
...
mod tests {
    ...
    fn unique_test_dir(label: &str) -> PathBuf {
        ...
        std::env::temp_dir().join(format!(
            "chatos-internal-mtls-{label}-{}-{nonce}",
            std::process::id()
        ))
    }
    ...
    fn generate_material(output_dir: &PathBuf) {
        let script =
            PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../../scripts/generate-chatos-mtls.sh");
        let status = Command::new(script)
            .arg(output_dir)
            .status()
            .expect("run ChatOS mTLS generator");
