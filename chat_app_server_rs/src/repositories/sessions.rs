...
fn normalize_from_doc(doc: &Document) -> Option<Session> {
    ...
    Some(Session { id, title, description, metadata, user_id, project_id, created_at, updated_at })
}
...
pub async fn get_session_by_id(id: &str) -> Result<Option<Session>, String> {
    with_db(
        |db| {
            let id = id.to_string();
            Box::pin(async move {
                let doc = db.collection::<Document>("sessions").find_one(doc! { "id": id }, None).await.map_err(|e| e.to_string())?;
...
pub async fn get_all_sessions(limit: Option<i64>, offset: i64) -> Result<Vec<Session>, String> {
    with_db(
        |db| {
            Box::pin(async move {
                let mut cursor = db.collection::<Document>("sessions").find(doc! {}, None).await.map_err(|e| e.to_string())?;
                let mut docs = Vec::new();
                while let Some(doc) = cursor.try_next().await.map_err(|e| e.to_string())? {
                    docs.push(doc);
                }
                let mut sessions: Vec<Session> = docs.into_iter().filter_map(|d| normalize_from_doc(&d)).collect();
...
pub async fn get_sessions_by_user_project(user_id: Option<String>, project_id: Option<String>, limit: Option<i64>, offset: i64) -> Result<Vec<Session>, String> {
    with_db(
        |db| {
            ...
            Box::pin(async move {
                ...
                if let Some(pid) = project_id { filter.insert("project_id", pid); }
                let mut cursor = db.collection::<Document>("sessions").find(filter, None).await.map_err(|e| e.to_string())?;
                let mut docs = Vec::new();
                while let Some(doc) = cursor.try_next().await.map_err(|e| e.to_string())? {
                    docs.push(doc);
                }
                let mut sessions: Vec<Session> = docs.into_iter().filter_map(|d| normalize_from_doc(&d)).collect();
                sessions.sort_by(|a, b| b.created_at.cmp(&a.created_at));
                if offset > 0 { sessions = sessions.into_iter().skip(offset as usize).collect(); }
                if let Some(l) = limit { sessions = sessions.into_iter().take(l as usize).collect(); }
                Ok(sessions)
            })
        },
        |pool| {
            ...
            Box::pin(async move {
                ...
                if let Some(uid) = user_id { query.push_str(" AND user_id = ?"); binds.push(uid); }
                if let Some(pid) = project_id { query.push_str(" AND project_id = ?"); binds.push(pid); }
...
pub async fn delete_session(id: &str) -> Result<(), String> {
    with_db(
        |db| {
            let id = id.to_string();
            Box::pin(async move {
                db.collection::<Document>("sessions").delete_one(doc! { "id": &id }, None).await.map_err(|e| e.to_string())?;
                db.collection::<Document>("messages").delete_many(doc! { "session_id": &id }, None).await.map_err(|e| e.to_string())?;
                Ok(())
            })
        },
        |pool| {
...
pub async fn update_session(id: &str, title: Option<String>, description: Option<String>, metadata: Option<Value>) -> Result<(), String> {
    let now = chrono::Utc::now().to_rfc3339();
    let now_mongo = now.clone();
    let now_sqlite = now.clone();
    ...
    with_db(
        |db| {
            ...
            Box::pin(async move {
                ...
                if let Some(t) = title_mongo.clone() { set_doc.insert("title", t); }
                if let Some(d) = description_mongo.clone() { set_doc.insert("description", d); }
                if let Some(m) = metadata_mongo.clone() { set_doc.insert("metadata", m); }
                set_doc.insert("updated_at", now_mongo.clone());
                db.collection::<Document>("sessions").update_one(doc! { "id": &id }, doc! { "$set": set_doc }, None).await.map_err(|e| e.to_string())?;
                Ok(())
            })
        },
        |pool| {
            ...
            Box::pin(async move {
                ...
                if title_sqlite.is_some() { set_clause.push("title = ?"); }
                if description_sqlite.is_some() { set_clause.push("description = ?"); }
                if metadata_sqlite.is_some() { set_clause.push("metadata = ?"); }
                set_clause.push("updated_at = ?");
                let query = format!("UPDATE sessions SET {} WHERE id = ?", set_clause.join(", "));
                let mut q = sqlx::query(&query);
                if let Some(t) = title_sqlite { q = q.bind(t); }
                if let Some(d) = description_sqlite { q = q.bind(d); }
                if let Some(m) = metadata_sqlite { q = q.bind(m); }
                q = q.bind(&now_sqlite);
                q = q.bind(&id);
