...
当前实施进度：2.0.10 目标模式已经完成。Project、Session、Message、Task、Memory、Agent 生命周期均由云端持有；ChatOS、Task Runner 与 Project Environment Agent 的模型只连接 MCP Management 聚合 endpoint。Local Connector 不再启动本地 Task Worker，也不再挂载 Chat、Session、Memory、Project Management、Task Board、Task Run 和 Environment Agent 业务 API，只保留 Workspace 文件/Git/代码导航、命令、本地 Skill/Plugin/MCP、Sandbox 和设备控制状态。旧审批 Memory Engine proxy、Memory Policy cache 与云端读取本地模型凭据入口已经删除。Local Command Approval Agent 完整留在设备侧，只读取本地已安装 capability snapshot、只调用本地只读代码工具与 `approval_decision`，不创建云端 MCP Runtime Session、不访问云端 Memory Engine。Task Runner 已停止生成 Harness/Local Connector ephemeral MCP endpoint；Provider、设备、Workspace 与 Sandbox 路由由程序根据权威 Project Execution Context 决定。配置中心参数 `chatos.ui.local_project_creation_enabled` 已控制云端界面是否展示新建本地 Workspace Project 入口。全量 `make verify`、仓库 smoke 和真实 `make smoke-cloud-mcp` 已通过；真实 smoke 会使用权威 Project Execution Context 验证云端 CodeMaintainerRead 固定走 Harness、本地 CodeMaintainerRead 固定走对应 device/workspace 的 Local Connector；Project Environment Agent 的 Project Environment 与 Sandbox Images 能力由 Plugin Management 固定配置，但 Provider 由 MCP Management 按 Project Context 决定，云端项目已真实执行 `MCP Management -> Sandbox Manager get_image_catalog`，本地 Connector 离线时则明确 409 且不回退云端。Smoke 还会创建并关闭 Runtime Session，真实执行 `MCP Management -> Task Runner`、`MCP Management -> Project Management` 和 `MCP Management -> ChatOS Notepad` 三条工具链路，同时验证 owner/caller 隔离、本地审批 Agent 云端拒绝、Runtime Token 防篡改和 Session 关闭后不可复用。
