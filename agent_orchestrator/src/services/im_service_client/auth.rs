use super::dto::{ImAuthLoginResponse, ImAuthMeResponse};
use super::http::{build_url, client, send_json_without_service_token, timeout_duration};

pub async fn auth_login(username: &str, password: &str) -> Result<ImAuthLoginResponse, String> {
    let req = client()
...
pub async fn auth_me(access_token: &str) -> Result<ImAuthMeResponse, String> {
