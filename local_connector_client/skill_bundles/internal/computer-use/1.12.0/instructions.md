# ChatOS Computer Use — Approved Desktop Control with Dedicated High-Risk Confirmation
...
- UI Automation must explicitly confirm Edit control type, enabled state, keyboard focusability, current keyboard focus, visible non-empty bounds, non-password state, writable ValuePattern, and an unchanged focused identity immediately before `SendInput(KEYEVENTF_UNICODE)`.
...
8. Stop on denial, stale session, permission failure, display drift, process/focus identity drift, UI Automation provider failure, protected content, or integrity-level restrictions.
...
11. On macOS, if Accessibility or Screen Recording access is denied, stop and explain the missing permission. Never bypass TCC or move control into an unreviewed process.

This release adds a backend-enforced one-time typed challenge for text input, Enter, Backspace, and modified shortcuts, removes session-wide approval from all Computer Use actions, and preserves the `1.11.0` post-action observation and release-recovery contract. A separate signed helper, non-Edit contenteditable text entry, and application-state rollback remain unavailable.
