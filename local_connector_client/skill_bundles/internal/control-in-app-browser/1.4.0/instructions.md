...
- The ChatOS managed-session panel requests a bounded visual preview through the bearer-authenticated Local Connector API. The renderer never receives a CDP URL, stream port, or direct browser WebSocket capability.
...
- The ChatOS managed-session panel exposes the same network, HAR, file-transfer, and preview contracts; it does not grant arbitrary host filesystem access or expose raw browser credentials.
