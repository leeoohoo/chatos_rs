...
尚未具备云端等价安全设施的配置继续 fail closed：OAuth 浏览器授权、refresh token 自动续期、包内相对 command/cwd、ConfigFile transport 均不会退化为明文、本地 OAuth Broker 或 MCP Management 本地执行。当前 Cloud OAuth 只接受仍在有效期内的受保护 Bearer access token；过期后必须重新授权/更新。后续仍需补由 Sandbox Manager 管理的 immutable artifact mount；在此之前 `PluginCloud` 同样声明 `cancel_supported=false`。
