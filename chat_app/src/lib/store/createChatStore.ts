    ...
    debugLog("chatService:", chatService)
                ...
                (subscribeWithSelector(
                    ...
                    abortCurrentConversation: () => {
                        debugLog('abortCurrentConversation 被调用');
                        try {
                            // 调用ChatService的停止方法
                            chatService.abortCurrentConversation();
                            debugLog('ChatService.abortCurrentConversation 调用成功');
                        } catch (error) {
                        ...
                        debugLog('状态已更新: isStreaming=false, isLoading=false');
                    },
                    ...
                    updateMcpConfig: async (config: McpConfig) => {
                        try {
                            ...
                            if (config.id) {
                                ...
                                debugLog('🔍 updateMcpConfig 创建数据:', createData);
                    ...
                    loadSystemContexts: async () => {
                        try {
                            ...
                            debugLog('🔍 [后端返回] loadSystemContexts 返回的数据:', contexts);
                            debugLog('🔍 [后端返回] 第一个上下文的 app_ids:', contexts[0]?.app_ids);
