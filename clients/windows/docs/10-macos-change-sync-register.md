...
| CP-20260922-001 | Agent 团队与需求调研 | Core、SQLite、Connector、Presentation、WinUI | 待实现 | 项目级渐进调研、附件文本读取和 lane 工具裁剪已补；继续实现 Inbox/opaque refs、成员提案和 Todo 执行合同 |
...
- Windows 代码修改：新增账号隔离 SQLite schema、Agent/Room/Message/Todo/Asset/Survey/Delivery/Run 模型与 Store；Responses API 工具循环；默认插件与成员 allowlist 交集驱动的 MCP 真执行，并复用权限/逐次审批、OAuth/Secret 和 Artifact 管线且按 run 清理；项目文件与审批终端；后台心跳/恢复；Presentation 状态机；项目工作区 WinUI；附件元数据与 payload 分表、UTF-8 正文按需读取；Agent 间私聊；共享资产 create/update 分离、分类对齐和项目经理自动维护唤醒；需求调研改为项目归属并加入渐进场景 Skill、跨团队任务核对和 v14 迁移。
...
- 已确认剩余代码差距：账号级全会话 Inbox 与已读游标、run-scoped opaque reference/文档草稿/发送 receipt、Agent 成员变更提案及 Human 审批、Todo 不可变执行合同/来源关系/任务级 builtin 与 Plugin 能力快照、完整 schedule state/start-next、独立 executor lane，以及触发图片/PDF 多模态输入和项目级调研中心 UI。
- Windows 真机要求：代码差距关闭后，验证 Agent/团队编辑与提案对话框、团队切换、附件/多模态、项目调研中心、模型工具、真实插件进程、Artifact、命令审批、崩溃恢复和长对话内存占用。
- 当前状态：`待实现`；当前已完成部分通过 Windows solution 464 项测试，生产源码大小门禁待本轮最终复核。
