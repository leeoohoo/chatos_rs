...
pub(super) fn task_project_from_row(row: &SqliteRow) -> Result<TaskProjectRecord, String> {
    Ok(TaskProjectRecord {
        ...
        cloud_import_source: row.get("cloud_import_source"),
        import_status: row.get("import_status"),
        source_git_url: row.get("source_git_url"),
        harness_space_identifier: row.get("harness_space_identifier"),
        harness_repo_identifier: row.get("harness_repo_identifier"),
        harness_repo_path: row.get("harness_repo_path"),
        harness_git_url: row.get("harness_git_url"),
        harness_git_ssh_url: row.get("harness_git_ssh_url"),
        harness_default_branch: row.get("harness_default_branch"),
        harness_provision_status: row.get("harness_provision_status"),
        harness_provision_error: row.get("harness_provision_error"),
        harness_provisioned_at: row.get("harness_provisioned_at"),
        description: row.get("description"),
...
pub(super) fn runtime_settings_from_row(row: &SqliteRow) -> Result<RuntimeSettingsRecord, String> {
    Ok(RuntimeSettingsRecord {
        ...
        sandbox_enabled: int_to_bool(row.get::<i64, _>("sandbox_enabled")),
