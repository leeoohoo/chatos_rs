use axum::{Router, Json, routing::{get, post}, extract::{Path, Query}};
...
pub fn router() -> Router {
    Router::new()
        ...
        .route("/api/agent_v2/chat/stream/simple", post(agent_chat_stream_simple))
        .route("/api/agent_v2/tools", get(agent_tools))
        .route("/api/agent_v2/status", get(agent_status))
        .route("/api/agent_v2/session/:session_id/reset", post(reset_session))
        .route("/api/agent_v2/session/:session_id/config", get(get_session_config).post(update_session_config))
        .route("/api/chat/stop", post(stop_chat))
}

async fn agent_chat_stream(Json(req): Json<ChatRequest>) -> Result<axum::response::Sse<impl futures::Stream<Item = Result<axum::response::sse::Event, std::convert::Infallible>>>, (StatusCode, Json<Value>)> {
...
async fn agent_chat_stream_simple(Json(req): Json<ChatRequest>) -> Result<axum::response::Sse<impl futures::Stream<Item = Result<axum::response::sse::Event, std::convert::Infallible>>>, (StatusCode, Json<Value>)> {
    let session_id = req.session_id.clone().unwrap_or_default();
    let content = req.content.clone().unwrap_or_default();
    if session_id.is_empty() || content.is_empty() {
        return Err((StatusCode::BAD_REQUEST, Json(json!({"error": "session_id 和 content 不能为空"}))));
...
async fn agent_tools(Query(query): Query<UserQuery>) -> (StatusCode, Json<Value>) {
    let (http_servers, stdio_servers, builtin_servers) = load_mcp_configs_for_user(query.user_id, None, None)
        .await
        .unwrap_or((Vec::new(), Vec::new(), Vec::new()));
    let mut exec = McpToolExecute::new(http_servers.clone(), stdio_servers.clone(), builtin_servers.clone());
...
async fn get_session_config(Path(session_id): Path<String>) -> Json<Value> {
    Json(json!({"success": true, "config": { "model": "gpt-4", "temperature": 0.7, "session_id": session_id }}))
}

async fn update_session_config(Path(_session_id): Path<String>, Json(_req): Json<Value>) -> Json<Value> {
...
async fn stop_chat(Json(req): Json<Value>) -> (StatusCode, Json<Value>) {
    ...
    if session_id.is_empty() {
        return (StatusCode::BAD_REQUEST, Json(json!({"success": false, "message": "缺少 session_id"})));
    }
    let ok = abort_registry::abort(session_id);
    if ok {
        return (StatusCode::OK, Json(json!({"success": true, "message": "停止中"})));
    }
    (StatusCode::OK, Json(json!({"success": false, "message": "未找到可停止的会话或已停止"})))
}

async fn stream_chat_v2(sender: SseSender, req: ChatRequest, always_send_done: bool, rename_session: bool, respect_model_flags: bool) {
    let session_id = req.session_id.clone().unwrap_or_default();
    let content = req.content.clone().unwrap_or_default();
    let cfg = Config::get();
    ...
    let allow_active_ctx = if respect_model_flags {
        model_cfg.get("use_active_system_context").and_then(|v| v.as_bool()).unwrap_or(true)
    } else {
        true
    };
    ...
    log_chat_begin(&session_id, &model, &base_url, use_tools, http_servers.len(), stdio_servers.len() + builtin_servers.len(), !api_key.is_empty());
...
async fn stream_chat_v2_agent(sender: SseSender, req: ChatRequest, rename_session: bool) {
    ...
    let model_config = match crate::services::v2::agent::load_model_config_for_agent(&agent_id).await {
        Ok(cfg) => cfg,
        Err(err) => {
            sender.send_json(&json!({ "type": Events::ERROR, "timestamp": chrono::Utc::now().to_rfc3339(), "data": { "error": err } }));
            sender.send_done();
