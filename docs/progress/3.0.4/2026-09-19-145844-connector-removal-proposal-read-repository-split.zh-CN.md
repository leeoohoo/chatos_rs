# 3.0.4 进度：拆分成员移除提案读取 Repository

- 时间：2026-09-19 14:58:44（Asia/Shanghai）
- 本轮目标：保持成员移除提案的列表、定点读取、幂等读取和 DEBUG statement 计数不变，继续收拢 Proposal 聚合根的 SQLite 读取边界。
- 起始提交：`34de7b917`
- 代码提交：`34d53792e`

## 实际改动

- 扩展 `AgentProposalRepository`，承载成员移除提案列表、按 proposal ID 定点读取以及按 proposer/delivery/request key 幂等读取。
- 将 `listAgentRemovalProposals` 和两个内部 `readRemovalProposal` 路径接入 repository。
- 将成员移除提案 SELECT 列清单收拢到 repository。
- facade 继续负责参数校验、房间存在性检查、权限校验以及创建、审批和拒绝事务编排。
...
- 成员移除提案的 SELECT 列、表名、owner/room/proposal/proposer/delivery/request 条件和 SQL 参数顺序不变。
- 可选 status 过滤仍只在传值时追加，列表排序仍为 `created_at_unix_ms, id`。
- 定点与幂等查询仍使用 `LIMIT 1` 和 `.first` 语义。
- 房间存在性、成员状态、项目经理保护、错误类型、事务边界和写入顺序不变。
...
- Membership、Team 与 Project proposal 的读取仍位于 facade。
...
- 分批将 Membership、Team 与 Project proposal 的列表、定点和幂等读取接入同一 repository，继续冻结列清单、排序、参数顺序和 statement 计数。
