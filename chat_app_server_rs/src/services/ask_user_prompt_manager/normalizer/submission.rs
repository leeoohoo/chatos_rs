...
pub fn parse_response_submission(
...
) -> Result<AskUserPromptResponseSubmission, String> {
    let obj = raw
        .as_object()
    ...
    if status != "ok" {
        ...
        return Ok(AskUserPromptResponseSubmission {
            status,
            values: None,
            selection: None,
            reason: obj
    ...
    Ok(AskUserPromptResponseSubmission {
        status,
        values,
        selection,
        reason: obj
...
fn extract_fields(prompt: &AskUserPromptPayload) -> Result<Vec<KvField>, String> {
...
fn extract_choice(prompt: &AskUserPromptPayload) -> Result<Option<ChoiceConfig>, String> {
