...
- 当前在制：阶段 4，依次完成 Windows Ask User、工具授权、人工复核和 Pet 全本地投影。
- 下一切片：把 Windows Ask User 的读取、提交、取消与附件授权改为消费同一 Local Agent Host 权威 Interaction 投影，并删除对应远程 API/Pet 数据源，不保留双轨。
