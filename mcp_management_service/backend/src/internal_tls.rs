...
fn load_internal_mtls_config_from_paths(
    ...
    for certificate in client_ca_certificates {
        client_roots
            .add(certificate)
            .map_err(|err| format!("invalid MCP Management mTLS client CA certificate: {err}"))?;
    }
    if client_roots.is_empty() {
        return Err("MCP Management mTLS client CA contains no certificates".to_string());
    }
    let client_verifier = WebPkiClientVerifier::builder(Arc::new(client_roots))
        .build()
        .map_err(|err| format!("build MCP Management mTLS client verifier failed: {err}"))?;
    let server_config = ServerConfig::builder()
        .with_client_cert_verifier(client_verifier)
        .with_single_cert(server_certificates, server_key)
        .map_err(|err| format!("build MCP Management mTLS server config failed: {err}"))?;
...
mod tests {
    ...
    fn unique_test_dir(label: &str) -> PathBuf {
        ...
        std::env::temp_dir().join(format!(
            "chatos-mcp-management-mtls-{label}-{}-{nonce}",
            std::process::id()
        ))
    }
    ...
    fn generate_material(output_dir: &PathBuf) {
        ...
        let status = Command::new(script)
            .arg(output_dir)
            .status()
            .expect("run MCP Management mTLS generator");
    ...
    async fn internal_listener_requires_a_trusted_client_certificate_and_tls() {
        ...
        let wrong_material_dir = unique_test_dir("wrong-ca");
        generate_material(&material_dir);
        generate_material(&wrong_material_dir);
        ...
        let wrong_ca_client = reqwest::Client::builder()
            ...
            .add_root_certificate(
                reqwest::Certificate::from_pem(&wrong_ca_pem).expect("parse wrong CA"),
            )
            .identity(
                reqwest::Identity::from_pem(&wrong_identity_pem).expect("parse wrong identity"),
            )
            .build()
            .expect("build wrong CA client");
        assert!(wrong_ca_client.get(url.as_str()).send().await.is_err());
        ...
        let _ = std::fs::remove_dir_all(wrong_material_dir);
    }
