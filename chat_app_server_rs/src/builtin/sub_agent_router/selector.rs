use crate::builtin::sub_agent_router::types::{AgentSpec, CommandSpec};
use crate::builtin::sub_agent_router::utils::tokenize;
...
pub fn pick_agent(agents: &[AgentSpec], options: PickOptions) -> Option<PickResult> {
    ...
    let desired_skills: Vec<String> = options
        ...
        .unwrap_or_default()
        .iter()
        .map(|s| normalize_token(Some(s.as_str())))
        .filter(|s| !s.is_empty())
        .collect();
    let query_tokens = tokenize(options.query.as_deref());
    let task_tokens = tokenize(Some(options.task.as_str()));
    ...
    for agent in agents {
        ...
        if !desired_category.is_empty()
            && !agent_category.is_empty()
            && desired_category != agent_category
        {
            continue;
        }

        let commands = agent.commands.clone().unwrap_or_default();
        let matched_command = resolve_command(&commands, command_id.as_str());
        ...
        for token in tokenize(Some(agent.name.as_str())) {
...
fn resolve_command(commands: &[CommandSpec], command_id: &str) -> Option<CommandSpec> {
    ...
    commands
        .iter()
        .find(|cmd| normalize_token(Some(cmd.id.as_str())) == target)
        .cloned()
        .or_else(|| {
            commands.iter().find_map(|cmd| {
                cmd.name.as_ref().and_then(|name| {
                    if normalize_token(Some(name.as_str())) == target {
                        Some(cmd.clone())
                    } else {
                        None
                    }
...
fn flatten_command_tokens(commands: &[CommandSpec]) -> Vec<String> {
    ...
    for cmd in commands {
        tokens.extend(tokenize(Some(cmd.id.as_str())));
