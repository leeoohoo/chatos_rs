...
pub(in crate::builtin::terminal_controller) async fn read_process_log_with_context(
    ctx: BoundContext,
    terminal_id: &str,
    offset: Option<i64>,
    limit: i64,
    ...
    let selected: Vec<String> = if offset.is_some() {
        line_buffer
            ...
            .cloned()
            .collect()
    } else if total_lines > requested_limit {
        line_buffer
            .into_iter()
...
pub(in crate::builtin::terminal_controller) async fn wait_process_with_context(
    ctx: BoundContext,
    terminal_id: &str,
    timeout_ms: u64,
) -> Result<Value, String> {
...
pub(in crate::builtin::terminal_controller) async fn write_process_with_context(
    ctx: BoundContext,
    terminal_id: &str,
    data: &str,
    submit: bool,
...
pub(in crate::builtin::terminal_controller) async fn kill_process_with_context(
    ctx: BoundContext,
    terminal_id: &str,
) -> Result<Value, String> {
    let terminal = get_terminal_for_context(&ctx, terminal_id).await?;
