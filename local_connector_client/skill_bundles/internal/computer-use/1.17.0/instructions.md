# ChatOS Computer Use — Safe Native Control, Window Geometry, and Native Window State
...
- Both platforms read back the same native window identity and requested geometry/state. A platform clamp, provider refusal, focus change, identity drift, or readback mismatch never claims success.
...
- After a control action runs, the Local Connector waits for a short bounded settle interval and attempts one screenshot. Click and drag recapture only the approved display identity; key, text, scroll, activation, and window geometry/state actions recapture the current main display.
...
- If the display changes, capture permission disappears, capture times out, the session is cancelled after input completed, or another observation error occurs, the result records `action_already_executed=true`, `automatic_replay_safe=false`, and a bounded failure reason. Ordinary completed input actions remain `success=true`; a window geometry/state action whose target state was not retained remains `success=false` together with its restoration result.
...
This release adds identity-bound frontmost-window move/resize on macOS and Windows, native writable `AXFullScreen` transitions on macOS, and explicitly non-fullscreen standard maximize/restore on Windows. It adds exact pre-action snapshot rebinding, fresh display-layout validation, post-action native readback, and bounded identity-guarded restoration after partial failure or in-flight cancellation. It preserves the signed one-shot helper, frontmost-window capture, fail-closed native/contenteditable text entry, high-risk challenge, per-action approval, privacy audit, transient post-action observation, activation recovery, input-release, and no-automatic-replay contracts from `1.16.0`.
