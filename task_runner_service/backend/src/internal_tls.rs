...
#[derive(Debug, Clone)]
pub struct TaskRunnerInternalTlsConfig {
...
impl TaskRunnerInternalTlsConfig {
    pub fn from_env(host: IpAddr, public_port: u16) -> Result<Self, String> {
        let port = required_env("TASK_RUNNER_INTERNAL_MTLS_PORT")?
            .parse::<u16>()
            .map_err(|err| format!("TASK_RUNNER_INTERNAL_MTLS_PORT must be a valid port: {err}"))?;
        if port == public_port {
            return Err(
                "TASK_RUNNER_INTERNAL_MTLS_PORT must differ from TASK_RUNNER_PORT".to_string(),
            );
        }
        Ok(Self {
            bind_addr: SocketAddr::new(host, port),
            server_cert_path: PathBuf::from(required_env("TASK_RUNNER_MTLS_SERVER_CERT_PATH")?),
            server_key_path: PathBuf::from(required_env("TASK_RUNNER_MTLS_SERVER_KEY_PATH")?),
            client_ca_cert_path: PathBuf::from(required_env(
                "TASK_RUNNER_MTLS_CLIENT_CA_CERT_PATH",
            )?),
        })
    }
}
...
pub fn load_internal_mtls_config(
    config: &TaskRunnerInternalTlsConfig,
...
fn load_internal_mtls_config_from_paths(
    ...
    for certificate in client_ca_certificates {
        client_roots
            .add(certificate)
            .map_err(|err| format!("invalid Task Runner mTLS client CA certificate: {err}"))?;
    }
    if client_roots.is_empty() {
        return Err("Task Runner mTLS client CA contains no certificates".to_string());
    }
    let client_verifier = WebPkiClientVerifier::builder(Arc::new(client_roots))
        .build()
        .map_err(|err| format!("build Task Runner mTLS client verifier failed: {err}"))?;
    let server_config = ServerConfig::builder()
        .with_client_cert_verifier(client_verifier)
        .with_single_cert(server_certificates, server_key)
        .map_err(|err| format!("build Task Runner mTLS server config failed: {err}"))?;
...
mod tests {
    ...
    fn unique_test_dir(label: &str) -> PathBuf {
        ...
        std::env::temp_dir().join(format!(
            "chatos-task-runner-mtls-{label}-{}-{nonce}",
            std::process::id()
        ))
    }
    ...
    fn generate_material(output_dir: &PathBuf) {
        let script = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
            .join("../../scripts/generate-task-runner-mtls.sh");
        let status = Command::new(script)
            .arg(output_dir)
            .status()
            .expect("run Task Runner mTLS generator");
