...
use self::payload::{
    ...
    parse_choice_block, parse_i64, parse_mixed_fields,
};
pub use self::payload::{normalize_kv_fields, ChoiceLimits, ChoiceOption, KvField};
use self::schema::{choice_schema, kv_schema, mixed_schema};

pub const ASK_USER_PROMPT_TIMEOUT_MS_DEFAULT: u64 = 86_400_000;

pub type AskUserStreamChunkCallback = Arc<dyn Fn(String) + Send + Sync>;
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct AskUserResponseSubmission {
    pub status: String,
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct AskUserDecision {
    pub status: String,
    pub response: AskUserResponseSubmission,
}

#[async_trait]
pub trait AskUserStore: Send + Sync {
...
#[derive(Clone)]
pub struct AskUserStoreRef(Arc<dyn AskUserStore>);

impl AskUserStoreRef {
    pub fn new(store: Arc<dyn AskUserStore>) -> Self {
        Self(store)
    }

    fn inner(&self) -> Arc<dyn AskUserStore> {
        self.0.clone()
    }
}

impl std::fmt::Debug for AskUserStoreRef {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("AskUserStoreRef").finish_non_exhaustive()
    }
}

#[derive(Debug, Clone)]
pub struct AskUserOptions {
    pub server_name: String,
    pub prompt_timeout_ms: u64,
    pub store: AskUserStoreRef,
}

#[derive(Clone)]
pub struct AskUserService {
...
impl AskUserService {
    pub fn new(opts: AskUserOptions) -> Result<Self, String> {
        ...
        let timeout_ms = opts
            .prompt_timeout_ms
            .clamp(10_000, ASK_USER_PROMPT_TIMEOUT_MS_DEFAULT);
        let server_name = opts.server_name;

        service.register_tool(
            "prompt_key_values",
            &format!(
                "Prompt user for structured key/value input fields and wait for submission (server: {server_name}). Use this for missing connection details, credentials, config values, paths, parameters, or other required facts. Mark password/token/key/passphrase fields with secret=true and never echo submitted secrets."
            ),
            kv_schema(),
        ...
        service.register_tool(
            "prompt_choices",
            "Prompt user for a single or multi-choice selection and wait for submission. Use this before major choices, risky actions, unclear goal/scope/success criteria, target environment selection, or when choosing among available connections/accounts/options.",
            choice_schema(),
            Arc::new(move |args, ctx| handle_prompt_choices(args, ctx, timeout_ms)),
        );
        service.register_tool(
            "prompt_mixed_form",
            "Prompt user with mixed form fields and optional choice selection in one interaction. Use this when a decision also needs structured fields, such as selecting a remote connection while collecting a secret password/token/passphrase field with secret=true.",
            mixed_schema(),
