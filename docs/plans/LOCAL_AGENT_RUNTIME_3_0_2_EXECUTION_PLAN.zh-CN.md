...
- 当前在制：完成 Main Chat 创建与 Task 创建的真实签名 App 冒烟；随后将审批与剧情 Profile 接入公共 Rust Runtime，物理删除 Swift `ChatOSAgentRuntime`。阶段 4 的 Windows Host 打包和原生 runner 验收保持未完成，不得标记完成。
