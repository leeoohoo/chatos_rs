...
- 当前在制：阶段 4，继续完成 Windows Pet 全本地投影并删除其远程 Activity Inbox、远程会话取消和 Task 独立控制残余。
- 下一切片：由账户级 Local Agent Projection 直接生成 Pet 的 Main Chat、Task、Ask User、Tool Approval 与 Needs Review 活动，删除 `IPetConversationControl`、`IPetActivityInboxService` 和 Pet 对远程 Realtime/Disposition 的依赖，不保留兼容。
