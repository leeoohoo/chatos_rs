...
| PDF | 部分可用，本机生成与结构编辑 | `1.8.0` 已覆盖 searchable ASCII 文本生成、A4/Letter 自动换行分页、Unicode title/author/subject/keywords Document Info 检查与更新、页码、文本提取、合并、按页提取、精确页序重排与删除、按页旋转、基于真实物理页位置和可选起始编号的动态页码、Unicode 标准 Text 便签批注及有界批注摘要，以及全页/选择页透明 ASCII 文本和 PNG/JPEG 图片盖章（含透明 PNG soft mask）；缺经过许可校验的 Unicode 嵌入字体、富排版/表格、OCR、markup/reply/附件等通用注释、表单/密码工作流、密码学签名、页面渲染和视觉验收 |
| Spreadsheets | 部分可用，本机多工作表创建、保守范围编辑、页面渲染与 Excel Live 有界读写 | 文件型 `1.2.0` 已覆盖 CSV、1–64 Sheet XLSX、typed cells、安全公式 allowlist、基础数字格式、列宽、冻结行、结构检查、distinct-output 范围更新、签名 LibreOffice/Poppler PDF 转换、连续页面瞬时 PNG 和逐页视觉 QA；Excel Live Control `1.3.0` 已能在不启动 Excel 的前提下检查运行状态、列出最多 32 个打开工作簿，以绑定当前 Excel 进程和私有 full-name identity 的不透明 workbook/worksheet ID 检查最多 64 个工作表，读取一个最多 256 cells 的规范 A1 范围，并在逐次人工审批、exact `range_snapshot_id`、写前逐格复验和写后双重读回下替换 blank/scalar/受限本地公式；部分失败会尝试逐格恢复并复验，但不声称完整 workbook transaction。仍缺 XLS/TSV、富样式、图表/透视表、宏、ISO 日期转换、Google Sheets handoff，以及 live 格式/对象、保存导出、完整事务回滚、视觉验证和 macOS/Windows 真实写入验收 |
| Presentations | 部分可用，本机多布局富媒体与简单矩形表格创建、保守编辑及页面渲染 | `1.23.0` 已覆盖六种原有 16:9 布局与正式 `table` layout、严格矩形 DrawingML 表格创建/追加及单元格/行列/格式保守编辑、editable text/bullets、PNG/JPEG、alt text、标准 speaker notes、真实 presentation-order 检查、distinct-output 追加、可见 slide/notes 的单 run 与同格式相邻 runs 精确替换、完整排列重排、可见位置删除，以及签名 LibreOffice/Poppler 连续 1–8 页 PNG、可选 PDF 和逐页视觉 QA 合同；标准 DrawingML chart 支持真实可见顺序只读检查，并以 literal caches 和内部唯一 chart part 创建/追加 2D clustered column、line、pie、area、doughnut，支持右/左/上/下图例、none/value/percentage 数据标签、category/value 轴标题，以及 column/line/area series 的 primary/secondary value-axis 分配、独立 secondary value-axis title、主/次值轴不裁剪数据的显式 minimum/maximum、`2–1000` 对数刻度、正数 major/minor unit 和九种受限 canonical number format。双轴 canonical 图表使用 primary bottom/left 与 hidden top/visible right 两组同类型 chart/axis 拓扑。只有无 chart relationships、公式、嵌入工作簿或外部数据，且由包含全部格式、轴归属、轴范围、对数刻度、轴单位和轴数字格式字段的完整检查快照重建后与原 XML 字节完全一致的 ChatOS canonical chart 才可绑定 snapshot 与 SHA-256 安全替换类型、标题、categories、series、values、legend、data labels 和 axis titles。仍缺 merged/nested/复杂单元格、跨表复制与任意区域/边框级表格格式编辑、跨 shape/paragraph/格式边界富文本与跨 run notes 编辑、任意 Office/第三方或工作簿关联图表编辑、scatter/bubble/radar/stock/surface/3D、tertiary axis、任意 tick marks/display units/crossing behavior、任意 custom number format、任意 chart styling/custom labels、chartEx/SmartArt、任意主题/母版导入和动画/转场 |
...
| Computer Use | 主要常用能力可用，macOS 签名隔离、macOS/Windows 观察、逐动作审批、高风险专用确认、结构化审计与动作后恢复 | `1.15.0` 已覆盖 macOS 窗口、Accessibility tree、多显示器截图、显示器身份绑定左/右单击与左键双击、安全可中断拖拽、导航键、有界滚动、应用激活与隐私保护 Unicode 文本输入，并将 macOS TCC 探测、观察、截图与输入全部迁入一次性独立 helper：Core/helper 双向严格 codesign 与相同 TeamIdentifier 校验、helper 直接父进程身份验证、256 KiB 请求/4 MiB 响应的版本化长度前缀 stdio、无网络 listener，以及私有 cancel marker、先释放输入后终止的 2 秒宽限。macOS 文本目标由原生 Accessibility API 严格检查前台 PID、enabled/focused/bounds、secure/protected 状态、可写 `AXValue`/`AXSelectedTextRange`、受限富文本角色的 `AXIsEditable=true` 与 editable ancestor，执行前对 application/focused/target 再次全量校验并使用 `CFEqual` 比较身份。Windows 覆盖当前用户窗口/显示器发现、瞬时 PNG、同等显示器绑定鼠标动作、导航键、滚动、PID 身份保护激活、有界 UI Automation control tree 与 secure-field-aware Unicode 文本输入；文本目标要么是可写 `Edit + ValuePattern`，要么是受限 `Document`/`Pane`/`Custom + TextEditPattern`，普通 `TextPattern` 不足以放行，执行前通过 `CompareElements` 严格复核身份。Accessibility tree 优先对可编辑目标脱敏；待审批与持久化历史不保留文本原文，结果只增加 `native_text_control`/`native_edit`/`contenteditable` 稳定目标分类。成功动作附加不可持久化瞬时后观察，观察失败保持动作已执行并禁止自动重放，鼠标/按键使用成对 release recovery；文本输入、Enter、Backspace 与带修饰键快捷键继续要求后端验证的一次性随机口令，且所有 Computer Use 动作均不支持会话级放行；`1.15.0` 已新增执行窗口内取消时的前台应用恢复，只有目标仍为前台且前后身份不变时才尝试，Windows 还会恢复本次激活前的目标最小化状态，任何用户/系统前台漂移都停止回滚。仍缺通用应用内容、导航副作用和任意窗口布局回滚 |
...
当前已发布 Computer Use `1.15.0` macOS 签名隔离、macOS/Windows 观察、逐动作审批、高风险专用确认、结构化审计、动作后恢复、安全 contenteditable 文本输入与有界应用激活回滚 Release。macOS 覆盖窗口清单、前台 Accessibility tree、多显示器枚举与瞬时 JPEG、显示器身份绑定左/右单击与左键双击、安全可中断拖拽、受限导航键、最多 256 个可见 Unicode 字符的隐私保护文本输入、有界横/纵向 scroll 和已运行应用激活；这些 TCC 探测、观察、截图与输入动作全部执行在一次性 `chatos_computer_use_helper` 内，Core 在启动前验证 helper 为 executable regular non-symlink file、严格 codesign 且 TeamIdentifier 相同，helper 再独立解析直接父进程并验证 exact Core 文件名、严格签名和相同 TeamIdentifier。Core/helper 只使用单请求版本化长度前缀 stdio，限制 256 KiB 请求、4 MiB 响应和 64 KiB stderr，不启动服务或网络 listener；每个审批动作使用新建 0700 目录中的 cancel marker，超时/取消先通知 helper 将运行中取消映射到 release guard，等待最多 2 秒后才终止未响应进程。
...
- 多显示器坐标和缩放。
- 应用激活已完成；仍缺全屏和窗口定位。
- 操作前观察、通用动作后瞬时观察/禁止自动重放恢复，以及应用激活执行窗口内取消回滚已完成；仍缺应用内容、导航副作用与任意窗口布局的通用回滚。
...
- [ ] Plugin hooks 和对应生命周期 cleanup（已完成 prepare/session/tool/run/disable 事件、macOS workspace-write 逐次人工审批及 Linux Bubblewrap 实现；Windows 隔离、Linux 真实主机验收与 packaged E2E 尚未完成）。
...
- 新增不可变 Presentations Skill/Plugin Release `1.24.0`。Catalog revision 为 `2026-07-25.22`；Release ID 为 `bundled-release-presentations-1-24-0`；发布时间为 `2026-07-26T13:00:00Z`；artifact revision 为 `presentations-1.24.0`；Skill JSON SHA-256 为 `e924ea9ab91ff96bd6446d290d3af58b26576b77a53e85a940a62b6aa4cb6a78`；Instructions SHA-256 为 `0e6cd46f473b13380e0406aae826fa46da5d2dd2ecf783a5051f2a3ed0a855d4`；Bundle hash 为 `bdcc9542b49a5efc6b49d5b801552813e587596b693134715b95fe66dab8043d`；Manifest SHA-256 为 `bfd265c913410e734ee284ca9c2584364797fce99319b3ce90e9918739de7fbc`；Artifact SHA-256 为 `05ae3eaada30fb65569c5523b2d2ccccba2315a12db437adfba922a8d6d912dc`；macOS arm64/Windows x64 staged content SHA-256 均为 `90b066ecf3dd131989511a5f39b8d0c6c28f3b49e3e1bf433c8c68ba7eedd3ee`；ready/all fingerprints 分别为 `63e4cf2782872258b4897903c544b93532bb41577ee0d34c27b798b6385a73f5` 和 `4af29afeb23a19a4949f5c43c638f06a627f604c4602dd07ee6f4cd63e5d075b`。
