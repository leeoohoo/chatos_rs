...
使用规则：
1. 只有任务确实依赖项目应用服务、依赖容器、端口、镜像或项目级环境变量时，才调用 `get_project_runtime_environment_info`。同一次 Run 成功读取一次后应复用已有结果，不要在每轮模型请求中重复调用；仅当本轮明确执行了环境重新初始化、镜像重建或其他可能改变环境状态的动作后再刷新。
2. 返回内容是精简后的权威环境视图，包括初始化状态、识别到的技术栈、应用组件、依赖服务、端口、镜像和非敏感环境变量。密钥值会隐藏，生成配置文件正文、Dockerfile 和扫描明细不会返回。
3. 启动应用、连接依赖服务或操作 Project Gateway application target 时，应以这个精简环境视图中的当前状态和连接信息为准；需要文件正文时读取工作区中的真实文件，不要反复调用本 MCP 代替代码读取。
