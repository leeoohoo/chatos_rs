...
- Actionable snapshot rows contain short-lived `cr...` target IDs. A target is bound to the exact tab, origin, latest snapshot, DOM path, role/type and accessible-name fingerprint. Native single-select rows include at most 20 enabled visible option labels and the currently selected label, never option values. Targets become invalid after navigation, click, text input, selection, scroll, history movement, upload, tab release, origin change or permission removal.
...
- This release does not request Chrome `downloads` permission and does not silently copy files from the user's Downloads directory. Use the isolated Browser Plugin for workspace-bounded downloads until an existing-session download handoff has a separate user-visible, non-overwriting contract.
- Task or Plugin cancellation sends a bounded cancel message to the extension, releases the Core pending request, tolerates a late extension result and cleans up in-progress upload buffers. Cancellation stops waiting and prevents undispatched work, but it cannot reverse an action already accepted by Chrome.
...
- This release still does not read cookies, storage, history entries, downloads, bookmarks or password values; does not run model-provided JavaScript; and does not provide arbitrary selectors, CDP, network interception or cross-origin silent control.
