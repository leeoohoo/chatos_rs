...
use crate::services::task_capability_registry::{
    ...
    find_task_capability_by_token, planning_task_capability_tokens,
};
...
pub(crate) use draft_graph::prepare_task_drafts_for_creation;
...
pub async fn create_tasks_for_turn(
    ...
    let planning_snapshot = planning::build_task_planning_snapshot(
        session_id.as_str(),
        conversation_turn_id.as_str(),
        &scope,
    ...
    for mut draft in draft_tasks {
        ...
        draft.planned_builtin_mcp_ids = planning::infer_task_builtin_mcp_ids(
            &draft,
            &scope,
            contact_authorized_builtin_mcp_ids.as_slice(),
            runtime_snapshot.as_ref(),
        ...
        draft.planned_context_assets = planning::merge_runtime_selected_command_assets(
