...
interface AgentMemoryConfigCardProps {
  ...
  projectHintMessage: string;
  onChange: (config: AgentMemoryJobConfig) => void;
  onSetNumber: (
...
export function AgentMemoryConfigCard({
  config,
  modelOptions,
  title,
  enabledLabel,
  ...
  onSave,
  onCreate,
}: AgentMemoryConfigCardProps) {
  return (
    <Card size="small" title={title}>
      {config ? (
        <Form layout="vertical">
          <Alert type="info" showIcon message={projectHintMessage} style={{ marginBottom: 12 }} />
          ...
          <Form.Item label={maxAgentsLabel}>
            <InputNumber
              min={1}
              value={config.max_agents_per_tick}
              onChange={(value) => onSetNumber('max_agents_per_tick', value, 1)}
              style={{ width: '100%' }}
            />
          </Form.Item>
