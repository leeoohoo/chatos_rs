...
final class ConversationHistoryMapperTests: XCTestCase {
    func testCompactHistoryMapsStableTurnAndFormalFinalAssistant() throws {
    ...
    func testHistoryDoesNotPromoteOrdinaryAssistantMessageToFinalReply() throws {
        let response = try decode(#"""
        {
          "items": [
            {
        ...
        XCTAssertNil(page.turns[0].finalAssistantMessage)
        XCTAssertTrue(page.turns[0].assistantReplies.isEmpty)
        XCTAssertEqual(page.turns[0].status, .completed)
    }
    ...
    func testUserMessageMapsPersistedAttachments() throws {
        let response = try decode(#"""
        {
          "items": [{
            ...
            "content": "请看附件",
            "metadata": {
              "conversation_turn_id": "turn-file",
              "attachments": [{
                "id": "attachment-1",
                "name": "design.pdf",
    ...
    private func decode(_ json: String) throws -> CompactHistoryResponseDTO {
