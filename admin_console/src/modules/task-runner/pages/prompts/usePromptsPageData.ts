...
export function usePromptsPageData({
  ...
  const promptsQuery = useQuery({
    queryKey: ['task-runner', 'prompts', taskFilterId, runFilterId, statusFilter, promptPage, promptPageSize],
    queryFn: () =>
      api.listPromptsPage({
        taskId: taskFilterId,
        runId: runFilterId,
  ...
  const runSummariesQuery = useQuery({
    queryKey: ['task-runner', 'run-summaries', displayRunIds.join(',')],
    queryFn: () => api.listRunSummaries({ ids: displayRunIds }),
    enabled: displayRunIds.length > 0,
  });

  const runSearchQuery = useQuery({
    queryKey: ['task-runner', 'run-summary-search', taskFilterId, runSearchTerm],
    queryFn: () =>
