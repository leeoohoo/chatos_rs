# ChatOS Computer Use — Safe Native Control and Activation Recovery
...
This release adds bounded in-flight application-activation recovery on macOS and Windows while preserving the signed one-shot macOS helper, fail-closed native/contenteditable text entry, high-risk challenge, per-action approval, privacy audit, transient post-action observation, input-release, and no-automatic-replay contracts from `1.14.0`. It does not claim general application-content or window-layout rollback.
