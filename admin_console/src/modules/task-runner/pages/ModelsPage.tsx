    ...
    <>
      ...
      <Space direction="vertical" size="large" style={{ width: '100%' }}>
        ...
        <ModelListTable
          ...
          onViewTasks={(modelId) =>
            navigate(`/task-runner/tasks?model_config_id=${encodeURIComponent(modelId)}`)
          }
          onViewRuns={(modelId) =>
            navigate(`/task-runner/runs?model_config_id=${encodeURIComponent(modelId)}`)
          }
        />
      </Space>
      ...
      <ModelDetailDrawer
        ...
        onOpenTask={(taskId) =>
          navigate(`/task-runner/tasks?task_id=${encodeURIComponent(taskId)}`)
        }
        onOpenRun={(runId) =>
          navigate(`/task-runner/runs?run_id=${encodeURIComponent(runId)}`)
        }
      />
