...
pub struct CodeMaintainerOptions {
    ...
    pub hooks: Option<CodeMaintainerHooksRef>,
}
...
pub trait CodeMaintainerHooks: Send + Sync {
    fn note_workspace_path_changed(&self, _path: &str) {}
}

#[derive(Clone)]
pub struct CodeMaintainerHooksRef {
    inner: Arc<dyn CodeMaintainerHooks>,
}

impl CodeMaintainerHooksRef {
    pub fn new(inner: Arc<dyn CodeMaintainerHooks>) -> Self {
        Self { inner }
    }

    pub(crate) fn note_workspace_path_changed(&self, path: &str) {
        self.inner.note_workspace_path_changed(path);
    }
}
