...
public sealed partial class AgentTeamPage : UserControl
{
    ...
    private async void OnSurveyClick(object sender, RoutedEventArgs e)
    {
        if (sender is not Button { DataContext: AgentRequirementSurvey survey }) return;
        if (survey.Status == AgentRequirementSurveyStatus.Submitted)
        {
            var resolution = survey.Resolution is null
                ? "Human 已提交，正在等待项目经理形成方案。"
                : $"方案摘要\n{survey.Resolution.Summary}\n\n解决方案\n{survey.Resolution.SolutionMarkdown}\n\n风险与待定项\n{survey.Resolution.RisksAndOpenQuestions}\n\n相关资料\n{survey.Resolution.RelatedMaterials}";
            await AlertAsync(survey.Draft.Title, resolution);
            return;
        }
        ...
        if (await ShowDialogAsync(survey.Draft.Title, new ScrollViewer
        {
            Content = panel,
            MaxHeight = 620,
            VerticalScrollBarVisibility = ScrollBarVisibility.Auto,
        }, "提交") != ContentDialogResult.Primary) return;
