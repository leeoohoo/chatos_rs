...
command -v openssl >/dev/null 2>&1 || {
  echo "[ERROR] openssl is required to generate Plugin Management mTLS material" >&2
  exit 1
}

CALLERS=(chatos-backend task-runner project-service local-connector-service memory-engine mcp-management-service)
...
if material_is_current; then
  echo "[INFO] Plugin Management mTLS material is current: $OUTPUT_DIR"
  exit 0
fi
...
openssl req -x509 -new -sha256 -days 3650 -key "$temporary_dir/ca.key" \
  -subj "/CN=ChatOS Plugin Management Internal CA" -out "$temporary_dir/ca.crt"
...
openssl req -new -sha256 -key "$temporary_dir/server.key" \
  -subj "/CN=plugin-management-backend" -out "$temporary_dir/server.csr"
cat >"$temporary_dir/server.ext" <<'EOF'
basicConstraints=critical,CA:FALSE
keyUsage=critical,digitalSignature,keyEncipherment
extendedKeyUsage=serverAuth
subjectAltName=DNS:plugin-management-backend,DNS:plugin-management-service,DNS:localhost,IP:127.0.0.1
EOF
...
echo "[INFO] generated Plugin Management mTLS material: $OUTPUT_DIR"
