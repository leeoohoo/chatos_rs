...
@Suite("Shared Local Agent protocol v28 fixtures")
struct LocalAgentProtocolV28FixtureTests {
    ...
    func retryTaskRequest() throws {
        ...
        #expect(localAgentProtocolVersion == 28)
    ...
    @Test("encodes Story design creation and apply against the shared contract")
    func storyDesignCommands() throws {
        let create = Request(
            protocolVersion: localAgentProtocolVersion,
            requestID: "request-story-design-1",
            ownerUserID: "user-1",
            command: .createStoryDesign(.init(
                runID: "story-run-1",
                storyRecordID: "agent-run:story-1",
                projectID: "project-1",
                expectedProjectRevision: 7,
                modelConfigID: "model-config-1",
                stage: .refine,
                targetIDs: ["segment-1", "segment-2"],
                baseProjectDigest: "sha256:" + String(repeating: "a", count: 64)
            ))
        )
        let apply = Request(
            protocolVersion: localAgentProtocolVersion,
            requestID: "request-story-apply-1",
            ownerUserID: "user-1",
            command: .applyStoryDesign(.init(
                runID: "story-run-1",
                storyRecordID: "agent-run:story-1",
                projectID: "project-1",
                expectedProjectRevision: 7,
                expectedStoryRevision: 12
            ))
        )

        let encoder = LocalAgentProtocolJSON.encoder()
        let encodedCreate = try JSONSerialization.jsonObject(with: encoder.encode(create)) as? NSDictionary
        let fixtureCreate = try JSONSerialization.jsonObject(
            with: Data(contentsOf: fixtureURL("create_story_design_request.json"))
        ) as? NSDictionary
        #expect(encodedCreate == fixtureCreate)

        let encodedApply = try JSONSerialization.jsonObject(with: encoder.encode(apply)) as? NSDictionary
        let fixtureApply = try JSONSerialization.jsonObject(
            with: Data(contentsOf: fixtureURL("apply_story_design_request.json"))
        ) as? NSDictionary
        #expect(encodedApply == fixtureApply)
    }
    ...
    private func fixtureURL(_ name: String) -> URL {
        URL(fileURLWithPath: #filePath)
            ...
            .deletingLastPathComponent()
            .deletingLastPathComponent()
            .appendingPathComponent("shared/fixtures/local_agent/v28")
