...
use super::{ChatosProvider, ChatosRequestBinding, ProviderCallError, ProviderWaitingForUser};
...
impl ChatosProvider {
        ...
        let response = self
            ...
            .map_err(|error| {
                ProviderCallError::provider_unavailable(format!(
                    "ChatOS Ask User start request failed: {error}"
                ))
            })?;
        let value = response.json::<Value>().await.map_err(|error| {
            ProviderCallError::invalid_response(format!(
                "ChatOS Ask User start response is invalid: {error}"
            ))
        })?;
        let prompt_id = value
            ...
            .ok_or_else(|| {
                ProviderCallError::invalid_response(format!(
                    "ChatOS Ask User start response has no prompt_id: {value}"
                ))
            })?;
        let _ = route;
    ...
    pub(in crate::providers) async fn resolve_waiting_user_call(
        ...
        let response = self
            ...
            .map_err(|error| {
                ProviderCallError::provider_unavailable(format!(
                    "ChatOS Ask User resolution request failed: {error}"
                ))
            })?;
        let value = response.json::<Value>().await.map_err(|error| {
            ProviderCallError::invalid_response(format!(
                "ChatOS Ask User resolution response is invalid: {error}"
            ))
        })?;
        ...
        .map_err(|error| {
            ProviderCallError::invalid_response(format!(
                "ChatOS Ask User response payload is invalid: {error}"
            ))
        })?;
        let kind = value
            .get("kind")
