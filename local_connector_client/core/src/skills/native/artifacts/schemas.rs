...
fn inspect_pptx_charts_tool() -> Value {
    tool(
        "inspect_pptx_charts",
        "Inspect standard internally related DrawingML chart parts in true visible slide order, returning chart type, title, legend state and position, data-label mode, literal or formula-backed axis titles, recognized or custom per-series RGB colors and bounded line-marker style/size with raw values, formulas, bounded cached series/category/value previews, full chart XML SHA-256, embedded-workbook metadata, and an exact edit snapshot only when the chart is byte-identical to ChatOS canonical self-contained output. External, shared, missing, unreferenced, chartEx, or over-limit chart structures fail closed.",
        json!({
            "type":"object",
            "properties":{
...
fn replace_pptx_chart_tool() -> Value {
    tool(
        "replace_pptx_chart",
        "Replace one uniquely owned standard chart only when its XML is byte-identical to the canonical self-contained column/line/pie/area/doughnut form generated by ChatOS. The bounded replacement may also update per-series RGB colors, canonical line-marker style/size, legend position, value/percentage data labels, and axis titles where supported. Requires the exact chart XML SHA-256 and complete edit snapshot from inspect_pptx_charts, writes a distinct PPTX, and preserves the slide frame, relationship, chart part name, and every unrelated package entry.",
        json!({
            "type":"object",
            "properties":{
                ...
                "replacement":self_contained_pptx_chart_schema(
                    "Replacement chart within the bounded self-contained column, line, pie, area, or doughnut contract.",
                    false,
                ),
                "overwrite":{"type":"boolean","default":false}
            },
...
fn create_pptx_tool() -> Value {
    ...
    tool(
        "create_pptx",
        "Create a bounded editable widescreen PPTX locally with title/body, title-only, section, two-column, image-right, full-image, simple rectangular table, or self-contained standard DrawingML column/line/pie/area/doughnut chart layouts with optional canonical RGB series colors and bounded line-marker styles, optional PNG/JPEG images, bullet lines, and speaker notes.",
        json!({
            "type":"object",
            "properties":{
...
fn self_contained_pptx_chart_schema(description: &str, complete_snapshot: bool) -> Value {
    ...
    json!({
        ...
        "properties":{
            "type":{"type":"string","enum":["column","line","pie","area","doughnut"],"description":"column creates a clustered vertical bar chart; line, pie, area, and doughnut use bounded standard 2D forms. Pie and doughnut require exactly one non-negative series with at least one positive value."},
            ...
            "series":{
                ...
                "items":{
                    ...
                    "properties":{
                        ...
                        "value_axis":{"type":"string","enum":["primary","secondary"],"default":"primary","description":"Assign this series to the primary left value axis or canonical secondary right value axis. Secondary is supported only for column, line, or area charts and requires at least one primary series."},
                        "color":{"type":["string","null"],"pattern":"^#[0-9A-Fa-f]{6}$","default":null,"description":"Optional canonical RGB series color in #RRGGBB form. Values are normalized to uppercase; line charts use the series line color while other supported charts use the series fill color."},
            ...
            "category_axis_title":{"type":"string","maxLength":1000,"default":"","description":"Optional category-axis title for column, line, or area charts. Pie and doughnut charts reject axis titles."},
            "value_axis_title":{"type":"string","maxLength":1000,"default":"","description":"Optional primary left value-axis title for column, line, or area charts. Pie and doughnut charts reject axis titles."},
            "secondary_value_axis_title":{"type":"string","maxLength":1000,"default":"","description":"Optional secondary right value-axis title. It requires at least one primary and one secondary column, line, or area series."},
...
fn append_pptx_slides_tool() -> Value {
    ...
    tool(
        "append_pptx_slides",
        "Append bounded editable slides, including self-contained standard DrawingML column/line/pie/area/doughnut chart slides with optional canonical RGB series colors, to an existing PPTX while preserving unchanged package parts and writing a distinct output file. New slides inherit the last existing slide's layout. Speaker notes require an existing notes master.",
        schema,
    )
}
