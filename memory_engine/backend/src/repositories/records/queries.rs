...
pub struct PendingRecordThreadScope {
    pub tenant_id: String,
    pub source_id: String,
    pub thread_id: String,
}

pub async fn list_pending_record_thread_scopes(
    db: &Db,
    limit: i64,
) -> Result<Vec<PendingRecordThreadScope>, String> {
    let now = crate::models::now_rfc3339();
    let rows = db
        .collection::<Document>("engine_records")
        .aggregate(vec![
            doc! {
                "$match": {
                    "$or": [
                    ...
                    "pipeline": [
                        ...
                        },
                        {
                            "$project": {
                                ...
                                "record_sync_inflight": 1,
                                "record_sync_lease_expires_at": 1,
                            }
                        },
                    ],
                    "as": "thread_state",
            ...
            doc! {
                "$set": { "thread_state": { "$first": "$thread_state" } }
            },
            doc! {
                "$match": {
                    "$and": [
                        {
                            "$expr": {
                                "$ne": [
                                    { "$ifNull": ["$thread_state.pending_record_count", -1] },
                        ...
                        {
                            "$or": [
                                { "thread_state.record_sync_inflight": { "$exists": false } },
                                { "thread_state.record_sync_inflight": { "$lte": 0 } },
                                { "thread_state.record_sync_lease_expires_at": { "$exists": false } },
                                { "thread_state.record_sync_lease_expires_at": Bson::Null },
                                { "thread_state.record_sync_lease_expires_at": { "$lte": &now } },
                            ]
                        },
                    ]
                }
            },
            doc! { "$limit": limit.clamp(1, 10_000) },
        ])
        .await
    ...
    rows.into_iter()
        .map(|row| {
            let scope = row.get_document("_id").map_err(|err| err.to_string())?;
            Ok(PendingRecordThreadScope {
                tenant_id: scope
                    .get_str("tenant_id")
                    .map_err(|err| err.to_string())?
                    .to_string(),
