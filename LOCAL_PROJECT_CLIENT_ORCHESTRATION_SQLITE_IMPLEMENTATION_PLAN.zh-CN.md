...
- 本地 Plan Mode 已接入进程内 Project Management Provider，直接复用 `chatos_project_mcp_contract` 的 16 个工具合同访问 SQLite；Plan Mode 只暴露 Project Management 工具，不加载用户配置的远端 MCP，也不会请求远端 Project Management Service `/mcp`。
...
Plan Mode、本地 Chat Agent、Task Worker 和 Project Environment Agent 不得调用远端 `/mcp`。客户端需要提供 in-process Project Management Tool Provider，复用现有 `chatos_project_mcp_contract` 的工具名称和输入输出合同，并直接访问 SQLite Repository。
