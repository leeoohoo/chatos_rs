...
当前 2.0.10 实施状态：Phase 0～7、全量门禁与云端真实联调均已完成。权威 Project Execution Context、Plugin Management capability 聚合、required MCP 阻断、工具 Schema 快照、稳定 `route_revision`、短期 Runtime Grant 和共享加密 Session Snapshot 已接通；聚合 `/mcp` 已支持初始化、工具发现与调用、固定路由校验、结构化 invocation 日志、Provider 超时、响应限额与取消。Local Connector、Harness、Cloud Sandbox、内部服务、External MCP 和 Plugin Runtime 均已纳入统一路由。Task Runner、ChatOS 与 Project Environment Agent 已删除 `off/shadow` 模式及旧直连 builder，其 Agent 工具只能通过 MCP Management 调用；Task Runner 也不再生成或保存 Harness/Local Connector ephemeral MCP endpoint，旧 endpoint header 不能选择能力、改变 Provider 或绕过 Sandbox 判定。Project Environment Agent 固定拥有 Project Environment 与 Sandbox Images MCP，但 Plugin Management 不再用 cloud 条件选择 Provider；MCP Management 会依据不可变 Project Context 将其分别绑定到 Project Service、Sandbox Manager 或权威 Local Connector pairing，本地 pairing 离线或不可用时明确 fail closed，绝不回退云端。五类 Memory Agent 固定为 `tool_plane=none`。Local Connector Command Approval Agent 是明确例外，模型循环、本地只读工具、`approval_decision`、人工审批、风险判断、白名单、Session Approval 和审批历史完整保留在设备侧。仓库提供 `make smoke-cloud-mcp` 作为可重复的真实云端验收：它读取真实云端/本地 Project Execution Context，验证云端 Workspace 工具固定走 Harness、本地 Workspace 工具固定走权威 Local Connector device/workspace；真实执行云端 Project Environment Agent 的 Sandbox Manager `get_image_catalog`，并在 Local Connector 离线时验证本地 Project Environment Runtime Session 明确 409 且无云端 fallback；随后验证 Runtime Session 创建/关闭、不可变 Route、命名空间化工具发现、Task Runner、Project Management 与 ChatOS Notepad 三类内部服务的真实 `tools/call`、owner/caller 隔离、本地审批 Agent 云端拒绝、Token 防篡改和关闭后不可复用。
...
3. 所有云端编排 Agent 的工具调用只连接一个聚合 MCP Server：MCP Management Service；本地命令审批 Agent 只调用本地工具。
...
| LocalCommandApproval | 本地审批 Agent 内部执行，不进入网关 | 不可用 | Local-only |
...
- 已删除 TaskRunnerSystemMcpAdapter、旧 external/system MCP loader、宿主 builtin Registry 执行链和 Project Runtime Environment 直连 builder。
...
- local_connector + LocalCommandApproval -> unavailable（本地审批 Agent 不进入 MCP Management）。
- harness/cloud_sandbox + LocalCommandApproval -> unavailable。
...
1. 云端编排 Agent Runtime 只连接一个聚合 MCP Server；本地命令审批 Agent 只连接本地工具执行器。
