...
command -v openssl >/dev/null 2>&1 || {
  echo "[ERROR] openssl is required to generate ChatOS mTLS material" >&2
  exit 1
}

CALLERS=(task-runner mcp-management-service)
...
if material_is_current; then
  echo "[INFO] ChatOS mTLS material is current: $OUTPUT_DIR"
  exit 0
fi
...
openssl req -x509 -new -sha256 -days 3650 -key "$temporary_dir/ca.key" \
  -subj "/CN=ChatOS Internal CA" -out "$temporary_dir/ca.crt"
...
openssl req -new -sha256 -key "$temporary_dir/server.key" \
  -subj "/CN=chatos-backend" -out "$temporary_dir/server.csr"
cat >"$temporary_dir/server.ext" <<'EOF'
basicConstraints=critical,CA:FALSE
keyUsage=critical,digitalSignature,keyEncipherment
extendedKeyUsage=serverAuth
subjectAltName=DNS:chatos-backend,DNS:localhost,IP:127.0.0.1
EOF
...
echo "[INFO] generated ChatOS mTLS material: $OUTPUT_DIR"
