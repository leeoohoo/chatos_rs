...
| Agent 团队 | Agent 配置、团队、成员、项目经理和默认 Agent | Windows 验收 | 本机账号隔离的 Agent Profile、模型/推理/职业/插件/Skill/心跳配置，项目团队、成员职责、项目经理、默认路由和 Human/Agent 私聊均已接入 SQLite、Presentation 与 WinUI；等待 Windows 真机交互验收 |
| Agent 团队 | 团队消息、@ 路由、附件和 Agent 间私聊 | Windows 验收 | durable message/delivery、精确 @、默认 Agent、4-hop/12-run 限流、附件选择/元数据时间线/按需保存、Agent 间私聊和崩溃恢复已实现；历史消息采用批量查询，附件正文已迁移到独立 payload 表，时间线不读取大 BLOB |
| Agent 团队 | Todo、依赖、进展、调度和共享资产 | Windows 验收 | Todo revision、依赖释放、排序、执行进展、项目经理状态唤醒、执行者资产更新建议、完整资产分类、显式 create/update revision 工具、新旧团队资产维护唤醒和 manager/executor 权限边界已实现并自动化覆盖 |
| Agent 团队 | 需求调研 | Windows 验收 | 项目经理或获授 `requirement.survey.manage` 的团队成员可创建结构化单选/多选调研，Human 在 WinUI 填写，提交后 durable 唤醒原调研 Agent（权限撤销时回退项目经理）形成方案、执行步骤、风险和资料；幂等 request key、真实答案校验与权限边界已测试 |
...
| 质量 | Core/API 自动化测试 | Windows 验收 | 当前 Core 25、API 45、Presentation 47、Connector 327、NetworkGuard 19，共 463 项测试通过；WindowsNative 和显式启用的 NetworkGuard 端到端测试仍需在 Windows 执行真实系统 API |
| 质量 | Connector 集成测试 | Windows 验收 | 当前 Connector 327 项，新增覆盖 Agent 团队账号隔离、路由限流、附件 payload 按需读取、Todo 依赖/进展、资产建议/权限/维护唤醒、专职 Agent 需求调研、心跳、Responses API、模型工具循环和插件 MCP run session；WindowsNative 还验证系统 API、Credential Manager、WinUI 和原生命令执行 |
