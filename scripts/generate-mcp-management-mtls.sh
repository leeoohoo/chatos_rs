...
if ! command -v openssl >/dev/null 2>&1; then
  echo "[ERROR] openssl is required to generate MCP Management mTLS material" >&2
  exit 1
fi

CALLERS=(chatos task-runner project-service configuration-center)
...
if material_is_current; then
  echo "[INFO] MCP Management mTLS material is current: $OUTPUT_DIR"
  exit 0
fi
...
openssl req -x509 -new -sha256 -days 3650 \
  ...
  -subj "/CN=ChatOS MCP Management Internal CA" \
...
openssl req -new -sha256 \
  ...
  -subj "/CN=mcp-management-service-backend" \
  -out "$temporary_dir/server.csr"
cat >"$temporary_dir/server.ext" <<'EOF'
basicConstraints=critical,CA:FALSE
keyUsage=critical,digitalSignature,keyEncipherment
extendedKeyUsage=serverAuth
subjectAltName=DNS:mcp-management-service-backend,DNS:mcp-management-service,DNS:localhost,IP:127.0.0.1
EOF
...
echo "[INFO] generated MCP Management mTLS material: $OUTPUT_DIR"
