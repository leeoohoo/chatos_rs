...
1. 前端项目创建入口在 `chatos/frontend/src/components/sessionList/CreateResourceModals.tsx`，目前 `server` 模式展示目录输入和目录选择按钮，`local_connector` 模式走本地连接器目录浏览。
2. 前端创建调用链是 `useSessionListActions.ts -> store createProject(name, rootPath) -> client.createProject({ name, root_path })`，`rootPath` 是必填语义。
3. `chatos/backend/src/api/projects/crud_handlers.rs` 目前强制 `root_path` 必填，并会校验它是存在且可写的服务端目录。
