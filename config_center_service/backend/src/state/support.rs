...
use crate::catalog::{
    ...
    PROJECT_SERVICE_MCP_RESULT_RABBITMQ_URL_CONFIG_KEY, PROJECT_SERVICE_PORT_CONFIG_KEY,
    ...
    SANDBOX_MANAGER_INTERNAL_MTLS_PORT_CONFIG_KEY, SANDBOX_MANAGER_LEASE_TTL_SECONDS_CONFIG_KEY,
    SANDBOX_MANAGER_MONGODB_DATABASE_CONFIG_KEY, SANDBOX_MANAGER_PORT_CONFIG_KEY,
...
pub(super) fn ensure_sandbox_manager_runtime_values(
    ...
    let mut changed_keys = Vec::new();
    for (key, fallback) in defaults {
        if key == SANDBOX_MANAGER_REQUIRE_AUTH_CONFIG_KEY
    ...
    for (key, fallback) in defaults {
        ...
        let requires_strict_auth = matches!(
            ...
            crate::catalog::PROJECT_SERVICE_REQUIRE_SIGNED_INTERNAL_REQUESTS_CONFIG_KEY
                ...
                | crate::catalog::SANDBOX_MANAGER_REQUIRE_SIGNED_INTERNAL_REQUESTS_CONFIG_KEY
        );
...
pub(super) fn ensure_project_service_runtime_values(
    ...
    let mut changed_keys = Vec::new();
    for (key, fallback) in defaults {
        if key == PROJECT_SERVICE_MCP_RESULT_RABBITMQ_URL_CONFIG_KEY {
