...
| PDF | 部分可用，本机生成、结构编辑、瞬时视觉 QA 与持久页面图片导出 | `1.19.0` 已覆盖 searchable ASCII 文本生成、图片生成 PDF、Unicode Document Info、标准 AcroForm 检查与精确快照填写、文本提取、合并/提取/重排/删除/旋转页面、动态页码、Unicode Text 便签、标准高亮/下划线/删除线/波浪线 markup、源 SHA-256 与页内索引绑定的标准 Text/markup 根批注回复、标准 FileAttachment 对象链及双 SHA-256 原子提取，以及 Catalog `/Names/EmbeddedFiles` 嵌套 Name Tree 的有界检查和源/附件双 SHA-256、preview index 绑定原子提取；同时覆盖精确 CropBox 页面几何、透明文本/图片盖章、经 Manifest 校验的 Poppler 瞬时页面渲染，以及最多 50 个物理页面的 96–300 DPI PNG 持久导出。仍缺经过许可校验的 Unicode 嵌入字体、富排版/表格、OCR、任意非标准附件关系与更多通用注释关系、密码工作流、密码学签名、自动视觉语义判断和任意页面内容编辑 |
