    ...
    AgentPluginToolRuntime? pluginTools = null)
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var recentMessages = executionTodo is null
            ...
            : [];
        var sourceMessages = executionTodo is null
            ? []
                ...
                try
                {
                    result = pluginSession?.CanExecute(call.Name) == true
                        ...
                        : await tools.ExecuteAsync(profile, member, room, delivery, call,
                            cancellationToken, references, executionTodo).ConfigureAwait(false);
                }
        ...
        IReadOnlyList<AgentMultimodalAttachment> multimodalAttachments)
    {
        ...
        var system = $"""
            ...
            2. 团队任务以 todo_list 为权威状态；执行由程序计算的 todo_schedule_state/todo_start_next 串行调度，不得自行把 Ready 改为 InProgress；经理通讯线程只能重排或取消，执行者用 todo_progress、todo_complete、todo_block 写入执行状态。
