...
mod tests {
    ...
    fn installs_and_uninstalls_verified_staged_bundled_plugin() {
        ...
        assert_eq!(
            ...
            "bundled-release-computer-use-1-17-0"
        );
        assert_eq!(installed.installed_version.version, "1.17.0");
        assert_eq!(
    ...
    fn rejects_tampered_staged_bundled_plugin_and_records_rejection() {
        ...
        let relative = Path::new("internal/computer-use/1.17.0");
    ...
    fn updates_bundled_plugin_and_preserves_verified_rollback_target() {
        ...
        let (_, inventory, _) =
            expected_manifest_and_inventory(&spec).expect("Computer Use inventory");
        let previous_relative_path = "installed/computer-use--fixture/1.16.0";
        ...
        let previous = InstalledPluginVersion {
            release_id: "bundled-release-computer-use-1-16-0".to_string(),
        ...
        assert_eq!(updated.installed_version.version, "1.17.0");
        assert_eq!(updated.plugin.previous_version.as_deref(), Some("1.16.0"));
        let rolled_back = installer
            .rollback(spec.plugin_id.as_str())
            .expect("rollback to previous bundled version");
        assert_eq!(rolled_back.version.version, "1.16.0");
