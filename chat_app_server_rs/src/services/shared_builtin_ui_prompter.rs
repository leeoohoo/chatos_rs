use async_trait::async_trait;
use serde_json::json;

use chatos_builtin_tools::{
    UiPromptDecision as SharedUiPromptDecision, UiPromptPayload as SharedUiPromptPayload,
    UiPromptResponseSubmission as SharedUiPromptResponseSubmission, UiPromptStreamChunkCallback,
    UiPrompterStore,
};

use crate::services::ui_prompt_manager::{
    UI_PROMPT_TIMEOUT_ERR, UiPromptDecision as ChatosUiPromptDecision,
    UiPromptPayload as ChatosUiPromptPayload,
    UiPromptResponseSubmission as ChatosUiPromptResponseSubmission,
    UiPromptStatus as ChatosUiPromptStatus, create_ui_prompt_record, create_ui_prompt_request,
    redact_response_for_store, update_ui_prompt_response, wait_for_ui_prompt_decision,
};
use crate::utils::events::Events;

#[derive(Debug, Clone, Default)]
pub struct ChatosUiPrompterStore;

#[async_trait]
impl UiPrompterStore for ChatosUiPrompterStore {
    async fn execute_prompt(
        &self,
        payload: SharedUiPromptPayload,
        on_stream_chunk: Option<UiPromptStreamChunkCallback>,
    ) -> Result<SharedUiPromptDecision, String> {
        let payload = shared_payload_into_chatos(payload);
        create_ui_prompt_record(&payload).await?;

        let (registered_payload, receiver) = create_ui_prompt_request(payload.clone()).await?;
        emit_ui_prompt_required_event(on_stream_chunk.as_ref(), &registered_payload);

        let decision = match wait_for_ui_prompt_decision(
            registered_payload.prompt_id.as_str(),
            receiver,
            registered_payload.timeout_ms,
        )
        .await
        {
            Ok(decision) => decision,
            Err(err) if err == UI_PROMPT_TIMEOUT_ERR => {
                let timeout_response = ChatosUiPromptResponseSubmission {
                    status: ChatosUiPromptStatus::Timeout.as_str().to_string(),
                    values: None,
                    selection: None,
                    reason: Some("timeout".to_string()),
                };
                let _ = update_ui_prompt_response(
                    registered_payload.prompt_id.as_str(),
                    ChatosUiPromptStatus::Timeout,
                    Some(json!({ "status": "timeout" })),
                )
                .await;
                emit_ui_prompt_resolved_event(
                    on_stream_chunk.as_ref(),
                    registered_payload.prompt_id.as_str(),
                    ChatosUiPromptStatus::Timeout,
                );
                return Ok(chatos_decision_into_shared(ChatosUiPromptDecision {
                    status: ChatosUiPromptStatus::Timeout,
                    response: timeout_response,
                }));
            }
            Err(err) => return Err(err),
        };

        let redacted_response = redact_response_for_store(&decision.response, &registered_payload);
        let _ = update_ui_prompt_response(
            registered_payload.prompt_id.as_str(),
            decision.status,
            Some(redacted_response),
        )
        .await;
        emit_ui_prompt_resolved_event(
            on_stream_chunk.as_ref(),
            registered_payload.prompt_id.as_str(),
            decision.status,
        );
        Ok(chatos_decision_into_shared(decision))
    }
}

fn shared_payload_into_chatos(payload: SharedUiPromptPayload) -> ChatosUiPromptPayload {
    ChatosUiPromptPayload {
        prompt_id: payload.prompt_id,
        conversation_id: payload.conversation_id,
        conversation_turn_id: payload.conversation_turn_id,
        tool_call_id: payload.tool_call_id,
        kind: payload.kind,
        title: payload.title,
        message: payload.message,
        allow_cancel: payload.allow_cancel,
        timeout_ms: payload.timeout_ms,
        payload: payload.payload,
    }
}

fn chatos_decision_into_shared(decision: ChatosUiPromptDecision) -> SharedUiPromptDecision {
    SharedUiPromptDecision {
        status: decision.status.as_str().to_string(),
        response: chatos_response_into_shared(decision.response),
    }
}

fn chatos_response_into_shared(
    response: ChatosUiPromptResponseSubmission,
) -> SharedUiPromptResponseSubmission {
    SharedUiPromptResponseSubmission {
        status: response.status,
        values: response.values,
        selection: response.selection,
        reason: response.reason,
    }
}

fn emit_ui_prompt_required_event(
    on_stream_chunk: Option<&UiPromptStreamChunkCallback>,
    payload: &ChatosUiPromptPayload,
) {
    let Some(callback) = on_stream_chunk else {
        return;
    };
    let chunk = json!({
        "event": Events::UI_PROMPT_REQUIRED,
        "data": payload,
    });
    if let Ok(serialized) = serde_json::to_string(&chunk) {
        callback(serialized);
    }
}

fn emit_ui_prompt_resolved_event(
    on_stream_chunk: Option<&UiPromptStreamChunkCallback>,
    prompt_id: &str,
    status: ChatosUiPromptStatus,
) {
    let Some(callback) = on_stream_chunk else {
        return;
    };
    let chunk = json!({
        "event": Events::UI_PROMPT_RESOLVED,
        "data": {
            "prompt_id": prompt_id,
            "status": status.as_str(),
        }
    });
    if let Ok(serialized) = serde_json::to_string(&chunk) {
        callback(serialized);
    }
}
