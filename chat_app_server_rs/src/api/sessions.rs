use axum::{Router, Json, routing::{get, delete}, extract::{Path, Query}};
...
pub fn router() -> Router {
    Router::new()
        ...
        .route("/api/sessions/:session_id/mcp-servers", get(list_mcp_servers).post(add_mcp_server))
        .route("/api/sessions/:session_id/mcp-servers/:mcp_config_id", delete(delete_mcp_server))
        .route("/api/sessions/:session_id/messages", get(get_session_messages).post(create_session_message))
}
...
async fn create_session(Json(req): Json<CreateSessionRequest>) -> (StatusCode, Json<Value>) {
    ...
    if title.trim().is_empty() {
        return (StatusCode::BAD_REQUEST, Json(serde_json::json!({"error": "会话标题不能为空"})));
    }
    let session = Session::new(title, req.description, req.metadata, req.user_id, req.project_id);
    ...
    let saved = SessionService::get_by_id(&session.id).await.ok().flatten().unwrap_or(session);
...
async fn get_session(Path(id): Path<String>) -> (StatusCode, Json<Value>) {
    match SessionService::get_by_id(&id).await {
        Ok(Some(session)) => (StatusCode::OK, Json(serde_json::to_value(session).unwrap_or(Value::Null))),
        Ok(None) => (StatusCode::NOT_FOUND, Json(serde_json::json!({"error": "会话不存在"}))),
        Err(err) => (StatusCode::INTERNAL_SERVER_ERROR, Json(serde_json::json!({"error": err}))),
    }
}

async fn update_session(Path(id): Path<String>, Json(req): Json<UpdateSessionRequest>) -> (StatusCode, Json<Value>) {
    if let Err(err) = SessionService::update(&id, req.title.clone(), req.description.clone(), req.metadata.clone()).await {
        return (StatusCode::INTERNAL_SERVER_ERROR, Json(serde_json::json!({"error": err})));
    }
    match SessionService::get_by_id(&id).await {
        Ok(Some(session)) => (StatusCode::OK, Json(serde_json::to_value(session).unwrap_or(Value::Null))),
...
async fn delete_session(Path(id): Path<String>) -> (StatusCode, Json<Value>) {
    match SessionService::delete(&id).await {
        Ok(_) => (StatusCode::OK, Json(serde_json::json!({"success": true, "message": "会话已删除"}))),
        Err(err) => (StatusCode::INTERNAL_SERVER_ERROR, Json(serde_json::json!({"error": err}))),
    }
}
...
async fn list_mcp_servers(Path(session_id): Path<String>) -> (StatusCode, Json<Value>) {
    match session_mcp_repo::list_session_mcp_servers(&session_id).await {
        Ok(res) => (StatusCode::OK, Json(serde_json::to_value(res).unwrap_or(Value::Null))),
        Err(err) => (StatusCode::INTERNAL_SERVER_ERROR, Json(serde_json::json!({"error": "获取会话MCP服务器失败", "detail": err}))),
    }
}
...
async fn add_mcp_server(Path(session_id): Path<String>, Json(req): Json<AddMcpServerRequest>) -> (StatusCode, Json<Value>) {
    ...
    if let Err(err) = session_mcp_repo::add_session_mcp_server(&item).await {
        return (StatusCode::INTERNAL_SERVER_ERROR, Json(serde_json::json!({"error": "添加会话MCP服务器失败", "detail": err})));
    }
    (StatusCode::CREATED, Json(serde_json::json!({"id": id, "session_id": session_id, "mcp_server_name": req.mcp_server_name, "mcp_config_id": req.mcp_config_id})))
}

async fn delete_mcp_server(Path((session_id, mcp_config_id)): Path<(String, String)>) -> (StatusCode, Json<Value>) {
    match session_mcp_repo::delete_session_mcp_server(&session_id, &mcp_config_id).await {
        Ok(_) => (StatusCode::OK, Json(serde_json::json!({"success": true}))),
        Err(err) => (StatusCode::INTERNAL_SERVER_ERROR, Json(serde_json::json!({"error": "删除会话MCP服务器关联失败", "detail": err}))),
    }
}

async fn get_session_messages(Path(session_id): Path<String>, Query(query): Query<PageQuery>) -> (StatusCode, Json<Value>) {
    let limit = parse_limit(query.limit);
    let offset = parse_offset(query.offset);
    ...
    match result {
        Ok(list) => {
            let out: Vec<Value> = list.into_iter().map(|m| serde_json::to_value(MessageOut::from(m)).unwrap_or(Value::Null)).collect();
            (StatusCode::OK, Json(Value::Array(out)))
        }
        Err(err) => (StatusCode::INTERNAL_SERVER_ERROR, Json(serde_json::json!({"error": "获取会话消息失败", "detail": err}))),
    }
}

async fn create_session_message(Path(session_id): Path<String>, Json(req): Json<CreateMessageRequest>) -> (StatusCode, Json<Value>) {
