...
export function CanvasComponent({ component, resolved, selected, primary, interactive, forcedState, tokens, slotContent, onPointerDown, onResizePointerDown, onPreviewActivate, onEditContents }: {
  ...
  return (
    <div data-component-id={component.id} data-selected={selected || undefined} data-primary-selection={primary || undefined} className={`canvas-component type-${component.type} ${component.library ? `library-component library-${component.library.name}` : ''} ${component.locked ? 'locked' : ''} ${interactive ? 'interactive' : ''}`} style={style} tabIndex={interactive && component.states?.focus ? 0 : undefined} onPointerEnter={() => interactive && setHovered(true)} onPointerLeave={() => { setHovered(false); setPressed(false); }} onPointerDown={(event) => { pointerOrigin.current = { x: event.clientX, y: event.clientY }; pointerMoved.current = false; suppressClick.current = event.shiftKey || event.metaKey || event.ctrlKey; if (interactive) setPressed(true); onPointerDown(event); }} onPointerUp={(event) => { const origin = pointerOrigin.current; pointerMoved.current = Boolean(origin && Math.hypot(event.clientX - origin.x, event.clientY - origin.y) > 4); setPressed(false); }} onPointerCancel={() => { pointerOrigin.current = null; pointerMoved.current = false; suppressClick.current = false; setPressed(false); }} onFocus={() => interactive && setFocused(true)} onBlur={() => setFocused(false)} onDoubleClick={(event) => { if (!interactive && onEditContents) { event.preventDefault(); event.stopPropagation(); onEditContents(); } }} onClick={(event) => { if (suppressClick.current) { event.preventDefault(); event.stopPropagation(); } else if (interactive && component.interaction) { event.stopPropagation(); onPreviewActivate(); } pointerOrigin.current = null; pointerMoved.current = false; suppressClick.current = false; }}>
