...
pub(super) async fn adopt_task_session_entries_for_retry(
    store: &AppStore,
    root_task_id: &str,
    ...
    let tasks = store
        ...
        .await?;
    let candidates = tasks.into_iter().filter(|task| {
        task_is_in_session(task, previous_session_id)
            && effective_task_manager_scope(task) == TaskManagerScope::RunChecklist
            && !matches!(
                effective_task_closure_state(task),
                TaskClosureState::Satisfied
    ...
    for mut task in candidates {
        ...
        task.task_tool_state.task_session_id = Some(next_session_id.to_string());
        ...
        task.task_tool_state.closure_reason = None;
        task.task_tool_state.superseded_by_run_id = None;
        task.task_tool_state.completed_at = None;
        task.task_tool_state.cancel_reason = None;
        task.task_tool_state.cancelled_at = None;
        task.task_tool_state.blocker_reason = None;
        task.task_tool_state.blocker_needs.clear();
        task.task_tool_state.blocker_kind = None;
        task.task_tool_state.lifecycle_updated_at = Some(now.clone());
        task.updated_at = now;
        prepared.push((original, task));
    }

    let mut adopted = Vec::with_capacity(prepared.len());
    let mut originals_to_restore = Vec::with_capacity(prepared.len());
    for (original, task) in prepared {
        match store.save_task(task).await {
            Ok(saved) => {
                originals_to_restore.push(original);
                adopted.push(saved);
            }
            Err(err) => {
                let mut rollback_errors = Vec::new();
                for original in originals_to_restore.into_iter().rev() {
                    if let Err(rollback_err) = store.save_task(original).await {
                        rollback_errors.push(rollback_err);
                    }
                }
                if rollback_errors.is_empty() {
                    return Err(format!(
                        "failed to adopt Task Manager retry session; prior changes were rolled back: {err}"
                    ));
                }
                return Err(format!(
                    "failed to adopt Task Manager retry session: {err}; rollback also failed: {}",
