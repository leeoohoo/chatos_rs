...
export function ToolingPage() {
  ...
  const notepadFoldersQuery = useQuery({
    queryKey: ['task-runner', 'tooling', 'notepad', 'folders', resolvedNotepadUserId],
    queryFn: () => api.listToolingNotepadFolders(resolvedNotepadUserId),
  });
  const notepadTagsQuery = useQuery({
    queryKey: ['task-runner', 'tooling', 'notepad', 'tags', resolvedNotepadUserId],
  ...
  const selectedNoteQuery = useQuery({
    queryKey: ['task-runner', 'tooling', 'notepad', 'note', resolvedNotepadUserId, selectedNoteId],
  ...
  const killTerminalMutation = useMutation({
    ...
    onSuccess: async () => {
      await Promise.all([
        queryClient.invalidateQueries({ queryKey: ['task-runner', 'tooling', 'terminal', 'processes'] }),
        queryClient.invalidateQueries({ queryKey: ['task-runner', 'tooling', 'terminal', 'logs'] }),
      ]);
  ...
  const writeTerminalMutation = useMutation({
    ...
    onSuccess: async () => {
      setTerminalInput('');
      await queryClient.invalidateQueries({ queryKey: ['task-runner', 'tooling', 'terminal', 'logs'] });
