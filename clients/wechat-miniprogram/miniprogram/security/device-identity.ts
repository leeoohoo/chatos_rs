...
class DeviceIdentityStore {
  ...
  private async loadOrCreate(): Promise<{ deviceId: string; keyPair: nacl.SignKeyPair }> {
    ...
    if (typeof storedDeviceId === 'string' && storedDeviceId.trim() && typeof storedSeed === 'string') {
      try {
        ...
        if (seed.length === nacl.sign.seedLength) {
          this.cached = { deviceId: storedDeviceId, keyPair: nacl.sign.keyPair.fromSeed(seed) }
          return this.cached
        }
      } catch {
    ...
    const seed = random.slice(0, nacl.sign.seedLength)
    ...
    this.cached = { deviceId, keyPair: nacl.sign.keyPair.fromSeed(seed) }
    return this.cached
  }
}
