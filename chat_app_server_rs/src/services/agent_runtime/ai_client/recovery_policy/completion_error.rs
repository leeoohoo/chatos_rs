...
impl AiClient {
    pub(in crate::services::agent_runtime::ai_client) async fn try_recover_from_completion_error(
        &mut self,
        err_msg: &str,
        ...
        if is_missing_tool_call_error(err_msg) {
            warn!(
                "[Agent Runtime] completion failed due to missing tool call context; rebuild stateless input"
            );
