...
TASK_RUNNER_FRONTEND_BASE_PATH="${TASK_RUNNER_FRONTEND_BASE_PATH:-${VITE_BASE_PATH:-/}}"
...
TASK_RUNNER_WORKSPACE_DIR_EFFECTIVE="${TASK_RUNNER_WORKSPACE_DIR:-${CHATOS_WORKSPACE_DIR:-$ROOT_DIR/.chatos/task-runner-workspace}}"
...
LOCAL_CONNECTOR_SERVICE_HOST_EFFECTIVE="${LOCAL_CONNECTOR_SERVICE_HOST:-127.0.0.1}"
if [[ "$LOCAL_CONNECTOR_SERVICE_HOST_EFFECTIVE" == "0.0.0.0" || "$LOCAL_CONNECTOR_SERVICE_HOST_EFFECTIVE" == "::" || "$LOCAL_CONNECTOR_SERVICE_HOST_EFFECTIVE" == "[::]" ]]; then
  LOCAL_CONNECTOR_SERVICE_HOST_EFFECTIVE="127.0.0.1"
fi
LOCAL_CONNECTOR_SERVICE_PORT_EFFECTIVE="${LOCAL_CONNECTOR_SERVICE_PORT:-39230}"
TASK_RUNNER_LOCAL_CONNECTOR_SERVICE_BASE_URL_EFFECTIVE="${TASK_RUNNER_LOCAL_CONNECTOR_SERVICE_BASE_URL:-${LOCAL_CONNECTOR_SERVICE_BASE_URL:-${CHATOS_LOCAL_CONNECTOR_SERVICE_BASE_URL:-http://${LOCAL_CONNECTOR_SERVICE_HOST_EFFECTIVE}:${LOCAL_CONNECTOR_SERVICE_PORT_EFFECTIVE}}}}"
TASK_RUNNER_LOCAL_CONNECTOR_INTERNAL_API_SECRET_EFFECTIVE="${TASK_RUNNER_LOCAL_CONNECTOR_INTERNAL_API_SECRET:-${LOCAL_CONNECTOR_INTERNAL_API_SECRET:-${CHATOS_LOCAL_CONNECTOR_INTERNAL_API_SECRET:-chatos-local-connector-dev-secret}}}"
...
start_task_runner_frontend() {
  launch_service \
    ...
    "cd \"$TASK_RUNNER_FRONTEND_DIR\" && TASK_RUNNER_FRONTEND_PORT=\"$TASK_RUNNER_FRONTEND_PORT\" TASK_RUNNER_API_PROXY_TARGET=\"$TASK_RUNNER_API_PROXY_TARGET\" VITE_BASE_PATH=\"$TASK_RUNNER_FRONTEND_BASE_PATH\" VITE_API_BASE_URL=\"$TASK_RUNNER_VITE_API_BASE_URL\" exec npm run dev -- --host 0.0.0.0 --port \"$TASK_RUNNER_FRONTEND_PORT\""
}
...
print_runtime_info() {
  ...
  echo "  Task Runner workspace base: $TASK_RUNNER_WORKSPACE_DIR_EFFECTIVE"
