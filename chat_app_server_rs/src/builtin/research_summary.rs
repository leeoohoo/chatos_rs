...
use crate::builtin::web_tools::provider::{ResearchExtractExecution, SearchOutcome};

pub(crate) fn build_empty_research_summary(page_success: Option<bool>) -> Value {
...
pub(crate) fn apply_research_execution_summary(
    summary: &mut Value,
...
pub(crate) fn set_research_summary_warning(summary: &mut Value, warning: Option<&str>) {
    let Some(map) = summary.as_object_mut() else {
        return;
    };
    map.insert(
