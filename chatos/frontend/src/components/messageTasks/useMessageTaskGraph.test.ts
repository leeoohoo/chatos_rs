...
describe('useMessageTaskGraph', () => {
  ...
  it('sends the user-selected execution service when retrying a multi-application task', async () => {
    const failedTask: MessageTaskRunnerTask = {
      id: 'task-multi-app',
      title: '多应用执行任务',
      status: 'failed',
    ...
    const request = vi.fn((path: string, init?: RequestInit) => {
      if (path.includes('/retry')) {
        expect(init).toEqual({
          method: 'POST',
          body: JSON.stringify({ execution_service_id: 'mdm-service' }),
        });
        return Promise.resolve({
          success: true,
          run: { id: 'run-retry', task_id: failedTask.id, status: 'queued' },
        });
      }
      graphReads += 1;
      return Promise.resolve({
        root_task_ids: [failedTask.id],
        nodes: [{
          task: {
            ...failedTask,
            status: graphReads === 1 ? 'failed' : 'queued',
            last_run_id: graphReads === 1 ? 'run-multi-app' : 'run-retry',
          },
          depth: 0,
          is_root: true,
          is_current_message: true,
    ...
    await waitFor(() => expect(result.current.allTasks[0]?.status).toBe('failed'));
    await act(async () => {
      await result.current.retryTask(failedTask, undefined, 'mdm-service');
    });
