...
#[derive(Debug, Deserialize)]
struct ProjectContactsQuery {
    limit: Option<i64>,
    offset: Option<i64>,
}

#[derive(Debug, Deserialize)]
struct AddProjectContactRequest {
...
async fn remove_project_contact(
    auth: AuthUser,
    ...
    let linked_rows =
        match memory_server_client::list_project_contacts(id.as_str(), Some(500), 0).await {
            Ok(items) => items,
            Err(err) => {
                return (
                    StatusCode::INTERNAL_SERVER_ERROR,
                    Json(
                        serde_json::json!({"error": "load project contacts failed", "detail": err}),
                    ),
                )
            }
        };

    let Some(linked) = linked_rows.into_iter().find(|item| item.contact_id == contact_id) else {
        return (
            StatusCode::NOT_FOUND,
