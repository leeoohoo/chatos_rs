...
use chatos_local_agent_runtime::{
    ...
    reduce_and_commit, renew_event_claim, request_run_control, retry_local_agent_task,
    ...
    DurableProviderContextCommit, DurableScheduler, DurableTaskState, ExecutedModelStep,
    ...
    RenewEventClaimRequest, RequestRunControl, RetriedLocalAgentTask, RetryLocalAgentTaskRequest,
...
impl LocalAgentHost {
    ...
    pub async fn retry_task(
        ...
        now: DateTime<Utc>,
    ) -> Result<RetriedLocalAgentTask, LocalAgentHostError> {
        command.validate()?;
        self.profiles.require("task_runner")?;
        let new_run_id = stable_host_id(
            "task-run-retry",
            &[
                self.scope.owner_user_id.as_str(),
        ...
        let initial_message_id = stable_host_id(
            "task-retry-message",
            &[self.scope.owner_user_id.as_str(), new_run_id.as_str()],
        );
        let retried = retry_local_agent_task(
            self.storage.as_ref(),
            RetryLocalAgentTaskRequest {
                scope: self.scope.clone(),
                task_id: command.task_id,
                expected_run_id: command.expected_run_id,
                new_run_id,
                initial_message_id,
                retry_instruction: command.instruction,
                origin_device_id: self.device_id.clone(),
                causation_id: request_id.to_string(),
                now,
            },
        ...
        .await?;
        self.scheduler.lock().await.schedule(&retried.start_event);
        self.scheduler_wake.notify_one();
        Ok(retried)
    }
    ...
    async fn existing_local_task_creation_outcome(
        ...
        let state = DurableTaskState::from_record(&task)
            .map_err(|error| format!("existing local Task state is invalid: {error}"))?;
        if state.source_thread_id != request.source_thread_id
            || state.source_turn_id != request.source_turn_id
        ...
        let run = self
            .load_run_record(&state.current_run_id)
            .await
        ...
        state
            .validate_run_identity(&task.metadata.id, &run.run)
            .map_err(|error| format!("existing Task Runner run is invalid: {error}"))?;
    ...
    async fn execute_mutation(
        ...
        match command {
            ...
            LocalAgentCommand::RetryTask(command) => {
                let retried = self
                    .host
                    .retry_task(request_id, command, Utc::now())
                    .await
                    .map_err(run_creation_ipc_error)?;
                Ok(LocalAgentIpcResponse::RunCreated {
                    operation_id: retried.start_event.event.event_id,
                    run: Box::new(retried.run_record.run),
                })
            }
