...
impl SummaryJobDefaults {
    pub fn from_env() -> Self {
        ...
        let token_limit = std::env::var("SUB_AGENT_SUMMARY_JOB_TOKEN_LIMIT")
            ...
            .unwrap_or(6000)
            .max(MIN_TOKEN_LIMIT);
        let round_limit = std::env::var("SUB_AGENT_SUMMARY_JOB_ROUND_LIMIT")
            .ok()
            .and_then(|value| value.parse::<i64>().ok())
            .unwrap_or(8)
            .max(MIN_ROUND_LIMIT);
        let target_summary_tokens = std::env::var("SUB_AGENT_SUMMARY_JOB_TARGET_TOKENS")
            .ok()
            .and_then(|value| value.parse::<i64>().ok())
            .unwrap_or(700)
            .max(MIN_TARGET_SUMMARY_TOKENS);
        let job_interval_seconds = std::env::var("SUB_AGENT_SUMMARY_JOB_INTERVAL_SECONDS")
            .ok()
            .and_then(|value| value.parse::<i64>().ok())
            .unwrap_or(30)
            .max(MIN_JOB_INTERVAL_SECONDS);
        let keep_last_n_messages = std::env::var("SUB_AGENT_SUMMARY_JOB_KEEP_LAST_N_MESSAGES")
            .ok()
            .and_then(|value| value.parse::<usize>().ok())
            .unwrap_or(6);
        let max_runs_per_tick = std::env::var("SUB_AGENT_SUMMARY_JOB_MAX_RUNS_PER_TICK")
            .ok()
            .and_then(|value| value.parse::<i64>().ok())
            .unwrap_or(50)
            .max(1);
