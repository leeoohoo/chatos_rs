...
- 当前在制：阶段 4，依次完成 Windows 人工复核和 Pet 全本地投影。
- 下一切片：把 Windows `needs_review`、非 Ask User paused Run 的原因展示与 Resume/Pause/Cancel 精确控制接入同一账户级 Local Agent Projection，并补齐 Main Chat 与 Task Presentation，不保留远程或推测状态。
