...
- 2026-09-14：完成 macOS Plugin 安装状态退出 `NativeConnectorPersistentState/state.json`。唯一原生协议直接升级到 v25，新增 owner + device 隔离的安装记录查询、CAS 写入与删除命令；Local Agent Host 使用共享 `PluginStateRepository` 保存 Plugin Release、版本、安装位置、摘要和启用状态，并以 `record_kind = installed_plugin` 与项目级 Capability 严格隔离。插件列表、启动、安装、卸载、启停、Browser CDP 配对及 Plugin/Skill Relay 只读取该权威 Store；删除旧插件身份自动迁移，不读取、不迁移、不 fallback、不双写旧 JSON。Rust Protocol/Host 全量、Swift v25 Fixture 与 Plugin Store 定向测试、Client Storage/Agent Tool Plane 边界检查通过；当前 macOS 环境未安装 `dotnet`，Windows v25 Fixture 测试仍须在 Windows/.NET 8 验收中执行。
- 2026-09-14：完成 macOS Local Connector 最后一份业务 JSON 状态删除。账户、设备、Workspace 和网关启停状态统一进入 owner-scoped `local_connector.pairing` Client Setting，随当前 SQLite/PostgreSQL Provider 保存；启动时只有 Local Agent Host 返回的当前账户记录可以激活，退出与换号会停止旧网关、Plugin 会话并清空进程缓存和路由。Gateway Token 同步改为账户隔离的 Keychain account。物理删除 `NativeConnectorPersistentState`、`NativeConnectorStateStore`、`stateURL` 配置和生产 `state.json` 路径，原支持目录只承载 Plugin 产物；不读取、不迁移、不 fallback、不双写遗留 JSON。Client Storage 审计中对应 legacy 项已删除，静态边界禁止旧类型和路径恢复。
- 2026-09-14：完成 macOS 自动命令审批迁入公共 Rust Runtime。唯一原生协议直接升级到 v26，新增严格有界的 `create_approval_review`；客户端只传 `model_config_id`、冻结操作描述和风险信息，API Key、Base URL 与任意 Provider 参数不进入 IPC、Run 或日志。`approval_review` Profile 只暴露一个终态 `approval_decision`，普通文字、多个调用、其他工具或无效结果均不能批准；`deny/ask_user` 强制清除 remember allow。Host 使用同一 Model Gateway、Durable Run、Provider Native/Memory Engine Context 与错误恢复边界；Memory Engine Strategy 只读取已同步语义消息，不重复注入审批文本。macOS 只观察 Host Run 终态，失败、暂停、取消、超时统一转人工；旧 Swift `AgentRuntime().run`、审批模型直连、`include_secret=true` 和审批只读工具实现已物理删除。协议 fixture 从 v25 直接替换为 v26，不保留旧版本。Rust Protocol/Profile/Host 全量、macOS 全量 275 项、Storage/Agent Tool Plane 边界检查通过；当前机器没有 `.NET 8`，Windows v26 类型与 fixture 已同步但仍需目标环境执行。
- 2026-09-14：阶段 6 删除前置切片完成。账户级 Agent 运行偏好、模型调用预算和上下文压缩预算已从旧 Swift `ChatOSAgentRuntime` 移入 `ChatOSCore`，设置页、Connector、媒体入口与测试只依赖公共数据契约；旧 Runtime 仅在剧情迁移期间反向依赖该契约，不再拥有跨业务配置类型。该切片没有把模型请求、Memory Context、Checkpoint、工具循环或重试 Loop 移入 Core，也没有形成兼容或双轨；对应类型测试已归属 `ChatOSCoreTests`。
- 2026-09-14：完成 macOS 剧情设计 Profile 的公共 Rust Runtime 接管。唯一协议升级到 v28，Story Studio 只创建、观察、暂停、恢复、取消并应用 Rust Host 持久化的 `story_design` Run；冻结的 `project_id`、Story Record、基础 revision、摘要、阶段和目标集合均由 Host 校验，Swift 不再执行模型请求、Memory Context、Checkpoint、Schema 校验、Tool Loop 或重试循环。物理删除整个 Swift `ChatOSAgentRuntime` product/source/tests、旧 Story Agent Session/Tools 及直连 Memory Engine service，不保留兼容适配器、fallback 或双轨。共享 v28 fixture、Rust Workspace、macOS 全量 278 项、Client Storage/Agent Tool Plane/签名边界检查通过；同时修复终端进程退出状态早于 stdout/stderr 尾部入库的竞态，确保 `process_wait` 返回完整输出。
- 当前在制：完成 Main Chat 创建与 Task 创建的真实签名 App 冒烟；随后拆除 macOS Gateway/Pairing 云端链路，把仍需保留的本机文件、Git、终端、插件和远程连接能力收归客户端 capability，再物理删除服务端 Local Connector、Remote Execution API、`chatos_sandbox_contract` 及其部署配置。阶段 4 的 Windows Host 打包和原生 runner 验收保持未完成，不得标记完成。
- 2026-09-14：补齐旧 MCP 执行面物理清理。删除顶层 `mcp/`、`chatos_mcp_runtime`、`chatos_mcp_service`、`chatos_local_workspace`、此前无消费者的 `chatos_remote_runtime` 与 `chatos_terminal_runtime`，同时移除 workspace 成员、依赖、缓存监控、告警和尺寸白名单。Plugin Management 只保留自身声明式系统工具目录、Provider 使用说明与其拥有的两组静态 schema；本地或插件工具统一标记为运行时动态发现，不再通过服务端执行实现生成目录。旧 `builtin` runtime kind、`builtin_kind` 字段和 Task Manager 兼容过滤也已删除，不迁移、不 fallback。架构边界检查新增目录永久缺席约束，全仓 Rust 构建通过。
- 旧 Swift `ChatOSAgentRuntime` 与 Memory Engine 已完成符号级能力对照。单次模型请求由 `memory_engine/backend/src/ai/client` 承担，Responses/Chat Completions 请求与 SSE 解析由 `ai/client/request.rs` 和 `ai/parsing.rs` 承担，摘要分块/多轮合并由 `services/ai_pipeline` 承担，上下文溢出缩减由 `ai_pipeline/overflow.rs` 与 `summary_pipeline.rs` 承担，瞬时错误分类和有界退避由 `ai/retry.rs` 与 Memory AI Job 承担；这些能力均已有 Memory Engine 自有实现，因此不复制 Swift 实现。旧 Runtime 剩余的 Tool Call 解析、Schema 校验、Memory Context 拼装、Checkpoint、重试循环和 `AgentRuntime.run` 全部属于客户端 Agent 执行面，必须由公共 Rust Runtime/Profile 接管，不得迁入 Memory Engine。完成公共 Rust Profile 接管审批与剧情后，旧 Runtime 目录和测试必须物理删除，不保留兼容适配器。
