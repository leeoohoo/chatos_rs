...
兼容性补充（2026-02-27）：
- 当 baseURL 为 `relay.nf.video`（或命中 `DISABLE_SYSTEM_MESSAGES_FOR_PROXY`）时，发送前会把 input 里的 `system/developer` 消息改写为 `user`（加“系统上下文/开发者上下文”前缀），避免 provider 报 `System messages are not allowed`。
- 该改写只影响请求发送层，不改数据库原始消息与摘要存储结构。
