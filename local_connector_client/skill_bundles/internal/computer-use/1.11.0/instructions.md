# ChatOS Computer Use — macOS and Windows Observation, Approved Control, and Post-Action Recovery

Use this Skill only on the current user's interactive desktop. Observe first; every input action is narrow, bounded, requires explicit local user approval, produces a privacy-preserving structured audit summary, and then attempts one transient post-action screenshot. macOS requires Accessibility and Screen Recording permission. Windows uses the current user's desktop and remains subject to foreground, protected-content, UAC integrity, UI Automation provider, and system-policy restrictions.
...
- `computer_press_key`: press one reviewed navigation key, optionally with reviewed modifiers. On Windows, `command` maps to the Windows key and `option` maps to Alt. Arbitrary letter key codes are not supported. Generated key-up recovery is armed before or paired with the corresponding key-down sequence.
...
- After a control action returns success, the Local Connector waits for a short bounded UI settle interval and attempts one screenshot. Click and drag recapture only the approved display identity; key, text, scroll, and activation recapture the current main display.
- The screenshot is delivered only through transient model input and is limited to 2 MiB so its base64 envelope stays inside the Plugin relay and model-input limits. Structured results retain capture scope, display identity, MIME type, byte count, and SHA-256, but never image pixels or base64.
- If the display identity changes, capture permission disappears, capture times out, the session is cancelled after the input completed, or any other observation error occurs, the action result remains `success: true` and records `action_already_executed: true`, `automatic_replay_safe: false`, and a bounded failure reason.
- Never repeat a click, drag, key, text, scroll, or activation merely because the post-action screenshot was unavailable. Observe again, inspect the current state, and decide from the new evidence.
- The recovery contract prevents generated mouse/key state from remaining latched where the platform API permits recovery. It does not roll back application state, undo navigation, remove typed text, reverse a drag/drop, or make an action idempotent.
- Before any ref-based or target-dependent next action, refresh the Accessibility/UI Automation tree because controls, focus, and layout may have changed even when a visual screenshot was attached.
...
- Audit cards state the applicable identity guard, privacy rule, cancellation boundary, post-action observation behavior, or input-release guarantee. They are descriptive evidence, not a substitute for live target revalidation.
...
8. Text for `computer_type_text` is shown in the transient local approval request. Approval history, action audit, and structured tool results persist only character counts and SHA-256, never the text itself. A transient post-action screenshot may visibly contain the updated control and must remain non-persistent.
...
11. Cancelling the Task/Plugin session revokes waiting approvals, marks any running native action cancelled, and prevents queued actions from executing later. A running drag must release the mouse before returning; double-click cancellation is observed only between complete down/up pairs. Cancellation after an action completed may skip its screenshot but must not convert the completed action into a replayable failure.
...
This release adds best-effort transient post-action screenshots, explicit no-automatic-replay metadata when observation fails, and paired mouse/key release recovery across approved actions. A separate signed helper, non-Edit contenteditable text entry, application-state rollback, and a dedicated high-impact confirmation layer remain unavailable.
