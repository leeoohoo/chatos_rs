...
use crate::attachments::persist_message_attachments;
...
use crate::task_state::{sync_task_from_run, DurableTaskState};
use crate::ui_events::{
...
#[derive(Debug, Clone)]
pub struct RetryLocalAgentTaskRequest {
    pub scope: RecordScope,
    pub task_id: String,
...
#[derive(Debug, Clone, PartialEq)]
pub struct RetriedLocalAgentTask {
    pub task_record: TaskRecord,
    pub run_record: AgentRunStateRecord,
    pub start_event: AgentEventStateRecord,
}
...
pub async fn retry_local_agent_task(
    storage: &dyn ClientStorage,
    request: RetryLocalAgentTaskRequest,
) -> StorageResult<RetriedLocalAgentTask> {
    validate_retry_task_request(&request)?;
    let mut operation = RetryLocalAgentTaskOperation {
        request: Some(request),
        result: None,
    };
    storage.transaction(&mut operation).await?;
    operation.result.ok_or(StorageError::Transaction {
        reason: "local Agent Task retry returned no result".to_string(),
    })
}

struct RetryLocalAgentTaskOperation {
    request: Option<RetryLocalAgentTaskRequest>,
    result: Option<RetriedLocalAgentTask>,
}

#[async_trait]
impl StorageTransaction for RetryLocalAgentTaskOperation {
    async fn execute(
        &mut self,
        repositories: &mut dyn TransactionRepositories,
    ) -> StorageResult<()> {
        let request = self.request.take().ok_or(StorageError::Transaction {
            reason: "local Agent Task retry request was already consumed".to_string(),
        })?;
        let query = RecordQuery {
            scope: request.scope.clone(),
            id: request.task_id.clone(),
        };
        let mut task = repositories
            .tasks()
            .get(&query)
            .await?
            .ok_or(StorageError::NotFound)?;
        let mut state = DurableTaskState::from_record(&task)?;
        let current_run_id = state.current_run_id.clone();
        if current_run_id == request.new_run_id {
            self.result =
                Some(load_idempotent_task_retry(repositories, task, state, &request).await?);
            return Ok(());
        }
        if current_run_id != request.expected_run_id {
        ...
        let current_run = repositories
            ...
            .get(&RecordQuery {
                scope: request.scope.clone(),
                id: current_run_id.clone(),
            })
            .await?
            .ok_or(StorageError::NotFound)?;
        state.validate_run_identity(&task.metadata.id, &current_run.run)?;
        if !matches!(
            current_run.run.status,
            LocalAgentRunStatus::Failed | LocalAgentRunStatus::Cancelled
        ) {
            return Err(StorageError::InvalidData {
                reason: "only a failed or cancelled Task Run can be retried".to_string(),
            });
        }
        ...
        let retry_instruction = request
            ...
            .map(str::trim)
            .map(str::to_string);
        let initial_content = retry_instruction
            .clone()
        ...
        let created = create_run_in_transaction(
            repositories,
            CreateLocalAgentRunRequest {
                scope: request.scope.clone(),
                run_id: request.new_run_id.clone(),
                profile_key: current_run.run.profile_key.clone(),
                owner_entity_type: current_run.run.owner_entity_type.clone(),
                owner_entity_id: current_run.run.owner_entity_id.clone(),
                project_id: Some(project_id.clone()),
                model_runtime_snapshot: current_run.run.model_runtime_snapshot.clone(),
                prompt_revision: current_run.run.prompt_revision.clone(),
                capability_snapshot_ref: current_run.run.capability_snapshot_ref.clone(),
                ...
                initial_message: Some(InitialRunMessage {
                    record_id: request.initial_message_id.clone(),
                    turn_id: request.new_run_id.clone(),
                    ...
                    message_source: "task_retry".to_string(),
                }),
        ...
        let expected_revision = task.metadata.revision;
        task.status = "queued".to_string();
        task.state = state.to_value()?;
        task.metadata.updated_at = request.now;
        let task_record = repositories
            ...
            .await?;
        self.result = Some(RetriedLocalAgentTask {
            task_record,
            run_record: created.run_record,
            start_event: created.start_event,
        });
        Ok(())
    }
}
...
async fn create_run_in_transaction(
    ...
    let initial_message_record_id = request
        .initial_message
        .as_ref()
    ...
    if let Some(existing_run) = existing_run {
        ...
        .await?;
        if let Some(message_record_id) = initial_message_record_id.as_deref() {
            persist_message_attachments(
                repositories,
                &existing_run.metadata.scope,
                &existing_run.run.run_id,
                &existing_run.run.owner_entity_id,
                existing_run.run.project_id.as_deref(),
                message_record_id,
    ...
    if let Some(message_record_id) = initial_message_record_id.as_deref() {
        persist_message_attachments(
            ...
            run_record.run.project_id.as_deref(),
            message_record_id,
            &request.initial_attachments,
            &run_record.metadata.origin_device_id,
            request.now,
...
fn requested_task_record(request: &CreateLocalAgentTaskRequest) -> StorageResult<TaskRecord> {
    let state = DurableTaskState::new(
        request.run.run_id.clone(),
        request.source_thread_id.clone(),
        request.source_turn_id.clone(),
        request.project_id.clone(),
        request.objective.clone(),
        request.acceptance_criteria.clone(),
        request.run.model_runtime_snapshot.model_config_id.clone(),
        request.run.model_runtime_snapshot.revision,
        request.prompt_snapshot.clone(),
        request.project_snapshot.clone(),
...
fn task_identity_matches(existing: &TaskRecord, requested: &TaskRecord) -> bool {
    let Ok(existing_state) = DurableTaskState::from_record(existing) else {
        return false;
    };
    let Ok(requested_state) = DurableTaskState::from_record(requested) else {
        return false;
    };
    existing.metadata.id == requested.metadata.id
        ...
        && existing_state.same_creation_identity(&requested_state)
}

fn validate_retry_task_request(request: &RetryLocalAgentTaskRequest) -> StorageResult<()> {
    for value in [
        request.scope.owner_user_id.as_str(),
        request.task_id.as_str(),
        request.expected_run_id.as_str(),
        request.new_run_id.as_str(),
        request.initial_message_id.as_str(),
        request.origin_device_id.as_str(),
        request.causation_id.as_str(),
    ] {
        if value.trim().is_empty() {
            return Err(StorageError::InvalidData {
                reason: "local Agent Task retry identifiers must not be empty".to_string(),
            });
        }
    }
    if request.expected_run_id == request.new_run_id {
        return Err(StorageError::InvalidData {
            reason: "local Agent Task retry must create a new Run".to_string(),
        });
    }
    if request
        .retry_instruction
    ...
    {
        return Err(StorageError::InvalidData {
            reason: "local Agent Task retry instruction must not be empty".to_string(),
        });
    }
    Ok(())
}

async fn load_idempotent_task_retry(
    repositories: &mut dyn TransactionRepositories,
    task: TaskRecord,
    ...
    let run_record = repositories
        ...
        .await?
        .ok_or(StorageError::NotFound)?;
    state.validate_run_identity(&task.metadata.id, &run_record.run)?;
    if state.current_retry_instruction.as_deref()
        != request.retry_instruction.as_deref().map(str::trim)
    {
        return Err(StorageError::Conflict {
            actual_revision: task.metadata.revision,
        });
    ...
    let start_event = repositories
        ...
        .await?
        .ok_or_else(|| StorageError::InvalidData {
            reason: "retried local Agent Run has no durable start event".to_string(),
        })?;
    Ok(RetriedLocalAgentTask {
        task_record: task,
        run_record,
        start_event,
    })
