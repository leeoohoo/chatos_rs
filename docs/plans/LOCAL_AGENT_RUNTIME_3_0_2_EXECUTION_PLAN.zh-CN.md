...
- 下一切片：迁移 macOS 剪贴板历史，使二进制内容继续留在客户端受控文件边界、元数据和索引只通过共享 Host 的 `ClipboardRepository` 落入当前 Provider；完成后删除 `ClipboardHistoryStore.swift` 的 SQLite 驱动，不兼容旧库、不 fallback、不双写。
