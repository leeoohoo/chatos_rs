...
use crate::db::Db;
use crate::models::{
    SummaryRollupJobConfig, UpsertSummaryRollupJobConfigRequest,
    default_summary_rollup_prompt_template,
};

use super::super::super::{auth::ADMIN_USER_ID, now_rfc3339};
use super::shared::summary_rollup_collection;
...
fn default_summary_rollup_job_config(user_id: &str) -> SummaryRollupJobConfig {
    SummaryRollupJobConfig {
        ...
        summary_prompt: Some(default_summary_rollup_prompt_template().to_string()),
        token_limit: 6000,
        round_limit: 50,
...
pub async fn upsert_summary_rollup_job_config(
    ...
    if current.summary_prompt.is_none() {
        current.summary_prompt = Some(default_summary_rollup_prompt_template().to_string());
    }
