...
尚未具备云端等价安全设施的配置继续 fail closed：OAuth 浏览器授权、refresh token 自动续期、仅声明 package-relative cwd 但仍使用 PATH command 的混合模式、ConfigFile transport 均不会退化为明文、本地 OAuth Broker 或 MCP Management 本地执行。当前 Cloud OAuth 只接受仍在有效期内的受保护 Bearer access token；过期后必须重新授权/更新。`PluginCloud` 当前仍声明 `cancel_supported=false`。
