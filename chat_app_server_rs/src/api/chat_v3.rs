...
pub fn router() -> Router {
    Router::new()
        ...
        .route("/api/agent_v3/session/:session_id/reset", post(reset_session))
}
...
async fn agent_chat_stream(Json(req): Json<ChatRequest>) -> Result<axum::response::Sse<impl futures::Stream<Item = Result<axum::response::sse::Event, std::convert::Infallible>>>, (StatusCode, Json<Value>)> {
    ...
    if req.ai_model_config.as_ref().and_then(|cfg| cfg.get("supports_responses").and_then(|v| v.as_bool())) != Some(true) {
        return Err((StatusCode::BAD_REQUEST, Json(json!({"error": "当前模型未启用 Responses API"}))));
    }
...
async fn stream_chat_v3(sender: SseSender, req: ChatRequest) {
    ...
    if model_cfg.get("supports_responses").and_then(|v| v.as_bool()) != Some(true) {
        sender.send_json(&json!({ "type": Events::ERROR, "timestamp": chrono::Utc::now().to_rfc3339(), "data": { "error": "当前模型未启用 Responses API" } }));
        return;
    }

    let model = model_cfg.get("model_name").and_then(|v| v.as_str()).unwrap_or("gpt-4o").to_string();
