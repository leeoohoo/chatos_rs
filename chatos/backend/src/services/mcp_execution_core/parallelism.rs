...
#[derive(Clone, Debug, PartialEq, Eq)]
pub(crate) struct ToolAccessProfile {
    pub(crate) kind: ToolAccessKind,
    pub(crate) scope: ToolScope,
}

pub(crate) fn should_parallelize_tool_batch(
    tool_calls: &[Value],
    ...
    for tool_call in tool_calls {
        let Some(prefixed_name) = super::tool_call_name(tool_call) else {
            return false;
        };
        ...
        let Ok(args) = super::parse_tool_args(args_val) else {
            return false;
        };
...
pub(crate) fn has_conflicting_tool_profiles(profiles: &[ToolAccessProfile]) -> bool {
