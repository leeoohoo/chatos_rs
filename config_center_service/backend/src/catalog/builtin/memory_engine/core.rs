...
pub(super) fn definitions(now: &str) -> Vec<ConfigDefinitionRecord> {
    vec![
        ...
        secret_definition(
            ...
            "Memory Engine 调用 User Service 读取用户运行时模型配置时使用的共享密钥",
            "Memory Engine / Security",
            "service",
            Some("memory-engine"),
        ...
        definition(
            ...
            "User Service Public Base URL",
            "Memory Engine 验证用户登录态时调用 User Service 公共接口使用的基础地址",
        ...
        definition(
            ...
            "Memory Engine 通过 mTLS 调用 User Service 内部模型运行时接口使用的基础地址",
            "Memory Engine / Downstream",
            "service",
            Some("memory-engine"),
            "string",
