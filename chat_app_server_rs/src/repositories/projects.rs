    ...
    with_db(
        |db| {
            ...
            Box::pin(async move {
                ...
                let mut cursor = db.collection::<Document>("projects").find(filter, None).await.map_err(|e| e.to_string())?;
                let mut docs = Vec::new();
                while let Some(doc) = cursor.try_next().await.map_err(|e| e.to_string())? {
                    docs.push(doc);
                }
                let mut items: Vec<Project> = docs.into_iter().filter_map(|d| normalize_doc(&d)).collect();
...
pub async fn get_project_by_id(id: &str) -> Result<Option<Project>, String> {
    with_db(
        |db| {
            let id = id.to_string();
            Box::pin(async move {
                let doc = db.collection::<Document>("projects").find_one(doc! { "id": id }, None).await.map_err(|e| e.to_string())?;
...
pub async fn update_project(id: &str, name: Option<String>, root_path: Option<String>, description: Option<String>) -> Result<(), String> {
    let now = chrono::Utc::now().to_rfc3339();
    let now_mongo = now.clone();
    let now_sqlite = now.clone();
    ...
    with_db(
        |db| {
            ...
            Box::pin(async move {
                ...
                if let Some(v) = name_mongo { set_doc.insert("name", v); }
                if let Some(v) = root_mongo { set_doc.insert("root_path", v); }
                if let Some(v) = desc_mongo { set_doc.insert("description", v); }
        ...
        |pool| {
            ...
            Box::pin(async move {
                ...
                if let Some(v) = name_sqlite { fields.push("name = ?"); binds.push(v); }
                if let Some(v) = root_sqlite { fields.push("root_path = ?"); binds.push(v); }
                if let Some(v) = desc_sqlite { fields.push("description = ?"); binds.push(v); }
...
pub async fn delete_project(id: &str) -> Result<(), String> {
    with_db(
        |db| {
            let id = id.to_string();
            Box::pin(async move {
                db.collection::<Document>("projects").delete_one(doc! { "id": &id }, None).await.map_err(|e| e.to_string())?;
                Ok(())
            })
        },
        |pool| {
            let id = id.to_string();
            Box::pin(async move {
                sqlx::query("DELETE FROM projects WHERE id = ?").bind(&id).execute(pool).await.map_err(|e| e.to_string())?;
                Ok(())
            })
        }
    ).await
