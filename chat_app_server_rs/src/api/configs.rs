use axum::{Router, Json, routing::{get, post, put}, extract::{Path, Query}};
...
use crate::services::builtin_mcp::{builtin_display_name, get_builtin_mcp_config, is_builtin_mcp_id, list_builtin_mcp_configs};
...
pub fn router() -> Router {
    Router::new()
        .route("/api/mcp-configs", get(list_mcp_configs).post(create_mcp_config))
        .route("/api/mcp-configs/:config_id", put(update_mcp_config).delete(delete_mcp_config))
        .route("/api/mcp-configs/:config_id/resource/config", get(get_mcp_resource_config))
        .route("/api/mcp-configs/resource/config", post(post_mcp_resource_config))
        .route("/api/ai-model-configs", get(list_ai_model_configs).post(create_ai_model_config))
        .route("/api/ai-model-configs/:config_id", put(update_ai_model_config).delete(delete_ai_model_config))
        .route("/api/system-contexts", get(list_system_contexts).post(create_system_context))
        .route("/api/system-contexts/:context_id", put(update_system_context).delete(delete_system_context))
        .route("/api/system-contexts/:context_id/activate", post(activate_system_context))
...
async fn list_mcp_configs(Query(query): Query<UserQuery>) -> (StatusCode, Json<Value>) {
    let configs = match mcp_repo::list_mcp_configs(query.user_id).await {
        Ok(list) => list,
        Err(err) => return (StatusCode::INTERNAL_SERVER_ERROR, Json(json!({"error": "获取MCP配置失败", "detail": err}))),
    };
    let mut out = Vec::new();
...
async fn create_mcp_config(Json(req): Json<McpConfigRequest>) -> (StatusCode, Json<Value>) {
    let Some(name) = req.name else {
        return (StatusCode::INTERNAL_SERVER_ERROR, Json(json!({"error": "创建MCP配置失败"})));
    };
    ...
    if let Err(err) = mcp_repo::create_mcp_config(&cfg).await {
        return (StatusCode::INTERNAL_SERVER_ERROR, Json(json!({"error": "创建MCP配置失败", "detail": err})));
    }
    ...
    let saved_cfg = match mcp_repo::get_mcp_config_by_id(&id).await {
        Ok(Some(cfg)) => cfg,
        Ok(None) => return (StatusCode::INTERNAL_SERVER_ERROR, Json(json!({"error": "创建MCP配置失败"}))),
        Err(err) => return (StatusCode::INTERNAL_SERVER_ERROR, Json(json!({"error": "创建MCP配置失败", "detail": err}))),
    };
...
async fn update_mcp_config(Path(config_id): Path<String>, Json(req): Json<McpConfigRequest>) -> (StatusCode, Json<Value>) {
    if is_builtin_mcp_id(&config_id) {
        return (StatusCode::FORBIDDEN, Json(json!({"error": "内置 MCP 配置不可编辑"})));
    }
    let existing = match mcp_repo::get_mcp_config_by_id(&config_id).await {
        Ok(cfg) => cfg,
        Err(err) => return (StatusCode::INTERNAL_SERVER_ERROR, Json(json!({"error": "更新MCP配置失败", "detail": err}))),
    };
    if existing.is_none() {
        return (StatusCode::NOT_FOUND, Json(json!({"error": "MCP配置不存在"})));
    }
    let mut cfg = existing.unwrap();
    let mut update_requested = false;
    if let Some(v) = req.name { cfg.name = v; update_requested = true; }
    if let Some(v) = req.command { cfg.command = v; update_requested = true; }
    if let Some(v) = req.r#type { cfg.r#type = v; update_requested = true; }
    if let Some(v) = req.args { cfg.args = Some(v); update_requested = true; }
    if let Some(v) = req.env { cfg.env = Some(v); update_requested = true; }
    if let Some(v) = req.cwd { cfg.cwd = Some(v); update_requested = true; }
    if let Some(v) = req.enabled { cfg.enabled = v; update_requested = true; }
    if update_requested {
        if let Err(err) = mcp_repo::update_mcp_config(&config_id, &cfg).await {
            return (StatusCode::INTERNAL_SERVER_ERROR, Json(json!({"error": "更新MCP配置失败", "detail": err})));
        }
    }
    ...
    let cfg = match mcp_repo::get_mcp_config_by_id(&config_id).await {
        Ok(Some(cfg)) => cfg,
        Ok(None) => return (StatusCode::INTERNAL_SERVER_ERROR, Json(json!({"error": "更新MCP配置失败"}))),
        Err(err) => return (StatusCode::INTERNAL_SERVER_ERROR, Json(json!({"error": "更新MCP配置失败", "detail": err}))),
    };
...
async fn delete_mcp_config(Path(config_id): Path<String>) -> (StatusCode, Json<Value>) {
    if is_builtin_mcp_id(&config_id) {
        return (StatusCode::FORBIDDEN, Json(json!({"error": "内置 MCP 配置不可删除"})));
    }
    let existing = match mcp_repo::get_mcp_config_by_id(&config_id).await {
        Ok(cfg) => cfg,
        Err(err) => return (StatusCode::INTERNAL_SERVER_ERROR, Json(json!({"error": "删除MCP配置失败", "detail": err}))),
    };
    if existing.is_none() {
        return (StatusCode::NOT_FOUND, Json(json!({"error": "MCP配置不存在"})));
    }
    if let Err(err) = mcp_repo::delete_mcp_config(&config_id).await {
        return (StatusCode::INTERNAL_SERVER_ERROR, Json(json!({"error": "删除MCP配置失败", "detail": err})));
    }
    (StatusCode::OK, Json(json!({ "message": "MCP配置删除成功", "id": config_id })))
}

async fn get_mcp_resource_config(Path(config_id): Path<String>) -> (StatusCode, Json<Value>) {
    if is_builtin_mcp_id(&config_id) {
        return (StatusCode::BAD_REQUEST, Json(json!({"error": "内置 MCP 不支持资源配置读取"})));
    }
    let cfg = if is_builtin_mcp_id(&config_id) {
        get_builtin_mcp_config(&config_id)
    } else {
        mcp_repo::get_mcp_config_by_id(&config_id).await.unwrap_or(None)
    };
    let cfg = match cfg {
        Some(c) => c,
        None => return (StatusCode::NOT_FOUND, Json(json!({"error": "MCP配置不存在"}))),
    };
    if cfg.r#type != "stdio" {
        return (StatusCode::BAD_REQUEST, Json(json!({"error": "仅支持stdio类型的MCP配置读取资源"})));
    }
    if cfg.command.is_empty() {
        return (StatusCode::BAD_REQUEST, Json(json!({"error": "MCP配置缺少可执行命令"})));
    }
    ...
    match read_mcp_resource_config(&cfg.command, &args, &env, cwd.as_deref()).await {
        Ok(text) => {
            ...
            (StatusCode::OK, Json(json!({ "success": true, "config": data, "alias": cfg.name })))
        }
        Err(err) => (StatusCode::INTERNAL_SERVER_ERROR, Json(json!({ "error": format!("读取MCP配置资源失败: {}", err) }))),
    }
}

async fn post_mcp_resource_config(Json(req): Json<ResourceByCommandRequest>) -> (StatusCode, Json<Value>) {
    if req.r#type.as_deref() != Some("stdio") {
        return (StatusCode::BAD_REQUEST, Json(json!({"error": "仅支持stdio类型的MCP配置读取资源"})));
    }
    let command = match req.command {
        Some(c) => c,
        None => return (StatusCode::BAD_REQUEST, Json(json!({"error": "缺少可执行命令"}))),
    };
    ...
    match read_mcp_resource_config(&command, &args, &env, req.cwd.as_deref()).await {
        Ok(text) => {
            let data = serde_json::from_str::<Value>(&text).unwrap_or_else(|_| json!({ "raw": text }));
            (StatusCode::OK, Json(json!({ "success": true, "config": data, "alias": alias })))
        }
        Err(err) => (StatusCode::INTERNAL_SERVER_ERROR, Json(json!({ "error": format!("读取MCP配置资源失败: {}", err) }))),
    }
}
...
async fn list_ai_model_configs(Query(query): Query<UserQuery>) -> (StatusCode, Json<Value>) {
    let configs = match ai_repo::list_ai_model_configs(query.user_id).await {
        Ok(list) => list,
        Err(err) => return (StatusCode::INTERNAL_SERVER_ERROR, Json(json!({"error": "获取AI模型配置失败", "detail": err}))),
    };
    let out = configs.into_iter().map(|cfg| {
        json!({
            "id": cfg.id,
            "name": cfg.name,
...
async fn create_ai_model_config(Json(req): Json<AiModelConfigRequest>) -> (StatusCode, Json<Value>) {
    let Some(name) = req.name else {
        return (StatusCode::INTERNAL_SERVER_ERROR, Json(json!({"error": "创建AI模型配置失败"})));
    };
    ...
    if let Err(err) = ai_repo::create_ai_model_config(&cfg).await {
        return (StatusCode::INTERNAL_SERVER_ERROR, Json(json!({"error": "创建AI模型配置失败", "detail": err})));
    }
    (StatusCode::CREATED, Json(ai_model_config_value(&cfg)))
}

async fn update_ai_model_config(Path(config_id): Path<String>, Json(req): Json<AiModelConfigRequest>) -> (StatusCode, Json<Value>) {
    let existing = match ai_repo::get_ai_model_config_by_id(&config_id).await {
        Ok(cfg) => cfg,
        Err(err) => return (StatusCode::INTERNAL_SERVER_ERROR, Json(json!({"error": "更新AI模型配置失败", "detail": err}))),
    };
    if existing.is_none() {
        return (StatusCode::NOT_FOUND, Json(json!({"error": "AI模型配置不存在"})));
    }
    let mut cfg = existing.unwrap();
    if let Some(v) = req.name { cfg.name = v; }
    ...
    if let Some(v) = req.api_key { cfg.api_key = Some(v); }
    if let Some(v) = req.base_url { cfg.base_url = Some(v); }
    if let Some(v) = req.enabled { cfg.enabled = v; }
    if let Some(v) = req.supports_images { cfg.supports_images = v; }
    if let Some(v) = req.supports_reasoning { cfg.supports_reasoning = v; }
    if let Some(v) = req.supports_responses { cfg.supports_responses = v; }
    ...
    if let Err(err) = ai_repo::update_ai_model_config(&config_id, &cfg).await {
        return (StatusCode::INTERNAL_SERVER_ERROR, Json(json!({"error": "更新AI模型配置失败", "detail": err})));
    }
...
async fn delete_ai_model_config(Path(config_id): Path<String>) -> (StatusCode, Json<Value>) {
    let existing = match ai_repo::get_ai_model_config_by_id(&config_id).await {
        Ok(cfg) => cfg,
        Err(err) => return (StatusCode::INTERNAL_SERVER_ERROR, Json(json!({"error": "删除AI模型配置失败", "detail": err}))),
    };
    if existing.is_none() {
        return (StatusCode::NOT_FOUND, Json(json!({"error": "AI模型配置不存在"})));
    }
    if let Err(err) = ai_repo::delete_ai_model_config(&config_id).await {
        return (StatusCode::INTERNAL_SERVER_ERROR, Json(json!({"error": "删除AI模型配置失败", "detail": err})));
    }
    (StatusCode::OK, Json(json!({ "message": "AI模型配置删除成功" })))
}

async fn list_system_contexts(Query(query): Query<UserQuery>) -> (StatusCode, Json<Value>) {
    let user_id = match query.user_id {
        Some(u) => u,
        None => return (StatusCode::BAD_REQUEST, Json(json!({"error": "user_id 为必填参数"}))),
    };
    let contexts = match ctx_repo::list_system_contexts(&user_id).await {
        Ok(list) => list,
        Err(err) => return (StatusCode::INTERNAL_SERVER_ERROR, Json(json!({"error": "获取系统上下文失败", "detail": err}))),
    };
    let mut out = Vec::new();
    for ctx in contexts {
...
async fn get_active_system_context(Query(query): Query<UserQuery>) -> (StatusCode, Json<Value>) {
    ...
    let ctx = match ctx_repo::get_active_system_context(&user_id).await {
        Ok(ctx) => ctx,
        Err(err) => return (StatusCode::INTERNAL_SERVER_ERROR, Json(json!({"error": "获取活跃系统上下文失败", "detail": err}))),
    };
    if let Some(ctx) = ctx {
        let app_ids = match ctx_repo::get_app_ids_for_system_context(&ctx.id).await {
            Ok(ids) => ids,
    ...
    (StatusCode::OK, Json(json!({ "content": "", "context": Value::Null })))
}

async fn create_system_context(Json(req): Json<SystemContextRequest>) -> (StatusCode, Json<Value>) {
    let Some(name) = req.name else {
        return (StatusCode::INTERNAL_SERVER_ERROR, Json(json!({"error": "创建系统上下文失败"})));
    };
    ...
    if let Err(err) = ctx_repo::create_system_context(&ctx).await {
        return (StatusCode::INTERNAL_SERVER_ERROR, Json(json!({"error": "创建系统上下文失败", "detail": err})));
    }
    if let Some(app_ids) = req.app_ids.clone() {
    ...
    let ctx = match ctx_repo::get_system_context_by_id(&id).await {
        Ok(Some(ctx)) => ctx,
        Ok(None) => return (StatusCode::INTERNAL_SERVER_ERROR, Json(json!({"error": "创建系统上下文失败"}))),
        Err(err) => return (StatusCode::INTERNAL_SERVER_ERROR, Json(json!({"error": "创建系统上下文失败", "detail": err}))),
    };
...
async fn update_system_context(Path(context_id): Path<String>, Json(req): Json<SystemContextRequest>) -> (StatusCode, Json<Value>) {
    let existing = match ctx_repo::get_system_context_by_id(&context_id).await {
        Ok(ctx) => ctx,
        Err(err) => return (StatusCode::INTERNAL_SERVER_ERROR, Json(json!({"error": "更新系统上下文失败", "detail": err}))),
    };
    let Some(mut ctx) = existing else {
        return (StatusCode::INTERNAL_SERVER_ERROR, Json(json!({"error": "更新系统上下文失败"})));
    };
    if let Some(v) = req.name { ctx.name = v; }
    if let Some(v) = req.content { ctx.content = Some(v); }
    if let Some(v) = req.is_active { ctx.is_active = v; }
    if let Err(err) = ctx_repo::update_system_context(&context_id, &ctx).await {
        return (StatusCode::INTERNAL_SERVER_ERROR, Json(json!({"error": "更新系统上下文失败", "detail": err})));
    }
    if let Some(app_ids) = req.app_ids {
    ...
    let ctx = match ctx_repo::get_system_context_by_id(&context_id).await {
        Ok(Some(ctx)) => ctx,
        Ok(None) => return (StatusCode::INTERNAL_SERVER_ERROR, Json(json!({"error": "更新系统上下文失败"}))),
        Err(err) => return (StatusCode::INTERNAL_SERVER_ERROR, Json(json!({"error": "更新系统上下文失败", "detail": err}))),
    };
...
async fn delete_system_context(Path(context_id): Path<String>) -> (StatusCode, Json<Value>) {
    if let Err(err) = ctx_repo::delete_system_context(&context_id).await {
        return (StatusCode::INTERNAL_SERVER_ERROR, Json(json!({"error": "删除系统上下文失败", "detail": err})));
    }
    (StatusCode::OK, Json(json!({ "message": "系统上下文删除成功" })))
}
...
async fn activate_system_context(Path(context_id): Path<String>, Json(req): Json<ActivateContextRequest>) -> (StatusCode, Json<Value>) {
    let user_id = match req.user_id {
        Some(u) => u,
        None => return (StatusCode::INTERNAL_SERVER_ERROR, Json(json!({"error": "激活系统上下文失败"}))),
    };
    if let Err(err) = ctx_repo::activate_system_context(&context_id, &user_id).await {
        return (StatusCode::INTERNAL_SERVER_ERROR, Json(json!({"error": "激活系统上下文失败", "detail": err})));
    }
    let list = match ctx_repo::list_system_contexts(&user_id).await {
        Ok(list) => list,
        Err(err) => return (StatusCode::INTERNAL_SERVER_ERROR, Json(json!({"error": "激活系统上下文失败", "detail": err}))),
    };
    let activated = list.into_iter().find(|c| c.id == context_id);
    let out = activated.map(|ctx| system_context_value(&ctx, None)).unwrap_or(Value::Null);
...
async fn read_mcp_resource_config(command: &str, args: &[String], env: &std::collections::HashMap<String, String>, cwd: Option<&str>) -> Result<String, String> {
    use tokio::io::{AsyncBufReadExt, BufReader, AsyncWriteExt};
    use std::process::Stdio;

    let mut cmd = tokio::process::Command::new(command);
    if !args.is_empty() { cmd.args(args); }
    if !env.is_empty() { cmd.envs(env); }
    ...
    if let Some(mut stdin) = child.stdin.take() {
        stdin.write_all((payload.to_string() + "\n").as_bytes()).await.map_err(|e| e.to_string())?;
    }
...
fn normalize_thinking_level_input(provider: &str, level: Option<String>) -> Result<Option<String>, String> {
    let level = level.map(|v| v.trim().to_string()).filter(|v| !v.is_empty());
    if level.is_none() {
        return Ok(None);
    }
