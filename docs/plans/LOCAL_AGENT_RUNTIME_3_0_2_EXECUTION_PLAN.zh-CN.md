...
- 当前在制：迁移 `NativeConnectorPersistentState` 剩余的账户、设备和 Workspace 配对状态后，物理删除 `NativeConnectorPersistentState`、`NativeConnectorStateStore` 与 `state.json`；并完成 Main Chat 创建与 Task 创建的真实签名 App 冒烟。阶段 4 的 Windows Host 打包和原生 runner 验收保持未完成，不得标记完成。
