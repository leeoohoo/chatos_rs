...
pub(super) fn project_from_row(row: &SqliteRow) -> ProjectRecord {
    ProjectRecord {
        ...
        harness_default_branch: row.try_get("harness_default_branch").ok().flatten(),
        harness_provision_status: row.try_get("harness_provision_status").ok().flatten(),
        harness_provision_error: row.try_get("harness_provision_error").ok().flatten(),
        harness_provisioned_at: row.try_get("harness_provisioned_at").ok().flatten(),
