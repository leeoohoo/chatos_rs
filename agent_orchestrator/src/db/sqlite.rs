...
const SQLITE_DB_ENV: &str = "AGENT_ORCHESTRATOR_DB_PATH";
const DEFAULT_SQLITE_DB_FILE: &str = "agent_orchestrator.db";
...
fn resolve_sqlite_db_path(cfg: &SqliteConfig) -> PathBuf {
    if let Ok(value) = std::env::var(SQLITE_DB_ENV) {
    ...
    let configured = cfg
        ...
        .unwrap_or(DEFAULT_SQLITE_DB_FILE);
...
fn migrate_legacy_sqlite_files_if_needed(target: &Path) -> Result<(), String> {
    ...
    let repo_root = Path::new(env!("CARGO_MANIFEST_DIR"))
        .parent()
        .unwrap_or_else(|| Path::new(env!("CARGO_MANIFEST_DIR")));
    let legacy_candidates = discover_runtime_sqlite_candidates(repo_root, target);

    for legacy_db in legacy_candidates {
        if target == legacy_db.as_path() || !legacy_db.exists() {
            continue;
        }
...
fn discover_runtime_sqlite_candidates(repo_root: &Path, target: &Path) -> Vec<PathBuf> {
    let mut out = Vec::new();

    let embedded_data_dir = Path::new(env!("CARGO_MANIFEST_DIR")).join("data");
    let Ok(entries) = std::fs::read_dir(embedded_data_dir) else {
        return discover_local_runtime_sqlite_candidates(repo_root, target, out);
    };
    for entry in entries.flatten() {
        let path = entry.path();
        if path == target || path.extension().and_then(|ext| ext.to_str()) != Some("db") {
            continue;
        }
        out.push(path);
    }

    discover_local_runtime_sqlite_candidates(repo_root, target, out)
}

fn discover_local_runtime_sqlite_candidates(
    repo_root: &Path,
    target: &Path,
    mut out: Vec<PathBuf>,
) -> Vec<PathBuf> {
    let local_root = repo_root.join(".local");
    let Ok(entries) = std::fs::read_dir(local_root) else {
        return out;
    };
    for entry in entries.flatten() {
        let service_root = entry.path();
        if !service_root.join("logs").is_dir() {
            continue;
        }
        let Ok(data_entries) = std::fs::read_dir(service_root.join("data")) else {
            continue;
        };
