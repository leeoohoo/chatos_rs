...
impl TaskRunnerMcpService {
    pub(in crate::mcp_server) async fn call_tool(
        ...
        if !tool_call_allowed_for_identity(
            name,
...
fn tool_call_allowed_for_identity(
    ...
    agent_tool_allowed_for_profile(name, tool_profile)
        || (is_admin && tool_profile == McpToolProfile::Default)
}

#[cfg(test)]
mod tests {
    use super::tool_call_allowed_for_identity;
    use crate::mcp_server::McpToolProfile;
