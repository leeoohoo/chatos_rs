...
- 下一切片：迁移 macOS Media Studio History，使图片/视频文件继续留在客户端受控文件边界，生成记录、项目关联和状态只通过共享 Host 的 `MediaStateRepository` 落入当前 Provider；完成后删除业务 JSON 状态文件，不兼容旧文件、不 fallback、不双写。
