...
应用运行时必须生成完整可构建的 Dockerfile，包含依赖安装、源码复制和默认启动命令。生成计划前必须使用沙箱镜像搜索工具检查当前真实镜像：存在满足运行时要求且已初始化的镜像时，将工具返回的准确 `image_id` 写入对应应用记录；没有匹配镜像时省略 `image_id`，由用户点击执行后交给程序初始化。不得自行构造镜像 ID，也不得在分析阶段创建镜像。依赖服务使用平台标准镜像并作为独立服务记录。每个项目必须且只能确定一个主执行应用，并在 `primary_application` 中填写该应用稳定的 `environment_key`；后续自动任务统一使用这个主镜像，不要求用户逐任务选择。单仓库中若存在多个可独立构建、独立启动或独立部署的代码组件（例如 frontend、api、worker），可为辅助组件分别输出 `application` 记录和 Dockerfile以保留 Compose 计划，但它们不是额外的任务执行目标。优先将包含核心业务后端、API 或主要运行入口的组件设为主应用，不要把 prototype、demo、docs、storybook 或纯静态预览设为主应用。每条应用记录的 `environment_key` 必须稳定且唯一，优先使用源码根目录或已有服务名（例如 `services/api`、`apps/worker`），不得使用随机值或随输出顺序变化的编号。Dockerfile、配置文件和日志不得包含密码、API Key、令牌或私钥。环境专用配置文件使用带 `chatos` 或 `sandbox` 标识的新文件名，不覆盖项目原文件；用户可编辑值使用环境变量占位符。
