...
fn replace_pptx_chart_tool() -> Value {
    tool(
        "replace_pptx_chart",
        "Replace one uniquely owned standard chart only when its XML is byte-identical to the canonical self-contained column/line/pie/area/doughnut form generated by ChatOS. The bounded replacement may also update per-series RGB colors, legend position, value/percentage data labels, and axis titles where supported. Requires the exact chart XML SHA-256 and complete edit snapshot from inspect_pptx_charts, writes a distinct PPTX, and preserves the slide frame, relationship, chart part name, and every unrelated package entry.",
        json!({
            "type":"object",
            "properties":{
...
fn create_pptx_tool() -> Value {
    ...
    tool(
        "create_pptx",
        "Create a bounded editable widescreen PPTX locally with title/body, title-only, section, two-column, image-right, full-image, simple rectangular table, or self-contained standard DrawingML column/line/pie/area/doughnut chart layouts with optional canonical RGB series colors, optional PNG/JPEG images, bullet lines, and speaker notes.",
        json!({
            "type":"object",
            "properties":{
...
fn self_contained_pptx_chart_schema(description: &str, complete_snapshot: bool) -> Value {
    ...
    let series_required = if complete_snapshot {
        json!(["name", "values", "value_axis", "color"])
    } else {
        json!(["name", "values"])
    };
    json!({
