...
export function McpCatalogPage({ user }: McpCatalogPageProps) {
  ...
  const saveOptimizedSkillMutation = useMutation({
    ...
    onSuccess: async () => {
      ...
      await queryClient.invalidateQueries({ queryKey: ['plugin-management', 'mcp-descriptor'] });
    },
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      setModalOpen(false);
      setEditing(null);
      queryClient.invalidateQueries({ queryKey: ['plugin-management', 'mcps'] });
    },
    onError: (error) => message.error((error as Error).message),
  });
