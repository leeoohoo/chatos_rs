...
start_backend() {
  launch_service \
    ...
    "$BACKEND_PID_FILE" \
    "$BACKEND_LOG_FILE" \
    "cd \"$BACKEND_DIR\" && CARGO_TARGET_DIR=\"$MEMORY_ENGINE_CARGO_TARGET_DIR\" MEMORY_ENGINE_HOST=\"$MEMORY_ENGINE_HOST\" MEMORY_ENGINE_PORT=\"$MEMORY_ENGINE_PORT\" MEMORY_ENGINE_MONGODB_URI=\"$MEMORY_ENGINE_MONGODB_URI\" MEMORY_ENGINE_MONGODB_DATABASE=\"$MEMORY_ENGINE_MONGODB_DATABASE\" MEMORY_ENGINE_OPERATOR_TOKEN=\"$MEMORY_ENGINE_OPERATOR_TOKEN\" exec cargo run --bin memory_engine"
}
...
start_frontend() {
  ...
  launch_service \
    ...
    "$FRONTEND_PID_FILE" \
    "$FRONTEND_LOG_FILE" \
    "cd \"$FRONTEND_DIR\" && VITE_MEMORY_ENGINE_API_BASE=\"$MEMORY_ENGINE_BASE_URL\" VITE_MEMORY_ENGINE_PORT=\"$MEMORY_ENGINE_PORT\" VITE_MEMORY_ENGINE_OPERATOR_TOKEN=\"$MEMORY_ENGINE_OPERATOR_TOKEN\" exec npm run dev -- --host 0.0.0.0 --port \"$MEMORY_ENGINE_FRONTEND_PORT\" --strictPort"
}
