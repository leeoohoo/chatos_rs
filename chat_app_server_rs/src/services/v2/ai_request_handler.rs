...
impl AiRequestHandler {
    ...
    pub async fn handle_request(
        ...
        if stream {
            ...
            self.handle_normal_request(url, payload, reasoning_enabled, session_id, token)
                .await
        }
    }
    ...
    async fn handle_stream_request(
        ...
        let mut tool_calls_map: std::collections::BTreeMap<usize, Value> =
                                ...
                                delta.get("tool_calls").and_then(|v| v.as_array())
                            {
                                for tc in tool_calls {
                                    ...
                                    if let Some(func) = tc.get("function") {
                                        ...
                                        {
                                            let cur = entry["function"]["name"]
                                                .as_str()
                                                .unwrap_or("")
                                                .to_string();
                                            entry["function"]["name"] =
                                                Value::String(format!("{}{}", cur, name));
                                        }
                                        if let Some(args) =
