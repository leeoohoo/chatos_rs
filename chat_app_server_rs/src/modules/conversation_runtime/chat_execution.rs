...
use crate::services::v2::ai_server::{AiServer as AiServerV2, ChatOptions as ChatOptionsV2};
use crate::services::v2::mcp_tool_execute::McpToolExecute as McpToolExecuteV2;
...
pub struct PreparedMcpExecutionV2 {
    pub executor: McpToolExecuteV2,
    pub unavailable_tools: Vec<Value>,
    pub prefixed_messages: Vec<Value>,
...
pub fn init_ai_server_v2(model_runtime: &ResolvedChatModelConfig) -> Result<AiServerV2, String> {
    AiServerV2::new(
        model_runtime.api_key.clone(),
        model_runtime.base_url.clone(),
        model_runtime.model.clone(),
        model_runtime.temperature,
        McpToolExecuteV2::new(Vec::new(), Vec::new(), Vec::new()),
    )
}
...
pub async fn prepare_mcp_execution_v2(
    session_id: &str,
    turn_id: &str,
    runtime_context: &mut ResolvedConversationRuntimeContext,
) -> PreparedMcpExecutionV2 {
    let (http_servers, stdio_servers, builtin_servers) = runtime_context.mcp_server_bundle.clone();
    let mut executor = McpToolExecuteV2::new(http_servers, stdio_servers, builtin_servers.clone());
    ...
    let prefixed_messages = if let Some(context) =
        build_task_board_runtime_context(session_id, turn_id, runtime_context)
    {
        load_prefixed_messages(&context).await.unwrap_or_default()
    } else {
        Vec::new()
    };
...
pub fn configure_ai_server_v2(
    ai_server: &mut AiServerV2,
    session_id: &str,
    turn_id: &str,
    runtime_context: &ResolvedConversationRuntimeContext,
    effective_settings: &Value,
    executor: McpToolExecuteV2,
) {
...
pub fn build_chat_options_v2(
    ...
    ChatOptionsV2 {
        ...
        prefixed_messages: Some(prefixed_messages),
    }
}
