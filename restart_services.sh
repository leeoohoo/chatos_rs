...
CHATOS_PROJECT_SERVICE_BASE_URL_EFFECTIVE="${CHATOS_PROJECT_SERVICE_BASE_URL:-${PROJECT_SERVICE_BASE_URL:-http://${PROJECT_SERVICE_HOST_EFFECTIVE}:${PROJECT_SERVICE_PORT_EFFECTIVE}}}"
PROJECT_SERVICE_SYNC_SECRET_EFFECTIVE="${PROJECT_SERVICE_SYNC_SECRET:-${CHATOS_PROJECT_SERVICE_SYNC_SECRET:-change_me_project_sync_secret}}"
...
build_main_backend() {
  echo "[INFO] building main backend..."
  (
    cd "$MAIN_BACKEND_DIR"
    if [[ -f .env ]]; then
      set -a
    ...
    cargo build --bin chat_app_server_rs
  )
}
