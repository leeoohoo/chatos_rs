  ...
  const submitPromptMutation = useMutation({
    ...
    onSuccess: async () => {
      await Promise.all([
        ...
        queryClient.invalidateQueries({ queryKey: ['task-runner', 'run-prompts'] }),
        queryClient.invalidateQueries({ queryKey: ['task-runner', 'run-events'] }),
        queryClient.invalidateQueries({ queryKey: ['task-runner', 'prompt'] }),
      ]);
