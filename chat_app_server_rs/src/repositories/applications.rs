    ...
    with_db(
        |db| {
            ...
            Box::pin(async move {
                ...
                let mut cursor = db.collection::<Document>("applications").find(filter, None).await.map_err(|e| e.to_string())?;
                let mut docs = Vec::new();
                while let Some(doc) = cursor.try_next().await.map_err(|e| e.to_string())? { docs.push(doc); }
                let mut items: Vec<Application> = docs.into_iter().filter_map(|d| normalize_doc(&d)).collect();
...
pub async fn get_application_by_id(id: &str) -> Result<Option<Application>, String> {
    with_db(
        |db| {
            let id = id.to_string();
            Box::pin(async move {
                let doc = db.collection::<Document>("applications").find_one(doc! { "id": id }, None).await.map_err(|e| e.to_string())?;
...
pub async fn delete_application(id: &str) -> Result<(), String> {
    with_db(
        |db| {
            let id = id.to_string();
            Box::pin(async move {
                db.collection::<Document>("applications").delete_one(doc! { "id": &id }, None).await.map_err(|e| e.to_string())?;
                db.collection::<Document>("mcp_config_applications").delete_many(doc! { "application_id": &id }, None).await.map_err(|e| e.to_string())?;
                db.collection::<Document>("system_context_applications").delete_many(doc! { "application_id": &id }, None).await.map_err(|e| e.to_string())?;
                db.collection::<Document>("agent_applications").delete_many(doc! { "application_id": &id }, None).await.map_err(|e| e.to_string())?;
                Ok(())
            })
        },
        |pool| {
            let id = id.to_string();
            Box::pin(async move {
                sqlx::query("DELETE FROM applications WHERE id = ?").bind(&id).execute(pool).await.map_err(|e| e.to_string())?;
                Ok(())
            })
        }
    ).await
