...
fn option_base() -> RemoteConnectionControllerOptions {
    RemoteConnectionControllerOptions {
        ...
        default_remote_connection_id: Some("conn_default".to_string()),
...
#[test]
fn hides_all_remote_tools_when_default_connection_is_missing() {
    let mut options = option_base();
    options.default_remote_connection_id = None;
    let service = RemoteConnectionControllerService::new(options).expect("init");
    assert!(service.list_tools().is_empty());
    assert_eq!(service.unavailable_tools().len(), 6);
}

#[test]
fn bound_connection_id_is_not_exposed_in_any_tool_schema() {
    let service = RemoteConnectionControllerService::new(option_base()).expect("init");
    let tools = service.list_tools();
    assert_eq!(tools.len(), 6);
    assert!(!tools
        .iter()
        .any(|tool| tool.get("name") == Some(&json!("list_connections"))));
    for tool in &tools {
        assert!(tool
