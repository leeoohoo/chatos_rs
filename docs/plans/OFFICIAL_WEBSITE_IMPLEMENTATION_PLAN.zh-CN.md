...
当前实施状态：已落地 `official_website_service/` 官网微服务，并纳入统一 Docker Compose 云端栈。旧的独立启动脚本和单服务 Dockerfile 已移除。

已提供的常用入口：

- `docker/deploy.sh up`：随全量云端栈启动官网前端和后端。
- `docker/deploy.sh logs official-website-backend`：查看官网后端日志。
- `docker/deploy.sh logs official-website-frontend`：查看官网前端日志。
- `docker/deploy.sh build`：随全量云端栈构建官网镜像。
...
- 后端服务：`official-website-backend`
- 前端服务：`official-website-frontend`
- 配置入口：`docker/.env`
- 编排入口：`docker/compose.yml`
- 启动入口：`docker/deploy.sh up`

不再保留独立官网启动脚本和旧单服务 Dockerfile。
