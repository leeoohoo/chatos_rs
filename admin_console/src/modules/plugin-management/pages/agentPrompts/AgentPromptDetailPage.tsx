...
export function AgentPromptDetailPage({
  ...
  const promptsQuery = useQuery({
    queryKey: ['plugin-management', 'agent-provider-prompts', agent.agent_key],
    queryFn: () => api.listAgentProviderPrompts(agent.agent_key),
    enabled: editable,
  });
  const versionQuery = useQuery({
    queryKey: ['plugin-management',
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: (record, variables) => {
      ...
      void queryClient.invalidateQueries({ queryKey: ['plugin-management', 'agent-prompt-completeness'] });
      if (variables.publish) {
        void queryClient.invalidateQueries({ queryKey: ['plugin-management', 'agent-prompt-versions', agent.agent_key] });
      }
