...
if ! command -v openssl >/dev/null 2>&1; then
  echo "[ERROR] openssl is required to generate Configuration Center mTLS material" >&2
  exit 1
fi
...
material_is_current() {
  ...
  openssl verify -CAfile "$OUTPUT_DIR/ca.crt" "$OUTPUT_DIR/server.crt" >/dev/null 2>&1 || return 1
  local caller
...
if material_is_current; then
  echo "[INFO] Configuration Center mTLS material is current: $OUTPUT_DIR"
  exit 0
fi
...
openssl req -x509 -new -sha256 -days 3650 \
  ...
  -subj "/CN=ChatOS Configuration Center Internal CA" \
...
openssl req -new -sha256 \
  ...
  -subj "/CN=configuration-center-backend" \
  -out "$temporary_dir/server.csr"
cat >"$temporary_dir/server.ext" <<'EOF'
basicConstraints=critical,CA:FALSE
keyUsage=critical,digitalSignature,keyEncipherment
extendedKeyUsage=serverAuth
subjectAltName=DNS:configuration-center-backend,DNS:configuration-center,DNS:localhost,IP:127.0.0.1
EOF
...
echo "[INFO] generated Configuration Center mTLS material: $OUTPUT_DIR"
