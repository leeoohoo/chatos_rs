...
- `list_project_tasks`: List project-management work items; supports fuzzy `keyword`, `status`/`requirement_id`, and `limit`/`offset` pagination.
...
8. For every actionable requirement, call `list_project_tasks` with `requirement_id` to check existing coverage. If no project work item covers it, call `create_project_task` to create at least one actionable project work item. If the work item needs a specific skill, use the `task_runner_service_search_installed_skills` / `task_runner_service_get_skill_detail` tools visible in the current planning runtime first, then write the confirmed ids into `task_runner_skill_ids`. This is an internal tool-use self-check; do not write "at least one technical document/project task" or similar requirements into business acceptance criteria or technical documents.
