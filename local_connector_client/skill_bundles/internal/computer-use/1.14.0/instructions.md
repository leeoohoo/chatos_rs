# ChatOS Computer Use — Safe Native and Contenteditable Text Entry
...
- Windows retains the in-process implementation and the same approval, display identity, input release, privacy, and no-replay contracts.
...
This release adds fail-closed non-Edit contenteditable text entry on macOS and Windows while preserving the signed one-shot macOS helper, high-risk challenge, per-action approval, privacy audit, post-action observation, input-release, and no-automatic-replay contracts from `1.13.0`. Application-state rollback remains unavailable.
