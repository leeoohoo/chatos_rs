...
| CP-20260922-001 | Agent 团队与需求调研 | Core、SQLite、Connector、Presentation、WinUI | 待实现 | 项目级统一渐进调研、附件文本读取、跨会话 Inbox、run-scoped opaque refs、文档发送 receipt、lane 工具裁剪、成员提案原子审批，以及 Todo 不可变执行合同/来源关系/builtin 能力快照/原子串行调度已补；继续实现 Plugin 选择与独立 executor lane |
...
- 已确认剩余代码差距：Todo 的 Plugin 选择快照、独立 executor lane，以及触发图片/PDF 多模态输入和项目级调研中心 UI。
