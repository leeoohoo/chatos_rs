...
const ProjectManagementAgentModelSettingsPanel: React.FC<Props> = ({ onClose }) => {
  const { t } = useI18n();
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then(([, settings]) => {
        ...
        setSelectedModelId(settings.project_management_agent_model_config_id || '');
        setThinkingLevel(settings.project_management_agent_thinking_level || '');
      })
      .catch((err) => {
        if (!cancelled) {
  ...
  const save = async () => {
    ...
    try {
      await client.updateAiModelSettings({
        project_management_agent_model_config_id: selectedModelId || null,
        project_management_agent_thinking_level: thinkingLevel || null,
      });
      onClose();
    } catch (err) {
  ...
  return (
    <ManagerFormDialog
      open
      title={t('projectManagementAgentModelSettings.title')}
      description={t('projectManagementAgentModelSettings.description')}
      ...
      <div className="space-y-4">
        ...
        <div>
          <label className="mb-2 block text-sm font-medium text-foreground">
            {t('projectManagementAgentModelSettings.model')}
          </label>
          <select
...
export default ProjectManagementAgentModelSettingsPanel;
