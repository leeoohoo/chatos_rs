...
Before calling `solution_upsert_design`, build a short requirement-to-block delivery inventory and check each primary candidate for: explanatory text, the required architecture or flow, any requested interface SVG, stable block IDs, and valid standalone SVG. Persist large SVGs individually with `solution_upsert_design_block`; if validation fails, correct the inline SVG and retry the same block rather than writing an external file. After the calls, read the saved workspace and verify every inventory ID and type exists, render each saved visual, and pass that same inventory to `solution_finalize`.
