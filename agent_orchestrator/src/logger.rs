...
const LOG_DIR_ENV: &str = "AGENT_ORCHESTRATOR_LOG_DIR";
...
fn migrate_legacy_logs_if_needed(target: &Path) -> Result<(), String> {
    let repo_root = Path::new(env!("CARGO_MANIFEST_DIR"))
        .parent()
        .unwrap_or_else(|| Path::new(env!("CARGO_MANIFEST_DIR")));
    let mut legacy_dirs = vec![Path::new(env!("CARGO_MANIFEST_DIR")).join("logs")];
    legacy_dirs.extend(discover_runtime_log_dirs(repo_root, target));
    ...
    for legacy_dir in legacy_dirs {
        ...
        fs::create_dir_all(target)
            .map_err(|err| format!("create migrated log dir failed: {err}"))?;
...
fn discover_runtime_log_dirs(repo_root: &Path, target: &Path) -> Vec<PathBuf> {
    let mut out = Vec::new();
    let local_root = repo_root.join(".local");
    let Ok(entries) = fs::read_dir(local_root) else {
        return out;
    };
    for entry in entries.flatten() {
        let candidate = entry.path().join("logs");
        if candidate == target || !candidate.is_dir() {
            continue;
        }
        out.push(candidate);
    }
