...
use crate::services::task_service_client::{self, TaskExecutionScopeDto, TaskRecordDto};
...
pub(super) struct PreparedTaskRuntime {
    pub(super) ai_server: AiServer,
    pub(super) model_runtime: ResolvedChatModelConfig,
    pub(super) runtime_context: crate::api::chat_stream_common::ResolvedChatStreamContext,
    pub(super) runtime_session_key: String,
    pub(super) max_tokens: Option<i64>,
}

impl PreparedTaskRuntime {
    pub(super) fn chat_options(&self, turn_suffix: &str, _task: Option<&TaskRecordDto>) -> ChatOptions {
        ChatOptions {
...
pub(super) async fn build_task_runtime(
    ...
    let effective_task_system_prompt = build_task_execution_system_prompt(&runtime_context, task);
...
pub(super) fn is_task_context_overflow_error(err: &str) -> bool {
...
fn build_task_execution_system_prompt(
    ...
    if let Some(task) = task {
        ...
        if !task.depends_on_task_ids.is_empty() {
            lines.push(format!("直接前置任务={}", task.depends_on_task_ids.join(", ")));
        }
...
async fn build_task_dependency_handoff_context(
    ...
    for task_id in task
        .depends_on_task_ids
        .iter()
        .chain(task.verification_of_task_ids.iter())
    {
        let normalized = task_id.trim();
        if normalized.is_empty()
            || related_task_ids
                .iter()
                .any(|item: &String| item == normalized)
        {
            continue;
        }
