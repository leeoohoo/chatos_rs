...
#[derive(Debug, Clone)]
pub struct PluginManagementInternalTlsConfig {
...
impl PluginManagementInternalTlsConfig {
    pub fn from_env(host: IpAddr, public_port: u16) -> Result<Self, String> {
        let port = required_env("PLUGIN_MANAGEMENT_INTERNAL_MTLS_PORT")?
            .parse::<u16>()
            .map_err(|err| {
                format!("PLUGIN_MANAGEMENT_INTERNAL_MTLS_PORT must be a valid port: {err}")
            })?;
        if port == public_port {
            return Err(
                "PLUGIN_MANAGEMENT_INTERNAL_MTLS_PORT must differ from PLUGIN_MANAGEMENT_SERVICE_PORT"
                    .to_string(),
            );
        }
        Ok(Self {
            bind_addr: SocketAddr::new(host, port),
            server_cert_path: PathBuf::from(required_env(
                "PLUGIN_MANAGEMENT_MTLS_SERVER_CERT_PATH",
            )?),
            server_key_path: PathBuf::from(required_env("PLUGIN_MANAGEMENT_MTLS_SERVER_KEY_PATH")?),
            client_ca_cert_path: PathBuf::from(required_env(
                "PLUGIN_MANAGEMENT_MTLS_CLIENT_CA_CERT_PATH",
            )?),
        })
    }
}
...
pub fn load_internal_mtls_config(
    config: &PluginManagementInternalTlsConfig,
...
fn load_internal_mtls_config_from_paths(
    ...
    for certificate in read_certificates(client_ca_cert_path)? {
        client_roots
            .add(certificate)
            .map_err(|err| format!("invalid Plugin Management mTLS client CA: {err}"))?;
    }
    if client_roots.is_empty() {
        return Err("Plugin Management mTLS client CA contains no certificates".to_string());
    }
    let verifier = WebPkiClientVerifier::builder(Arc::new(client_roots))
        .build()
        .map_err(|err| format!("build Plugin Management mTLS client verifier failed: {err}"))?;
    let server_config = ServerConfig::builder()
        .with_client_cert_verifier(verifier)
        .with_single_cert(server_certificates, server_key)
        .map_err(|err| format!("build Plugin Management mTLS server config failed: {err}"))?;
...
mod tests {
    ...
    fn unique_test_dir(label: &str) -> PathBuf {
        ...
        std::env::temp_dir().join(format!(
            "chatos-plugin-management-mtls-{label}-{}-{nonce}",
            std::process::id()
        ))
    }
    ...
    fn generate_material(output_dir: &PathBuf) {
        let script = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
            .join("../../scripts/generate-plugin-management-mtls.sh");
        let status = Command::new(script)
            .arg(output_dir)
            .status()
            .expect("run Plugin Management mTLS generator");
    ...
    async fn internal_listener_requires_a_trusted_client_certificate_and_tls() {
        ...
        let identity_pem = std::fs::read(material_dir.join("mcp-management-service.identity.pem"))
