...
fn load_internal_mtls_config_from_paths(
    ...
    for certificate in client_ca_certificates {
        client_roots.add(certificate).map_err(|err| {
            format!("invalid Configuration Center mTLS client CA certificate: {err}")
        })?;
    }
    if client_roots.is_empty() {
        return Err("Configuration Center mTLS client CA contains no certificates".to_string());
    }
    let client_verifier = WebPkiClientVerifier::builder(Arc::new(client_roots))
        .build()
        .map_err(|err| format!("build Configuration Center mTLS client verifier failed: {err}"))?;
    let server_config = ServerConfig::builder()
        .with_client_cert_verifier(client_verifier)
        .with_single_cert(server_certificates, server_key)
        .map_err(|err| format!("build Configuration Center mTLS server config failed: {err}"))?;
